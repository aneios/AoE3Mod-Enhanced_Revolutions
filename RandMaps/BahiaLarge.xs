...
if (rmGetIsTreaty() == true)
	forestConstraint = rmCreateClassDistanceConstraint("forest vs. forest", rmClassID("classForest"), 20);
else
	forestConstraint = rmCreateClassDistanceConstraint("forest vs. forest", rmClassID("classForest"), 30);
int forestConstraintSmall = rmCreateClassDistanceConstraint("forest vs. forest small", rmClassID("classForest"), 18);
int avoidTownCenter1 = rmCreateTypeDistanceConstraint("avoid Town Center", "townCenter", 27);
...
if (cNumberTeams > 2) {
	...
	rmSetPlacementTeam(0);
	rmPlacePlayersLine(0.15, 0.78, 0.50, 0.78, 0, 0);
	rmSetPlacementTeam(1);
	rmPlacePlayersLine(0.15, 0.22, 0.50, 0.22, 0, 0); 
}
...
if ((PlayerNum == 2)) {
	rmPlaceObjectDefAtLoc(silverID, 0, 0.12, 0.92);
	...
	if (rmGetIsTreaty() == false) {
		...
		rmPlaceObjectDefAtLoc(silverID, 0, 0.50, 0.7);
		rmPlaceObjectDefAtLoc(silverID, 0, 0.50, 0.3);
		rmPlaceObjectDefAtLoc(silverID, 0, 0.05, 0.65);
		rmPlaceObjectDefAtLoc(silverID, 0, 0.05, 0.35);
		rmPlaceObjectDefAtLoc(herdID, 0, 0.42, 0.65);
		rmPlaceObjectDefAtLoc(herdID, 0, 0.42, 0.35);
		}
	...
	rmPlaceObjectDefAtLoc(silverID, 0, 0.14, 0.6);
	rmPlaceObjectDefAtLoc(silverID, 0, 0.14, 0.4);
	rmPlaceObjectDefAtLoc(herdID, 0, 0.48, 0.3, 1);
	rmPlaceObjectDefAtLoc(herdID, 0, 0.48, 0.7, 1);
	rmPlaceObjectDefAtLoc(herdID, 0, 0.48, 0.3, 1);
	rmPlaceObjectDefAtLoc(herdID, 0, 0.48, 0.7, 1);
...
float forestPass = (8 * numPlayer);
if (rmGetIsTreaty() == true)
	forestPass = 10+10*numPlayer;

float forestFailCount = 0;
for (i=0;  < forestPass) {
	int forestNorth = rmCreateArea(("Forest north " + i), terrainMix2);
	rmSetAreaSize(forestNorth, rmAreaTilesToFraction(150));
...
int fishVsFishID = rmCreateTypeDistanceConstraint("fish v fish", "fish", 24);
int whaleVsWhale = rmCreateTypeDistanceConstraint("whale v whale", "humpbackWhale", 64);
int fishLand = rmCreateTerrainDistanceConstraint("fish land", "land", true, 8);
for (i=0;  < ((PlayerNum * 2) + 2)) {
...
int underbrushID = rmCreateObjectDef("grass props");
	...
	rmPlaceObjectDefAtLoc(underbrushID, 0, 0.50, 0.50, 50*PlayerNum);
...
int PlateauunderbrushID = rmCreateObjectDef("plateau props");
	...
	rmPlaceObjectDefAtLoc(PlateauunderbrushID, 0, 0.50, 0.50, 5*PlayerNum);
