...
//	int avoidGoldVeryFar = rmCreateClassDistanceConstraint ("gold avoid gold very ", rmClassID("Gold"), 74.0);
	...
	int avoidWhale=rmCreateTypeDistanceConstraint("avoid whale", "fish", 34.0-cNumberNonGaiaPlayers/2);
	...
	int whaleLand = rmCreateTerrainDistanceConstraint("whales avoid land", "Land", true, 12.0);
...
//	rmAddObjectDefConstraint(playergold3ID, avoidCenter);
	...
	rmAddObjectDefItem(playerTreeID, "TreeSonora", rmRandInt(2,2), 2.0);
...
*/
	...
	if (PlayerNum == 2)
		rmSetObjectDefMaxDistance(whaleID, rmXFractionToMeters(0.03));
	else
	{
...
//		rmAddObjectDefConstraint(whaleID, whaleLand);
	...
	if (PlayerNum == 2)
	{
		rmPlaceObjectDefAtLoc(whaleID, 0, 0.05, 0.80);
		rmPlaceObjectDefAtLoc(whaleID, 0, 0.05, 0.50);
		rmPlaceObjectDefAtLoc(whaleID, 0, 0.05, 0.20);
		rmPlaceObjectDefAtLoc(whaleID, 0, 0.95, 0.80);
		rmPlaceObjectDefAtLoc(whaleID, 0, 0.95, 0.50);
		rmPlaceObjectDefAtLoc(whaleID, 0, 0.95, 0.20);
	}
	else
	{
		for (i=0; < whalecount)
