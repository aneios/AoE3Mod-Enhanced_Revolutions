// LARGE BAJA CALIFORNIA (1v1, TEAM, FFA)
// designed by Garja
//Ported by Durokan and Interjection for DE
// updated December 2021 by vividlyplain - no more whales clipping through land, we're all safe now : ]
...
void main(void)
{
	...
	rmSetSeaLevel(1);	// this is height of river surface compared to surrounding land. River depth is in the river XML.
...
//	rmSetMapElevationHeightBlend(0.3); //
	...
	rmTerrainInitialize("california\desert5_cal", 3.0);
//	rmTerrainInitialize("water"); //
	rmSetMapType("california");
	rmSetMapType("desert");
//	rmSetMapType("grass");
	rmSetMapType("water");
	rmSetLightingSet("Baja_California_Skirmish");
	...
	int avoidForest=rmCreateClassDistanceConstraint("avoid forest", rmClassID("Forest"), 32.0+PlayerNum); //15.0
	...
	int avoidPronghornFar = rmCreateTypeDistanceConstraint("avoid pronghorn far", "pronghorn", 75.0);
	int avoidPronghorn = rmCreateTypeDistanceConstraint("avoid pronghorn", "pronghorn", 56.0);
	...
	int avoidBighornFar = rmCreateTypeDistanceConstraint("avoid bighorn far", "bighornsheep", 72.0);
	int avoidBighorn = rmCreateTypeDistanceConstraint("avoid  bighorn", "bighornsheep", 66.0);
	...
	int avoidGoldFar = rmCreateClassDistanceConstraint ("gold avoid gold far", rmClassID("Gold"), 75.0); // 82
//	int avoidGoldVeryFar = rmCreateClassDistanceConstraint ("gold avoid gold very ", rmClassID("Gold"), 74.0);
	...
	int avoidNuggetFar = rmCreateTypeDistanceConstraint("nugget avoid nugget Far", "AbstractNugget", 72.0);
	...
	int avoidIslandShort  = rmCreateClassDistanceConstraint("avoid island short", rmClassID("island"), 8.0);
	int avoidIsland  = rmCreateClassDistanceConstraint("avoid island", rmClassID("island"), 16.0);
	...
	int avoidWhale=rmCreateTypeDistanceConstraint("avoid whale", "fish", 55.0-cNumberNonGaiaPlayers/2);
	...
	int avoidLand = rmCreateTerrainDistanceConstraint("avoid land ", "Land", true, 6);
	...
	int whaleLand = rmCreateTerrainDistanceConstraint("whales avoid land", "Land", true, 16.0);
				...
				if (teamZeroCount == 1 || teamOneCount == 1) // one team is one player
				{
					if (teamZeroCount < teamOneCount) // 1v2, 1v3, 1v4, etc.
					{
						...
						if (teamOneCount == 2)
							...
							rmPlacePlayersLine(0.83, 0.60, 0.64, 0.07, 0.00, 0.25);
					}
					else // 2v1, 3v1, 4v1, etc.
					{
						...
						if (teamZeroCount == 2)
							...
							rmPlacePlayersLine(0.36, 0.93, 0.17, 0.40, 0.00, 0.25);
					}
				}
	...
	for (i=0; < 6)
	{
        ...
        rmSetAreaHeightBlend(SeaHoleID, 2);
...
//	rmSetAreaCliffPainting(pass4ID, true, true, true, 0.1 , false); //  paintGround,  paintOutsideEdge,  paintSide,  minSideHeight,  paintInsideEdge
	...
	for (i=1; < numPlayer)
	{
		...
		rmSetAreaSize(playerareaID, rmAreaTilesToFraction(115));
...
//	rmAddGroupingConstraint(nativeID1, avoidNatives);
	...
	if (cNumberTeams == 2 && teamZeroCount_dk != teamOneCount_dk)
		rmSetObjectDefMaxDistance(TCID, 16.0);
	else
        rmSetObjectDefMaxDistance(TCID, 0.0);
	rmAddObjectDefConstraint(TCID, avoidStartingResourcesShort);
...
//	rmAddObjectDefToClass(playergold2ID, classStartingResource);
	...
	rmAddObjectDefConstraint(playergold2ID, avoidIsland);
...
//	rmAddObjectDefConstraint(playergold3ID, avoidWater);
	...
	rmAddObjectDefConstraint(playergold3ID, avoidIsland);
...
//	rmAddObjectDefToClass(playerTree2ID, classForest);
	...
	rmAddObjectDefItem(playerherdID, "pronghorn", 7, 3.0);
	...
	rmAddObjectDefConstraint(playerherdID, avoidGoldMin);
...
//	rmAddObjectDefConstraint(playerherdID, avoidStartingResourcesShort);
	...
	rmAddObjectDefItem(player2ndherdID, "pronghorn", rmRandInt(10,10), 4.0);
...
//	rmAddObjectDefConstraint(player2ndherdID, avoidTradeRouteShort);
	...
	rmAddObjectDefConstraint(player2ndherdID, avoidIsland);
	rmAddObjectDefConstraint(player2ndherdID, avoidNativesShort);

// 3nd herd
	int player3rdherdID = rmCreateObjectDef("player 3rd herd");
	rmAddObjectDefItem(player3rdherdID, "pronghorn", rmRandInt(8,8), 4.0);
	...
	rmAddObjectDefConstraint(player3rdherdID, avoidIsland);
...
//	rmAddObjectDefConstraint(player3rdherdID, avoidTradeRouteShort);
	...
	rmAddObjectDefConstraint(playerNuggetID, avoidIsland);
...
//		rmPlaceObjectDefAtLoc(playerTreeID, i, rmXMetersToFraction(xsVectorGetX(TCLoc)), rmZMetersToFraction(xsVectorGetZ(TCLoc)));
	...
	for(i=0; < minecount)
	{
		...
		rmAddObjectDefConstraint(commongoldID, avoidIsland);
...
//		rmSetAreaObeyWorldCircleConstraint(forestID, false);
		...
		rmAddAreaConstraint(forestID, avoidIsland);
...
//		rmAddAreaConstraint(forestID, avoidEdge);
	...
	for (i=0; < randomtreecount)
	{
		...
		rmAddObjectDefConstraint(randomtreeID, avoidIsland);
	...
	int centralforestcount = 4+4*PlayerNum;
	...
	for (i=0; < centralforestcount)
	{
		...
		rmAddObjectDefConstraint(centraltreebigID, avoidIsland);
	...
	for (i=0; < centralforestcount-PlayerNum/2)
	{
		...
		rmAddObjectDefConstraint(centraltreesmallID, avoidIsland);
	...
	//Pronghorn herds
	int pronghorncount = 1+4*PlayerNum;
	...
	for (i=0; < pronghorncount)
	{
		...
		rmAddObjectDefConstraint(pronghornherdID, avoidIsland);
	...
	//Bighorn herds
	int bighorncount = 1+4*PlayerNum;
	...
	for (i=0; < bighorncount)
	{
		...
		rmAddObjectDefConstraint(bighornherdID, avoidIsland);
...
//	}
	...
	for (i=1; < treasure1count+PlayerNum+1)
	{
		...
		rmAddObjectDefConstraint(Nugget1ID, avoidIsland);
...
*/
	...
	for (i=0; < 100)
	{
		...
		rmAddObjectDefConstraint(UnderbrushcliffID, avoidIsland);
