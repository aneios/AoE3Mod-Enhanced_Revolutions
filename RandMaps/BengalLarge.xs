...
//	rmSetMapElevationHeightBlend(1);
	...
	rmSetSeaType("Bengal_Marsh");
	rmSetBaseTerrainMix("deccan_grass_b"); // 
	rmTerrainInitialize("water", 0.0); // 
	rmSetMapType("deccan"); 
	rmSetMapType("grass");
	rmSetMapType("water");
	rmSetLightingSet("Bengal_Skirmish"); //age3_demo
	...
	int avoidNilgauFar = rmCreateTypeDistanceConstraint("avoid Nilgau far", "ypNilgai", 66.0); //48
	...
	int avoidEleFar = rmCreateTypeDistanceConstraint("avoid Ele far", "ypWildElephant", 65.0);
	...
	int avoidGold = rmCreateClassDistanceConstraint ("gold avoid gold med", rmClassID("Gold"), 44.0);
	int avoidGoldFar = rmCreateClassDistanceConstraint ("gold avoid gold far", rmClassID("Gold"), 72.0);
	int avoidGoldVeryFar = rmCreateClassDistanceConstraint ("gold avoid gold very far", rmClassID("Gold"), 90.0);
	...
	int avoidNugget=rmCreateTypeDistanceConstraint("nugget avoid nugget", "AbstractNugget", 66.0);
	int avoidNuggetFar = rmCreateTypeDistanceConstraint("nugget avoid nugget Far", "AbstractNugget", 85.0);
	...
	int avoidBuffalo=rmCreateTypeDistanceConstraint("avoid water buffalo", "ypWaterBuffalo", 85.0);
...
//			rmSetAreaMix(hillID, "deccan_grass_b");	
			rmSetAreaMix(hillID, "yellow_river_c");
...
//		rmSetAreaMix(playerislandID, "deccan_grass_b");
        rmSetAreaMix(playerislandID, "yellow_river_c");		// deccan_grass_a
...
*/
	...
	//	rmAddAreaToClass(playerareaID, classIsland);
		...
		rmSetAreaMix(playerareaID, "yellow_river_c");
...
*/
	...
	//Big islands
	for (i=0; <4+6*PlayerNum)
	{
		int BigislandID=rmCreateArea("Big island"+i);
		rmSetAreaSize(BigislandID, rmAreaTilesToFraction(450+25*PlayerNum), rmAreaTilesToFraction(470+33*PlayerNum)); //0.019, 0.021 rmAreaTilesToFraction(380), rmAreaTilesToFraction(420)
		rmSetAreaMix(BigislandID, "yellow_river_c");
...
//		rmAddAreaTerrainLayer(BigislandID, "yellow_river\forest_yellow_riv", 0, 2);
	...
	// 	rmAddConnectionArea(shallowsID, BigislandID);
		...
		rmAddAreaConstraint(BigislandID, avoidIsland);
...
//	rmSetGroupingMaxDistance(nativeID1, 0.00);
	...
	if (TeamNum <= 2) // 1v1 and TEAM
	{
		if (PlayerNum == 2)
		{
			for (i=0; < 2)
			{
				int natIslandID = rmCreateArea("native island"+i);
				rmSetAreaSize(natIslandID, rmAreaTilesToFraction(400));
				if (i < 1)
					rmSetAreaLocation(natIslandID,  0.31, 0.77);
				else 
					rmSetAreaLocation(natIslandID,  0.77, 0.31);
				rmAddAreaToClass(natIslandID, classIsland);
				rmSetAreaCoherence(natIslandID, 0.85);
				rmSetAreaObeyWorldCircleConstraint(natIslandID, false);
				rmSetAreaMix(natIslandID, "yellow_river_c");
				rmAddAreaTerrainLayer(natIslandID, "yellow_river\grass2_yellow_riv", 0, 5); 
				rmSetAreaWarnFailure(natIslandID, false);
				rmSetAreaObeyWorldCircleConstraint(natIslandID, false);
//				rmSetAreaSmoothDistance(natIslandID, 10);
//				rmSetAreaElevationType(natIslandID, cElevTurbulence);
//				rmSetAreaElevationVariation(natIslandID, 2.5);
				rmSetAreaBaseHeight(natIslandID, 3.0);
//				rmSetAreaElevationMinFrequency(natIslandID, 0.09);
//				rmSetAreaElevationOctaves(natIslandID, 3);
//				rmSetAreaElevationPersistence(natIslandID, 0.4);    
		...
		else
		{
			...
			//	rmAddAreaToClass(nativeareaID, classIsland);
				...
				rmSetAreaMix(nativeareaID, "yellow_river_c");
...
//				rmSetAreaElevationPersistence(nativeareaID, 0.4);    
		...
		if (PlayerNum >= 6)	
		{
			...
			//	rmAddAreaToClass(nativeareasouthID, classIsland);
				...
				rmSetAreaMix(nativeareasouthID, "yellow_river_c");
...
//				rmSetAreaElevationPersistence(nativeareamiddleID, 0.4);    
		...
		if (PlayerNum >= 6)	
		{
			...
			//	rmAddAreaToClass(nativeareanorthID, classIsland);
				...
				rmSetAreaMix(nativeareanorthID, "yellow_river_c");
...
//		rmAddObjectDefConstraint(commongoldID, avoidTradeRouteSocket);
	...
	// Inland forest
	int forestinlandcount = 2+8*PlayerNum;
	if (PlayerNum >= 4)
		forestinlandcount = 2+6*PlayerNum;
	if (PlayerNum >= 6)
		forestinlandcount = 2+4*PlayerNum;
...
//		rmAddAreaConstraint(forestinlandID, avoidEdge);
		...
		int forestinlandtreeID = rmCreateObjectDef("forest inland trees"+i);
		...
		//	rmAddObjectDefConstraint(forestinlandtreeID, avoidImpassableLandShort);
			...
			rmPlaceObjectDefAtLoc(forestinlandtreeID, 0, 0.50, 0.50, 8);
	}
	

	// Marsh forest
	int forestimarshcount = 2+8*PlayerNum;
	if (PlayerNum >= 4)
		forestimarshcount = 2+6*PlayerNum;
	if (PlayerNum >= 6)
		forestimarshcount = 2+4*PlayerNum;
...
//		rmSetAreaForestUnderbrush(forestmarshID, 0.2);
		...
		rmAddAreaConstraint(forestmarshID, avoidForest);
...
//		rmAddAreaConstraint(forestmarshID, avoidEdge);
		...
		int forestmarshtreeID = rmCreateObjectDef("forest marsh trees"+i);
...
//			rmAddObjectDefConstraint(forestmarshtreeID, avoidImpassableLandShort);
			...
			rmPlaceObjectDefAtLoc(forestmarshtreeID, 0, 0.50, 0.50, 8);
	}
	...
	//Nilgai herds
	int staticHerdID = rmCreateObjectDef("static herd for team");
	rmAddObjectDefItem(staticHerdID, "ypNilgai", 8, 6.0);
	rmSetObjectDefMinDistance(staticHerdID, 0.0);
	rmSetObjectDefMaxDistance(staticHerdID, rmXFractionToMeters(0.6));
	rmSetObjectDefCreateHerd(staticHerdID, true);
	rmAddObjectDefConstraint(staticHerdID, avoidForestMin);
	rmAddObjectDefConstraint(staticHerdID, avoidGoldShort);
	rmAddObjectDefConstraint(staticHerdID, avoidCliff);
	rmAddObjectDefConstraint(staticHerdID, avoidEdge);
	if (PlayerNum > 2) {
		rmPlaceObjectDefAtLoc(staticHerdID, 0, 0.85, 0.70);	
		rmPlaceObjectDefAtLoc(staticHerdID, 0, 0.70, 0.85);	
		}
...
//			rmAddObjectDefConstraint(lowgroundherdID, stayNE);
	...
	int treasure1count = 5+2*PlayerNum;
	int treasure2count = 3+PlayerNum;
	int treasure34count = 1+PlayerNum;

	
	// Treasures lvl4
	for (i=0; < treasure34count/2)
	{
...
//		rmAddObjectDefConstraint(Nugget4ID, avoidTradeRouteSocket);
		...
		rmAddObjectDefConstraint(Nugget4ID, avoidTownCenterMed);
		rmAddObjectDefConstraint(Nugget4ID, avoidNilgauMin); 
...
//			rmAddObjectDefConstraint(Nugget4ID, stayNorthHalf); 
		if (PlayerNum >= 4 && TeamNum <= 2 && rmGetIsTreaty() == false)
...
//		rmAddObjectDefConstraint(Nugget1ID, avoidTradeRouteSocket);
	...
	int buffalocount = 4+2.5*PlayerNum;
