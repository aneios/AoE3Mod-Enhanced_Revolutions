// LARGE CENTRAL PLAIN (1V1, TEAM, FFA)
// designed by Garja
...
//	int avoidForestFar=rmCreateClassDistanceConstraint("avoid forest far", rmClassID("Forest"), 38.0); //45.0
	...
	int avoidypSerowFar = rmCreateTypeDistanceConstraint("avoid ypSerow far", "ypSerow", 65.0);
	...
	int avoidYPMuskDeerFar = rmCreateTypeDistanceConstraint("avoid YPMuskDeer far", "YPMuskDeer", 65.0);
...
*/
	...
	for (i=0; < greenforestcount)
	{
		...
		rmSetAreaSize(greenforestID, rmAreaTilesToFraction(200), rmAreaTilesToFraction(250));
...
//		rmSetAreaObeyWorldCircleConstraint(greenforestID, false);
	...
	for (i=0; < greenforestsmallcount)
	{
		...
		rmSetAreaSize(greenforestsmallID, rmAreaTilesToFraction(125), rmAreaTilesToFraction(150));
...
//		rmSetAreaObeyWorldCircleConstraint(greenforestsmallID, false);
	...
	for (i=0; < dirtforestcount)
	{
		...
		rmSetAreaSize(dirtforestID, rmAreaTilesToFraction(125), rmAreaTilesToFraction(150));
//		rmSetAreaObeyWorldCircleConstraint(dirtforestID, false);
		...
		for (j=0; < rmRandInt(8,10))
		{
			...
			rmAddObjectDefItem(dirtforesttreeID, "ypTreeGinkgo", rmRandInt(1,2), 4.0);
