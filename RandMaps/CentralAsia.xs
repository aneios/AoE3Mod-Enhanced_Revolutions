...
//	rmSetBaseTerrainMix(paintMix1); 
	...
	int avoidForestMed=rmCreateClassDistanceConstraint("avoid forest med", rmClassID("Forest"), 12.0);
	...
	int avoidGoldTypeShort = rmCreateTypeDistanceConstraint("coin avoids coin short", "gold", 30-1.5*PlayerNum);
	...
	int avoidGoldFar = rmCreateClassDistanceConstraint ("gold avoid gold far", rmClassID("Gold"), 50-PlayerNum);
...
//	rmSetAreaMix(midLineID, forTesting); 	
	...
	int bigMidIslandID=rmCreateArea("Big Mid Island");
	rmSetAreaSize(bigMidIslandID, 0.62+.005*PlayerNum);
	rmSetAreaLocation(bigMidIslandID, 0.5, 0.5);
//	rmSetAreaMix(bigMidIslandID, "italy_snow"); 		// for testing
	rmSetAreaCoherence(bigMidIslandID, 1.00);
	rmBuildArea(bigMidIslandID); 
	
	int avoidBigMidIsland = rmCreateAreaDistanceConstraint("avoid big mid island ", bigMidIslandID, 8.0);
	int avoidBigMidIslandMin = rmCreateAreaDistanceConstraint("avoid big mid island min", bigMidIslandID, 0.5);
	int avoidBigMidIslandFar = rmCreateAreaDistanceConstraint("avoid big mid island far", bigMidIslandID, 16.0);
	int stayBigMidIsland = rmCreateAreaMaxDistanceConstraint("stay big mid island ", bigMidIslandID, 0.0);
...
//	rmAddAreaConstraint(midSmIslandID, avoidWaterShort);
	...
	for (i=0; < pondcount)
	{
		...
		rmAddObjectDefItem(fishID, fishies, 2, 4.0);
		...
		rmPlaceObjectDefAtLoc(fishID, 0, 0.50, 0.50, 6+3*PlayerNum);
	}
...
//		rmSetAreaMix(avoidTHisID, "testmix");
    ...
    rmSetObjectDefMinDistance(playerGold3ID, 55);
	...
	if (TeamNum == 2)
	{
		...
		rmAddObjectDefConstraint(playerGold3ID, stayMidSmIsland);
	}
	else
		rmAddObjectDefConstraint(playerGold3ID, avoidMidIslandFar);
	...
	rmAddObjectDefConstraint(playerHerd4ID, stayDryGrass);
...
//			rmPlaceObjectDefAtLoc(playerGold3ID, i, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));
	...
	int riverforestcount = 20+10*PlayerNum;
...
//			rmAddObjectDefItem(riverForestID, "berrybush", 1, 3);
	...
	// Main Forest
	int mainforestcount = 14+2*PlayerNum;
...
//		rmAddObjectDefConstraint(mapHerd2ID, avoidForestMin);
	...
	int herd1count = PlayerNum;
	if (PlayerNum > 2)
		herd1count = 2*PlayerNum;
	...
	int treasure1count = 6+2*PlayerNum;
