...
void main(void)
{
  ...
  int islandEdgeConstraint=rmCreatePieConstraint("island edge of map", 0.5, 0.5, 0, rmGetMapXSize()-5, 0, 0, 0);
	...
	int avoidFish1=rmCreateTypeDistanceConstraint("fish v fish", fish1, 30.0);	
	...
	int whaleVsWhaleID=rmCreateTypeDistanceConstraint("whale v whale", whale1, 105.0);	
  ...
  int avoidGold=rmCreateTypeDistanceConstraint("avoid gold", "minegold", 65.0);
	int avoidRandomBerries=rmCreateTypeDistanceConstraint("avoid random berries", "berrybush", 75.0);
	int avoidHuntable1=rmCreateTypeDistanceConstraint("avoid huntable1", huntable1, 50.0);
  ...
  int avoidNuggetWater=rmCreateTypeDistanceConstraint("avoid water nuggets", "ypNuggetBoat", 100.0); 
  int avoidNuggetWater2=rmCreateTypeDistanceConstraint("avoid water nuggets2", "ypNuggetBoat", 150.0); 
  ...
  int flagLand = rmCreateTerrainDistanceConstraint("flag vs land", "land", true, 15.0);
	...
	rmSetAreaSize(bigIslandID, 0.21);
  ...
  int cliffSetup = 0;
	...
	for(i=0; <numTries){
		...
		rmSetAreaSize(smallMesaID, rmAreaTilesToFraction(300+50*cNumberNonGaiaPlayers), rmAreaTilesToFraction(350+75*cNumberNonGaiaPlayers));
