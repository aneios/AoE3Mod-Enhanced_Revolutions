...
void main(void)
{
	...
	rmSetMapType("colorado");
	rmSetMapType("grass");
	rmSetMapType("land");
	rmSetLightingSet("colorado_skirmish");
	...
	int avoidCoinStart = rmCreateTypeDistanceConstraint("avoid coin starts", "gold", 30.0);
	...
	int avoidBighornSheepMin = rmCreateTypeDistanceConstraint("avoid bighornsheep min", "BighornSheep", 4.0);
	...
	rmSetAreaTerrainType(cliffID1, "rockies\groundsnow6_roc");
	...
	rmAddAreaTerrainLayer(middleGrass, "rockies\groundsnow6_roc", 0, 2);
	rmAddAreaTerrainLayer(middleGrass, "rockies\ground1_roc", 2, 6);
	rmAddAreaTerrainLayer(middleGrass, "rockies\ground3_roc", 6, 10);
...
{
	natType = "Comanche";
	natGrpName = "native Comanche village ";
}
else if (whichNative == 2)
{
	natType = "Cheyenne";
	natGrpName = "native Cheyenne village ";
}
else
{
	natType = "Navajo";
	natGrpName = "native Navajo village ";
}

	int subCiv0 = rmGetCivID(natType);
	rmSetSubCiv(0, natType);

	int ComancheVillageID = -1;
	ComancheVillageID = rmCreateGrouping("Comanche village", natGrpName+3);
	...
	ComancheVillageAID = rmCreateGrouping("Comanche village A", natGrpName + 2);
	...
	if (cNumberTeams == 2)
	{
		...
		else
		{
			...
			if (teamOneCount <= 3)
				...
				rmSetPlacementSection(0.73, 0.90);
	...
	else
	{
        ...
        if(cNumberTeams>2 || teamZeroCount != teamOneCount) {
            rmSetObjectDefMaxDistance(startingTCID, 25.0);
	...
	rmAddObjectDefItem(StartBighornSheepID, "BighornSheep", 8, 4.0);
	rmSetObjectDefCreateHerd(StartBighornSheepID, true);
	rmSetObjectDefMinDistance(StartBighornSheepID, 12);
	rmSetObjectDefMaxDistance(StartBighornSheepID, 12);
	...
	rmAddObjectDefItem(StartElkT, "Elk", 11, 6.0);
	rmSetObjectDefCreateHerd(StartElkT, true);
	rmSetObjectDefMinDistance(StartElkT, 36);
	rmSetObjectDefMaxDistance(StartElkT, 36);
	...
	rmSetObjectDefMaxDistance(startSilverID, 13.0);
	...
	rmSetObjectDefMinDistance(startSilverIDT, 30.0);
	rmSetObjectDefMaxDistance(startSilverIDT, 30.0);
	rmAddObjectDefConstraint(startSilverIDT, avoidStartResource);
	...
	{
		int silverIDT = rmCreateObjectDef("mines for team");
			...
			rmAddObjectDefConstraint(silverIDT, avoidStartResource);
			...
			rmPlaceObjectDefAtLoc(silverIDT, 0, 0.5, 0.5, PlayerNum*3);
	}

	rmSetStatusText("", 0.70);
	//   ******************************************* 1v1 hunts *****************************************X marks the spot

	if (PlayerNum <= 3)
	{
		int ElkID1 = rmCreateObjectDef("Elk herd");
		rmAddObjectDefItem(ElkID1, "Elk", rmRandInt(10, 12), 4.0);
		...
		rmAddObjectDefItem(BighornSheepID1, "BighornSheep", rmRandInt(10, 12), 4.0);
...
/*		if (rmRandFloat(0.5, 1) < 0.5)
		{
			rmPlaceObjectDefAtLoc(BighornSheepID1, 0, 0.46, 0.87, 1); // behind cliffs
			rmPlaceObjectDefAtLoc(BighornSheepID1, 0, 0.54, 0.13, 1);

			rmPlaceObjectDefAtLoc(ElkID1, 0, 0.7, 0.4, 1); // in front of tc (second hunt)
			rmPlaceObjectDefAtLoc(ElkID1, 0, 0.3, 0.6, 1);

			rmPlaceObjectDefAtLoc(BighornSheepID1, 0, 0.75, 0.75, 1); // on the traderoute
			rmPlaceObjectDefAtLoc(BighornSheepID1, 0, 0.25, 0.25, 1);

			rmPlaceObjectDefAtLoc(ElkID1, 0, 0.9, 0.48, 1); // cliff sided edge
			rmPlaceObjectDefAtLoc(ElkID1, 0, 0.1, 0.52, 1);
		}
		else
		{*/
			rmPlaceObjectDefAtLoc(BighornSheepID1, 0, 0.25, 0.85, 1); // behind tc (second hunt)
			rmPlaceObjectDefAtLoc(BighornSheepID1, 0, 0.75, 0.15, 1);

			rmPlaceObjectDefAtLoc(ElkID1, 0, 0.16, 0.56, 1); // in front of tc, near to the edge a bit
			rmPlaceObjectDefAtLoc(ElkID1, 0, 0.84, 0.44, 1);

			rmPlaceObjectDefAtLoc(BighornSheepID1, 0, 0.75, 0.67, 1); // on the traderoute
			rmPlaceObjectDefAtLoc(BighornSheepID1, 0, 0.25, 0.33, 1);

			rmPlaceObjectDefAtLoc(ElkID1, 0, 0.50, 0.85, 1); // cliff sided top and bottom
			rmPlaceObjectDefAtLoc(ElkID1, 0, 0.50, 0.15, 1);
//		}
...
//		rmSetAreaBaseHeight(YellowForest, 5.0);
		rmSetAreaCoherence(YellowForest, 0.2);
		rmAddAreaToClass(YellowForest, rmClassID("classForest"));
		rmAddAreaConstraint(YellowForest, avoidCenter);
		rmAddAreaConstraint(YellowForest, avoidStartResource);
		...
		rmAddObjectDefItem(northtreeID, "ypTreeGinkgo", 2, 4.0); // 1,2
		...
		rmAddObjectDefConstraint(northtreeID, avoidElkMin);
		rmAddObjectDefConstraint(northtreeID, avoidBighornSheepMin);
		rmPlaceObjectDefAtLoc(northtreeID, 0, 0.50, 0.50, 4);

		int northtreeID1 = rmCreateObjectDef("north tree rockies" + i);
		rmAddObjectDefItem(northtreeID1, "TreeRockies", 2, 4.0); // 1,2
		...
		rmAddObjectDefConstraint(northtreeID1, avoidElkMin);
		rmAddObjectDefConstraint(northtreeID1, avoidBighornSheepMin);
		rmPlaceObjectDefAtLoc(northtreeID1, 0, 0.50, 0.50, 5);
	}

	rmSetStatusText("", 0.80);

	//   ******************************************* team hunts *****************************************X marks the spot

	if (PlayerNum > 2)
	{

		int ElkIDT = rmCreateObjectDef("Elk herd team");
			rmAddObjectDefItem(ElkIDT, "Elk", rmRandInt(10, 12), 4.0);
			...
			rmPlaceObjectDefAtLoc(ElkIDT, 0, 0.5, 0.5, PlayerNum*2);

		int BighornSheepIDT = rmCreateObjectDef("BighornSheep herd team");
			rmAddObjectDefItem(BighornSheepIDT, "BighornSheep", rmRandInt(10, 12), 4.0);
			...
			rmPlaceObjectDefAtLoc(BighornSheepIDT, 0, 0.5, 0.5, PlayerNum*2);
	}
	...
	int nuggetID3 = rmCreateObjectDef("nugget3");
	rmAddObjectDefItem(nuggetID3, "Nugget", 1, 0.0);
	rmSetObjectDefMinDistance(nuggetID3, 0.0);
	rmSetObjectDefMaxDistance(nuggetID3, rmXFractionToMeters(0.35));
	...
	rmAddObjectDefConstraint(nuggetID3, avoidTownCenterFar2);
	rmAddObjectDefConstraint(nuggetID3, avoidCliffshort);
	rmAddObjectDefConstraint(nuggetID3, stayMiddleEdge2);
	...
	rmPlaceObjectDefAtLoc(nuggetID1, 0, 0.5, 0.5, 2+PlayerNum*4);
