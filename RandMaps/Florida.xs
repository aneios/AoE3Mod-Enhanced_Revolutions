...
if (rmGetNomadStart() == false) {
	if (TeamNum == 2 && teamZeroCount_dk == teamOneCount_dk) {	
...
//	rmAddObjectDefToClass(playergold2ID, classStartingResource);
	...
	rmAddObjectDefItem(player2ndherdID, "Deer", rmRandInt(10,10), 4.0);
...
*/		
	...
	if (TeamNum == 2 && teamZeroCount_dk == teamOneCount_dk) {	
		if(rmGetNomadStart() == false && ypIsAsian(i)) {
...
//		rmAddAreaConstraint(swampforestID, avoidImpassableLandFar);
	...
	// ************ Herds *************
	int swap = rmRandInt(1,2);

	// some deer
	int staticDeerID = rmCreateObjectDef("static herd");
	rmAddObjectDefItem(staticDeerID, "Deer", rmRandInt(10,10), 5.0);
	rmSetObjectDefMinDistance(staticDeerID, rmXFractionToMeters(0.0));
	rmSetObjectDefMaxDistance(staticDeerID, rmXFractionToMeters(0.03));
	rmSetObjectDefCreateHerd(staticDeerID, true);
	rmAddObjectDefConstraint(staticDeerID, avoidSeaEast);
	rmAddObjectDefConstraint(staticDeerID, avoidNatives);
	rmAddObjectDefConstraint(staticDeerID, avoidTradeRouteSocketShort);
	rmAddObjectDefConstraint(staticDeerID, avoidForestMin);
	rmAddObjectDefConstraint(staticDeerID, avoidGoldTypeShort);
	rmAddObjectDefConstraint(staticDeerID, avoidStartingResourcesShort);
	rmAddObjectDefConstraint(staticDeerID, avoidEdge);
	if (PlayerNum == 2)
	{
		if (swap == 1)
		{
			rmPlaceObjectDefAtLoc(staticDeerID, 0, 0.50, 0.40);
			rmPlaceObjectDefAtLoc(staticDeerID, 0, 0.30, 0.60);
			rmPlaceObjectDefAtLoc(staticDeerID, 0, 0.70, 0.60);
			rmPlaceObjectDefAtLoc(staticDeerID, 0, 0.80, 0.33);
			rmPlaceObjectDefAtLoc(staticDeerID, 0, 0.20, 0.33);
		}
		else
		{
			rmPlaceObjectDefAtLoc(staticDeerID, 0, 0.50, 0.60);
			rmPlaceObjectDefAtLoc(staticDeerID, 0, 0.35, 0.35);
			rmPlaceObjectDefAtLoc(staticDeerID, 0, 0.65, 0.35);
			rmPlaceObjectDefAtLoc(staticDeerID, 0, 0.20, 0.65);
			rmPlaceObjectDefAtLoc(staticDeerID, 0, 0.80, 0.65);
		}
	}
	...
	for (i=0; < 2)
	{
		int turkey1ID = rmCreateObjectDef("first turkeys"+i);
		rmAddObjectDefItem(turkey1ID, "Turkey", rmRandInt(6,6), 5.0);
		rmSetObjectDefMinDistance(turkey1ID, 0.0);
		rmSetObjectDefMaxDistance(turkey1ID, rmXFractionToMeters(0.03));
		rmSetObjectDefCreateHerd(turkey1ID, true);
//		rmAddObjectDefConstraint(turkey1ID, avoidImpassableLandShort);
		rmAddObjectDefConstraint(turkey1ID, avoidSeaEast);
		rmAddObjectDefConstraint(turkey1ID, avoidNatives);
		rmAddObjectDefConstraint(turkey1ID, avoidTradeRouteSocketShort);
		rmAddObjectDefConstraint(turkey1ID, avoidForestMin);
		rmAddObjectDefConstraint(turkey1ID, avoidGoldTypeShort);
		rmAddObjectDefConstraint(turkey1ID, avoidStartingResourcesShort);
		rmAddObjectDefConstraint(turkey1ID, avoidEdge);
		if (rmGetIsKOTH() == true)
		{
			rmAddObjectDefConstraint(turkey1ID, avoidKOTH);
			rmAddObjectDefConstraint(turkey1ID, avoidKOTHLand);
		}
		if (PlayerNum == 2)
		{
			if (i <= 0)
			{
				if (swap == 1)
					rmPlaceObjectDefAtLoc(turkey1ID, 0, 0.05, 0.50);
				else
					rmPlaceObjectDefAtLoc(turkey1ID, 0, 0.10, 0.40);
			}
			else if (i <= 1)
			{
				if (swap == 1)
					rmPlaceObjectDefAtLoc(turkey1ID, 0, 0.90, 0.50);
				else
					rmPlaceObjectDefAtLoc(turkey1ID, 0, 0.90, 0.40);
			}
		}
	}
...
//		rmAddObjectDefConstraint(DeerherdID, avoidImpassableLandShort);
		...
		rmAddObjectDefConstraint(DeerherdID, avoidTurkeyFar);
