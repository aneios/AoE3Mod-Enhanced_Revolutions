...
void main(void)
{
   ...
   int IslandLoc = 1;								
	...
	rmSetAreaSize(bigIslandID, 0.36);				//Defines island's size.  .40, .40 looks about right.
	rmSetAreaCoherence(bigIslandID, 0.80);				//Determines raggedness of island's coastline.  Lower the number, more the blobby.
...
//	rmAddAreaTerrainLayer(bigIslandID, "caribbean\ground_shoreline3_crb", 15, 20);
	...
	rmAddAreaInfluenceSegment(bigIslandID, 0.20, 0.43, 0.34, 0.21);  //Segment 4 - short lower bit // last was .20, .44, .36, .21
	...
	int teamOneCount = rmGetNumberPlayersOnTeam(1);
		...
		if (cNumberTeams == 2 && teamZeroCount == teamOneCount)
		{
			...
			rmPlacePlayersLine(0.3, 0.58, 0.29, 0.29, 0.0, 0.0); //Team 0 is in the south 
			
			rmSetPlacementTeam(1);
			rmPlacePlayersLine(0.65, 0.62, 0.715, 0.31, 0.0, 0.0); //Team 1 is in the north
			}
			else
			{
			rmSetPlacementTeam(1);
			rmPlacePlayersLine(0.3, 0.58, 0.29, 0.29, 0.0, 0.0); //Team 1 is in the south 
			
			rmSetPlacementTeam(0);
			rmPlacePlayersLine(0.65, 0.62, 0.715, 0.31, 0.0, 0.0); //Team 0 is in the north
			}
		}
	...
	if (whichVariation == 2 || whichVariation == 3) 
	{
		...
		{  
			...
			rmAddGroupingToClass(caribs6VillageID, classNatives);			
   ...
   // Really big mountain for 8 players, big mountain for 6 or 7 players, and small mountain for 5 or less players.
		...
		{
			rmSetAreaSize(smallMesaID, rmAreaTilesToFraction(777));  //First # is minimum square meters of material it will use to build.  Second # is maximum.  Currently I have them both set to the same because I want a certain size mountain every time.
		}
		else if ( cNumberNonGaiaPlayers < 8 )
		{
			rmSetAreaSize(smallMesaID, rmAreaTilesToFraction(1111));  //First # is minimum square meters of material it will use to build.  Second # is maximum.  Currently I have them both set to the same because I want a certain size mountain every time.
		}
		else
		{
			rmSetAreaSize(smallMesaID, rmAreaTilesToFraction(1444));  //First # is minimum square meters of material it will use to build.  Second # is maximum.  Currently I have them both set to the same because I want a certain size mountain every time.
		}
	...
	rmSetObjectDefMaxDistance(TCID, 0.0);  // Originally 10.0 -- JSB -- Allows TC placement spot to float a bit along the lines I tell them to place. 
...
//	rmAddObjectDefConstraint(TCID, avoidCW);
 ...
 if(ypIsAsian(i) && rmGetNomadStart() == false)	
  ...
  if(rmGetIsKOTH()) {	
	...
	rmPlaceObjectDefAtLoc(whaleID, 0, 0.46, 0.60, cNumberNonGaiaPlayers*2 + rmRandInt(3,4));  //Was .43, .67 // .37, .66 -- The whales will be placed from this spot. 1 per player, plus 1 or 2 more.
