...
//	rmSetMapElevationHeightBlend(1);
	...
	rmSetMapType("grass");
	rmSetMapType("water");
	rmSetLightingSet("Indonesia_Skirmish");
	...
	// Resource avoidance
	int avoidForest = rmCreateClassDistanceConstraint("avoid forest", rmClassID("Forest"), 40.0); //15.0
	...
	int avoidSerowFar = rmCreateTypeDistanceConstraint("avoid Serow far", "ypSerow", 72.0);
	...
	int avoidEleFar = rmCreateTypeDistanceConstraint("avoid Ele far", "ypWildElephant", 55.0);
	int avoidEle = rmCreateTypeDistanceConstraint("avoid Ele", "ypWildElephant", 50.0);
	...
	int avoidGold = rmCreateClassDistanceConstraint ("gold avoid gold med", rmClassID("Gold"), 65.0-1*PlayerNum);
	...
	int avoidNuggetWater = rmCreateTypeDistanceConstraint("avoid nugget water", "AbstractNugget", 125.0);
...
//	int avoidTownCenterMed = rmCreateTypeDistanceConstraint("avoid Town Center med", "townCenter", 40.0);
	...
	int avoidWhale=rmCreateTypeDistanceConstraint("avoid whale", "fish", 90.0);
	int avoidFish=rmCreateTypeDistanceConstraint("avoid fish", "fish", 30.0);
	int avoidBuffalo=rmCreateTypeDistanceConstraint("avoid buffalo", "ypWaterBuffalo", 85.0);
...
//	rmAddObjectDefConstraint(player2ndherdID, avoidImpassableLand);
	...
	rmAddObjectDefConstraint(playerLombardID, avoidStartingResources);
	...
	rmAddObjectDefConstraint(playerCommunityPlazaID, avoidStartingResources);
	...
	rmAddObjectDefConstraint(playerUSSaloonID, avoidStartingResources);
	...
	rmAddObjectDefItem(playerAfricanTowerID, "deTower", 1, 0.0);
	rmSetObjectDefMinDistance(playerAfricanTowerID, 12.0);
	rmSetObjectDefMaxDistance(playerAfricanTowerID, 16.0);
	rmAddObjectDefConstraint(playerAfricanTowerID, avoidStartingResources);
	rmAddObjectDefConstraint(playerAfricanTowerID, avoidImpassableLand);
...
//			rmPlaceObjectDefAtLoc(playerNuggetID, i, rmXMetersToFraction(xsVectorGetX(TCLoc)), rmZMetersToFraction(xsVectorGetZ(TCLoc)));		
	...
	int coppercount = 2+1.5*PlayerNum;
...
//		rmAddObjectDefConstraint(commoncopperID, avoidTradeRouteSocket);
	...
	// Inland forest
	int forestinlandcount = 4+3*PlayerNum; //12
...
//		rmAddAreaConstraint(forestcentralID, avoidCliff);
		...
		for (j=0; < rmRandInt(16,18))
		{
			...
			rmAddObjectDefItem(forestcentraltreeID, "ypTreeBorneo", rmRandInt(1,1), 3.0);
			rmAddObjectDefItem(forestcentraltreeID, "ypTreeBamboo", rmRandInt(1,1), 3.0);
...
//		rmAddObjectDefConstraint(Nugget4ID, avoidTradeRouteSocket);
		...
		if (PlayerNum >= 3 && rmGetIsTreaty() == false)
