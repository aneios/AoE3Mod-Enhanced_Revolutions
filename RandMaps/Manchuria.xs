...
//	rmSetMapElevationHeightBlend(1);
	...
	int avoidSerowFar = rmCreateTypeDistanceConstraint("avoid Serow far", "ypSerow", 55-PlayerNum);
...
//	rmAddAreaTerrainLayer(SeaID, "coastal_japan\ground_grass1_co_japan", 0, 2);
	...
	rmAddAreaInfluenceSegment(midLineID, 0.50, 1.00, 0.50, 0.00);
//	rmSetAreaMix(midLineID, "testmix"); 	// for testing
...
*/
	...
	rmAddObjectDefItem(playerTreeID, "ypTreeMongolianFir", 7, 5.0);
	...
	rmAddObjectDefItem(playerTree2ID, "ypTreeMongolianFir", 12, 6.0);
	rmAddObjectDefItem(playerTree2ID, "ypTreeSaxaul", 3, 6.0);
	...
	rmAddObjectDefConstraint(playerTree2ID, avoidMidIsland);
...
//		rmPlaceObjectDefAtLoc(playergold3ID, i, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));
		...
		rmPlaceObjectDefAtLoc(player2ndherdID, i, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));
//		rmPlaceObjectDefAtLoc(player3rdherdID, i, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));
...
*/	
	...
	for (i=0; < forestlowcount)
	{
		...
		rmSetAreaSize(forestlowID, rmAreaTilesToFraction(77));
...
//		rmAddAreaConstraint(forestlowID, avoidEdge);
		...
		//	rmAddObjectDefConstraint(forestlowtreeID, avoidForestShort);
			...
			rmPlaceObjectDefAtLoc(forestlowtreeID, 0, 0.50, 0.50, 6);
		}
	...
	for (i=0; < foresthighcount)
	{
		...
		rmSetAreaSize(foresthighID, rmAreaTilesToFraction(77));
...
//		rmAddAreaConstraint(foresthighID, avoidEdge);
		...
		//	rmAddObjectDefConstraint(foresthightreeID, avoidForestShort);
			...
			rmPlaceObjectDefAtLoc(foresthightreeID, 0, 0.50, 0.50, 7);
		}
	...
	int lowgroundherdcount = 3.5*PlayerNum-2;
	int highgroundherdcount = 1+PlayerNum;
	
		
	//Lowground herds
	int herdID = rmCreateObjectDef("herd");
	rmAddObjectDefItem(herdID, "ypSerow", 7, 3.0);
	rmSetObjectDefMinDistance(herdID, 0.00);
	rmSetObjectDefMaxDistance(herdID, rmXFractionToMeters(0.03));
	rmSetObjectDefCreateHerd(herdID, true);
	rmAddObjectDefConstraint(herdID, avoidForestMin);
	rmAddObjectDefConstraint(herdID, avoidGoldMin);
	rmAddObjectDefConstraint(herdID, avoidStartingResources); 
	rmAddObjectDefConstraint(herdID, avoidImpassableLand);
	rmPlaceObjectDefAtLoc(herdID, 0, 0.65, 0.45);	
	rmPlaceObjectDefAtLoc(herdID, 0, 0.35, 0.45);	
	...
	for (i=0; < lowgroundherdcount)
	{
		...
		rmSetObjectDefMinDistance(lowgroundherdID, 0.10);
		rmSetObjectDefMaxDistance(lowgroundherdID, rmXFractionToMeters(0.45));
		...
		rmAddObjectDefConstraint(lowgroundherdID, avoidMidLineFar);
		if (TeamNum <= 2)
	...
	//Highground herds
		...
		rmAddObjectDefConstraint(highgroundherdID, avoidMidLineFar);
