// LARGE MANCHURIA
// designed by Garja
//visual update for DE by Durokan & Interjection
...
//	rmSetMapElevationHeightBlend(1);
	...
	// Resource avoidance
	int avoidForest=rmCreateClassDistanceConstraint("avoid forest", rmClassID("Forest"), 55.0); //15.0
	...
	int avoidSerowFar = rmCreateTypeDistanceConstraint("avoid Serow far", "ypSerow", 66.0);
	int avoidSerow = rmCreateTypeDistanceConstraint("avoid Serow", "ypSerow", 52.0);
	...
	int avoidSheepFar = rmCreateTypeDistanceConstraint("avoid Sheep far", "ypMarcoPoloSheep", 70.0);
	int avoidSheep = rmCreateTypeDistanceConstraint("avoid  Sheep", "ypMarcoPoloSheep", 55.0);
	...
	int avoidMuskdeer = rmCreateTypeDistanceConstraint("avoid muskdeer ", "ypmuskdeer", 55.0);
	...
	int avoidGoldFar = rmCreateClassDistanceConstraint ("gold avoid gold far", rmClassID("Gold"), 82.0);
	...
	int avoidNuggetFar = rmCreateTypeDistanceConstraint("nugget avoid nugget Far", "AbstractNugget", 80.0);
	int avoidTownCenterVeryFar = rmCreateTypeDistanceConstraint("avoid Town Center Very Far", "townCenter", 85.0+2*PlayerNum);
	int avoidTownCenterFar = rmCreateTypeDistanceConstraint("avoid Town Center Far", "townCenter", 70.0+2*PlayerNum);
	...
	int avoidWhale=rmCreateTypeDistanceConstraint("avoid whale", "fish", 48.0+1*PlayerNum);
	int avoidFish=rmCreateTypeDistanceConstraint("avoid fish", "fish", 16.0);
	int avoidYak=rmCreateTypeDistanceConstraint("avoid yak", "ypyak", 82.0);
	...
	int avoidLandFar = rmCreateTerrainDistanceConstraint("avoid land far ", "Land", true, 20.0);
	...
	int avoidPatch2 = rmCreateClassDistanceConstraint("avoid patch2", rmClassID("patch2"), 13.0);
		...
		if (cNumberTeams <= 2) // 1v1 and TEAM
		{
			...
			else if (teamZeroCount == teamOneCount) // equal N of players per TEAM
			{
				if (teamZeroCount == 2) // 2v2
				{
					...
					rmPlacePlayersLine(0.16, 0.48, 0.21, 0.66, 0.00, 0.20);

					rmSetPlacementTeam(1);
					rmPlacePlayersLine(0.84, 0.48, 0.79, 0.66, 0.00, 0.20);
				}
				else // 3v3, 4v4
				{
					rmSetPlacementTeam(0);
						rmSetTeamSpacingModifier(0.50);
						rmSetPlacementSection(0.700, 0.870);
						rmPlacePlayersCircular(0.32, 0.32, 0.0);
			//		rmPlacePlayersLine(0.13, 0.38, 0.25, 0.72, 0.00, 0.20);

					rmSetPlacementTeam(1);
						rmSetTeamSpacingModifier(0.50);
						rmSetPlacementSection(0.130, 0.300);
						rmPlacePlayersCircular(0.32, 0.32, 0.0);					
			...
			else // unequal N of players per TEAM
			{
				if (teamZeroCount == 1 || teamOneCount == 1) // one team is one player
				{
					...
					{
						rmSetPlacementTeam(0);
						rmPlacePlayersLine(0.16, 0.48, 0.19, 0.48, 0.00, 0.20);
						...
						if (teamOneCount == 2)
							...
							rmSetTeamSpacingModifier(0.50);
							rmSetPlacementSection(0.110, 0.340);
							rmPlacePlayersCircular(0.36, 0.36, 0.0);	
					}
					else // 2v1, 3v1, 4v1, etc.
					{
						rmSetPlacementTeam(1);
						rmPlacePlayersLine(0.84, 0.48, 0.81, 0.48, 0.00, 0.20);

						rmSetPlacementTeam(0);
						if (teamZeroCount == 2)
							rmPlacePlayersLine(00.16, 0.48, 0.21, 0.66, 0.00, 0.20);
						else
							rmSetTeamSpacingModifier(0.50);
							rmSetPlacementSection(0.640, 0.890);
							rmPlacePlayersCircular(0.36, 0.36, 0.0);
					}
				}
				else if (teamZeroCount == 2 || teamOneCount == 2) // one team has 2 players
				{
					if (teamZeroCount < teamOneCount) // 2v3, 2v4, etc.
					{
						...
						rmSetPlacementTeam(1);
							rmSetTeamSpacingModifier(0.50);
							rmSetPlacementSection(0.110, 0.325);
							rmPlacePlayersCircular(0.36, 0.36, 0.0);	
					}
					else // 3v2, 4v2, etc.
					{
						rmSetPlacementTeam(0);
							rmSetTeamSpacingModifier(0.50);
							rmSetPlacementSection(0.675, 0.890);
							rmPlacePlayersCircular(0.36, 0.36, 0.0);
...
/*	// old player placement - modified by vividlyplain 
	int teamZeroCount = rmGetNumberPlayersOnTeam(0);
	int teamOneCount = rmGetNumberPlayersOnTeam(1);
...
*/	
    ...
    rmSetAreaSize(highgroundID, 0.22, 0.28);
	rmSetAreaLocation(highgroundID, 0.5, 0.84);
	rmAddAreaInfluenceSegment(highgroundID, 1.00, 0.92, 0.00, 0.92);
	rmSetAreaMix(highgroundID, "mongolia_grass");
//	rmSetAreaMix(highgroundID, "rockies_snow");		// for testing
    rmSetAreaCoherence(highgroundID, 0.66);
	rmSetAreaSmoothDistance(highgroundID, 5);
    rmBuildArea(highgroundID); 
	
	int stayNearHighground = rmCreateAreaMaxDistanceConstraint("stay near high ground", highgroundID, 30);
	...
	int avoidHighgroundVeryFar = rmCreateAreaDistanceConstraint("avoid high ground very far", highgroundID, 30);
	int avoidHighgroundExtreme = rmCreateAreaDistanceConstraint("avoid high ground extreme", highgroundID, 66);

	//Mid ground 
	int midGroundID = rmCreateArea("mid ground");
    rmSetAreaWarnFailure(midGroundID, false);
	rmSetAreaObeyWorldCircleConstraint(midGroundID, false);
    rmSetAreaSize(midGroundID, 0.6, 0.6);
	rmSetAreaLocation(midGroundID, 0.5, 0.40);
	rmSetAreaMix(midGroundID, "mongolia_grass_b");
    rmSetAreaCoherence(midGroundID, 0.75);
	rmSetAreaSmoothDistance(midGroundID, 8);
	rmAddAreaConstraint(midGroundID, avoidHighground);
    rmBuildArea(midGroundID); 
	
	int avoidMidGround = rmCreateAreaDistanceConstraint("avoid mid ground", midGroundID, 0.5);
	int avoidMidGroundFar = rmCreateAreaDistanceConstraint("avoid mid ground far", midGroundID, 5.0);	
	...
	rmSetAreaWaterType(SeaID, "Manchuria Coast"); 
//	rmAddAreaTerrainLayer(SeaID, "coastal_japan\ground_grass1_co_japan", 0, 2);
    ...
    rmSetAreaSize(lowgroundID, 0.45, 0.55);
	rmSetAreaLocation(lowgroundID, 0.5, 0.4);
	rmSetAreaMix(lowgroundID, "mongolia_grass_a");
    rmSetAreaCoherence(lowgroundID, 0.75);
	rmSetAreaSmoothDistance(lowgroundID, 8);
	rmAddAreaConstraint(lowgroundID, avoidHighgroundVeryFar);
	rmAddAreaConstraint(lowgroundID, avoidSea);
	...
	rmSetAreaMix(spurID, "mongolia_grass_a");
	...
	if (PlayerNum > 5)
		rmSetAreaSize(midIslandID, 0.32);
	else
...
//	rmSetAreaBaseHeight(midIslandID, 1.5);
...
//	rmSetAreaBaseHeight(midSmIslandID, 1.5);
	...
	rmSetAreaSize(SWareaID, 0.25, 0.25);
	rmSetAreaLocation(SWareaID, 0.25, 0.15);
	...
	rmSetAreaSize(NEareaID, 0.25, 0.25);
	rmSetAreaLocation(NEareaID, .75, 0.15);
...
//	rmSetAreaTerrainType(NEareaID, "new_england\ground2_cliff_ne"); // for testing
	...
	if (i == 0)
	{
		...
		rmSetAreaMix(hillID, "mongolia_desert");	
//		rmAddAreaTerrainLayer(hillID, "mongolia\ground_grass1_co_japan", 0, 2);
	...
	else if (i == 1)
	{
		...
		rmSetAreaCliffType(hillID, "Manchuria"); 
...
//	rmSetAreaTerrainType(playerareaID, "new_england\ground2_cliff_ne"); // for testing
	...
	if (rmGetIsKOTH() == true) {
		...
		if (randLoc == 2) {
			...
			xLoc = 0.5;
			yLoc = 0.1;
			ypKingsHillLandfill(xLoc, yLoc, 0.007, 1.0, "coastal_japan_a", 0);
		}
...
//	TPvariation = 0.2; // <--- TEST
	...
	if (TPvariation > 0.5 && TPvariation <= 1.0) // 1 route
	{
		...
		socketLoc = rmGetTradeRouteWayPoint(tradeRouteID, 0.40-0.005*PlayerNum);
		...
		rmPlaceObjectDefAtPoint(socketID, 0, socketLoc);
		socketLoc = rmGetTradeRouteWayPoint(tradeRouteID, 0.60+0.005*PlayerNum);
	...
	else if (TPvariation <= 0.5) // 2 routes
	{
		...
		rmAddTradeRouteWaypoint(tradeRoute2ID, 0.70, 0.92); 
		...
		if (PlayerNum < 6)
		{
			socketLoc2 = rmGetTradeRouteWayPoint(tradeRoute2ID, 0.72);
			rmPlaceObjectDefAtPoint(socket2ID, 0, socketLoc2);
		}
		else
		{
...
//	Natsvariation = 0.2; // <--- TEST
	...
	if (Natsplacement > 0.3)
	{
		...
		{
			nativeID1 = rmCreateGrouping("Zen temple A", "native tengri village 0"+rmRandInt(1,5));
		...
		//	rmAddGroupingConstraint(nativeID1, avoidNatives);
			...
			nativeID3 = rmCreateGrouping("Zen temple B", "native tengri village 0"+rmRandInt(1,5));
	...
	//	rmAddGroupingConstraint(nativeID4, avoidNatives);
		...
		nativeID5 = rmCreateGrouping("Zen temple C", "native tengri village 0"+rmRandInt(1,5));
...
//	minevariation = 5; // <--- TEST	
	...
	rmSetObjectDefMaxDistance(playergoldID, 13.0);
...
*/
	...
	rmAddObjectDefItem(playerherdID, "ypMarcoPoloSheep", 5, 3.0);
	...
	rmAddObjectDefItem(player2ndherdID, "ypSerow", 11, 7.0);
...
//	rmAddObjectDefConstraint(player2ndherdID, avoidTradeRouteShort);
	...
	rmAddObjectDefConstraint(player2ndherdID, avoidMidIsland);
...
*/	
	...
	for(i=1; < numPlayer)
	{
		...
		rmSetObjectDefMaxDistance(colonyShipID, rmXFractionToMeters(0.10));
		...
		rmAddObjectDefConstraint(colonyShipID, avoidEdge);
      	rmPlaceObjectDefAtLoc(colonyShipID, i, rmPlayerLocXFraction(i), 0.10);
	}   
...
*/	
	...
	// Lowground forest
	int forestlowcount = 4+6*PlayerNum;
	if (PlayerNum >= 4)
		forestlowcount = 3+5*PlayerNum;
	if (PlayerNum >= 6)
		forestlowcount = 2+4*PlayerNum;
	...
	for (i=0; < forestlowcount)
	{
		...
		rmSetAreaSize(forestlowID, rmAreaTilesToFraction(111));
		rmSetAreaMix(forestlowID, "mongolia_forest");
		rmSetAreaObeyWorldCircleConstraint(forestlowID, true);
		...
		rmAddAreaConstraint(forestlowID, avoidStartingResources); 
		...
		rmAddAreaConstraint(forestlowID, avoidKOTH);
		if (TeamNum <= 2)
...
//		rmAddAreaConstraint(forestlowID, avoidEdge);
		...
		stayInForestLowPatch = rmCreateAreaMaxDistanceConstraint("stay in forest low patch"+i, forestlowID, 0.0);
			...
			int forestlowtreeID = rmCreateObjectDef("forest lowground trees"+i);
			rmAddObjectDefItem(forestlowtreeID, "ypTreeMongolianFir", 3, 3.0);
		...
		//	rmAddObjectDefConstraint(forestlowtreeID, avoidForestShort);
			...
			rmPlaceObjectDefAtLoc(forestlowtreeID, 0, 0.50, 0.50, 9);
		}

	// Highground forest
	int foresthighcount = 2+2*PlayerNum;
	...
	for (i=0; < foresthighcount)
	{
		...
		rmSetAreaSize(foresthighID, rmAreaTilesToFraction(111));
		rmSetAreaMix(foresthighID, "mongolia_forest");
		rmSetAreaObeyWorldCircleConstraint(foresthighID, true);
		...
		rmAddAreaConstraint(foresthighID, avoidStartingResources); 
		...
		rmAddAreaConstraint(foresthighID, avoidKOTH);
		if (TeamNum <= 2)
...
//		rmAddAreaConstraint(foresthighID, avoidEdge);
		...
		stayInForestHighPatch = rmCreateAreaMaxDistanceConstraint("stay in forest high patch"+i, foresthighID, 0.0);
			...
			int foresthightreeID = rmCreateObjectDef("forest highground trees"+i);
			rmAddObjectDefItem(foresthightreeID, "ypTreeMongolia", 2, 3.0);
		...
		//	rmAddObjectDefConstraint(foresthightreeID, avoidForestShort);
			...
			rmPlaceObjectDefAtLoc(foresthightreeID, 0, 0.50, 0.50, 8);
		}
	
	
		// Random trees in the lowground
	int treelowcount = 5*PlayerNum;

		int randomlowtreeID = rmCreateObjectDef("random trees low ground ");
		...
		rmAddObjectDefConstraint(randomlowtreeID, avoidKOTH);
		if (TeamNum <= 2)
			rmAddObjectDefConstraint(randomlowtreeID, avoidHill);
		rmPlaceObjectDefAtLoc(randomlowtreeID, 0, 0.50, 0.50, treelowcount);
	
	// Random trees in the highground
		int treehighcount = 2+2*PlayerNum;
	
		int randomhightreeID = rmCreateObjectDef("random trees highground ");
		...
		rmPlaceObjectDefAtLoc(randomhightreeID, 0, 0.50, 0.50, treehighcount);
	...
	int lowgroundherdcount = 4*PlayerNum;
	int highgroundherdcount = 2+PlayerNum;
	
		
	//Lowground herds
		int lowgroundherdID = rmCreateObjectDef("lowground herd");
		...
		rmAddObjectDefConstraint(lowgroundherdID, avoidStartingResources); 
...
//		rmAddObjectDefConstraint(lowgroundherdID, avoidTradeRouteShort);
		...
		rmAddObjectDefConstraint(lowgroundherdID, avoidKOTH);
		if (TeamNum <= 2)
		...
		rmPlaceObjectDefAtLoc(lowgroundherdID, 0, 0.50, 0.50, lowgroundherdcount);	
	
	//Highground herds
		int highgroundherdID = rmCreateObjectDef("highground herd");
		rmAddObjectDefItem(highgroundherdID, "ypMuskdeer", rmRandInt(8,8), 7.0);
		rmSetObjectDefMinDistance(highgroundherdID, 0.06);
		rmSetObjectDefMaxDistance(highgroundherdID, rmXFractionToMeters(0.45));
		...
		rmAddObjectDefConstraint(highgroundherdID, avoidStartingResources); 
//		rmAddObjectDefConstraint(highgroundherdID, avoidTradeRouteShort);
		...
		rmAddObjectDefConstraint(highgroundherdID, avoidKOTH);
		if (TeamNum <= 2)
...
//			rmAddObjectDefConstraint(highgroundherdID, stayNE);
		rmPlaceObjectDefAtLoc(highgroundherdID, 0, 0.50, 0.50, highgroundherdcount);	
	...
	int treasure3count = 1+PlayerNum;
	...
	// Treasures lvl4
		...
		if (PlayerNum >= 4)
			rmPlaceObjectDefAtLoc(Nugget4ID, 0, 0.50, 0.50, treasure4count);
	...
	for (i=0; < yaksCount)
	{
		...
		rmAddObjectDefConstraint(yakID, avoidStartingResources);
...
//			rmAddObjectDefConstraint(yakID, avoidHighground); // avoidHighground
	...
	rmPlaceObjectDefAtLoc(whaleID, 0, 0.50, 0.10, whalecount);
	...
	//Fish
		...
		rmPlaceObjectDefAtLoc(fishID, 0, 0.50, 0.15, fishcount);
