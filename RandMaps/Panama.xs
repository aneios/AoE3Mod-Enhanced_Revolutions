...
//	rmSetOceanReveal(true);
	...
	// Resource avoidance
	int avoidForest=rmCreateClassDistanceConstraint("avoid forest", rmClassID("Forest"), 20.0); //15.0
	...
	int avoidHunt1Far = rmCreateTypeDistanceConstraint("avoid hunt 1 far", food1, 45.0);
	int avoidHunt1 = rmCreateTypeDistanceConstraint("avoid hunt 1", food1, 30.0);
	int avoidHunt1Short = rmCreateTypeDistanceConstraint("avoid hunt 1 short", food1, 20.0);
	int avoidHunt1Min = rmCreateTypeDistanceConstraint("avoid hunt 1 min", food1, 10.0);
	int avoidHunt2Far = rmCreateTypeDistanceConstraint("avoid hunt 2 far", food2, 54.0);
	int avoidHunt2 = rmCreateTypeDistanceConstraint("avoid hunt 2 ", food2, 30.0);
	int avoidHunt2Short = rmCreateTypeDistanceConstraint("avoid hunt 2 short ", food2, 24.0);
	...
	int avoidGoldMed = rmCreateClassDistanceConstraint ("gold avoid gold med", rmClassID("Gold"), 40.0);
	...
	int avoidWhale=rmCreateTypeDistanceConstraint("avoid whale", "MinkeWhale", 36+2*PlayerNum);
...
//		rmAddObjectDefConstraint(goldID, stayMidSmIsland);
	...
	for (i=0; < 32) {
		...
		rmPlaceObjectDefAtLoc(rimTreeID, 0, 0.50, 0.50, 60+15*PlayerNum);
	...
	int Nugget3SID = rmCreateObjectDef("nugget L3 S"); 
		...
		if (PlayerNum > 2)
			rmPlaceObjectDefAtLoc(Nugget3SID, 0, 0.60, 0.60, treasure3count);
	...
	int Nugget2ID = rmCreateObjectDef("nugget L2 "); 
		...
		rmPlaceObjectDefAtLoc(Nugget2ID, 0, 0.60, 0.60, treasure2count);
	...
	int Nugget1ID = rmCreateObjectDef("nugget L1 "); 
		...
		rmPlaceObjectDefAtLoc(Nugget1ID, 0, 0.60, 0.60, treasure1count);
	...
	int fishID = rmCreateObjectDef("fish");
		...
		rmPlaceObjectDefAtLoc(fishID, 0, 0.50, 0.50, 2*fishcount);
	...
	int fish2ID = rmCreateObjectDef("fish2");
		...
		rmPlaceObjectDefAtLoc(fish2ID, 0, 0.50, 0.50, 2*fishcount);
