// LARGE ANDES
// Dec 06 - YP update
//split Andes into Andes with river and Andes upper without river for DE -- Durokan
// LARGE version by vividlyplain, July 2021
// updated natives, tcfloat, and rivers September 2021, vividlyplain
// updated November 2021 - town centers stay on correct side of river now and playears have a little more room
...
void main(void)
{
   ...
   int subCiv2=-1;
	...
	if (whichNative == 3) {
		...
		sktType1 = "socketmapuche";
		sktType2 = "socketZapotec";
		}

	// All Mapuche all the time
	// adding in some variety
   ...
   // Nature avoidance
	int southForestConstraint=rmCreateClassDistanceConstraint("forest avoids forest", rmClassID("classForest"), 40.0);
	int avoidLlama=rmCreateTypeDistanceConstraint("llama avoids llama", "Llama", 80.0);
	int avoidDeer=rmCreateTypeDistanceConstraint("deer avoids deer", "Deer", 75.0);
	int avoidGuanaco=rmCreateTypeDistanceConstraint("guanaco avoids guanaco", "Guanaco", 64.0);
	int avoidCoin=rmCreateTypeDistanceConstraint("avoid coin", "gold", 40.0);
	int avoidCoinFar=rmCreateTypeDistanceConstraint("avoid coin far", "gold", 75.0);
   ...
   // Unit avoidance
	...
	int avoidNat1SocketsTC=rmCreateTypeDistanceConstraint("avoid nat1 Socket from TC", sktType1, 85);
	int avoidNat2SocketsTC=rmCreateTypeDistanceConstraint("avoid nat2 Socket from TC", sktType2, 85);
...
*/	
   ...
   rmBuildAllAreas();
	...
	{ 
		...
		// Rivers on both sides 70% (not true for DE with the andes split into andes and upper
		float RiverType = rmRandFloat(0,1);
		//if(RiverType < 0.5)
        if(true)
		{
			//int riverID = rmRiverCreate(-1, "Andes River", 12, 8, 3, 3);
			int riverID = rmRiverCreate(-1, "Andes River", 1, 1, 8+cNumberNonGaiaPlayers/2, 8+cNumberNonGaiaPlayers/2);
			...
			rmRiverSetShallowRadius(riverID, 10+cNumberNonGaiaPlayers);
			if (cNumberTeams > 2) {
			...
			int river2ID = rmRiverCreate(-1, "Andes River", 1, 1, 8+cNumberNonGaiaPlayers/2, 8+cNumberNonGaiaPlayers/2);
			rmRiverAddWaypoint(river2ID, 0.9, 0.0);
			rmRiverAddWaypoint(river2ID, 0.6, 0.3);
			rmRiverAddWaypoint(river2ID, 0.35, 0.35);
			rmRiverAddWaypoint(river2ID, 0.0, 0.25);
			rmRiverSetShallowRadius(river2ID, 10+cNumberNonGaiaPlayers);
			if (cNumberTeams > 2) {
...
//	rmSetAreaMix(midIslandID, "testmix"); 	// for testing
	...
	if (subCiv0 == rmGetCivID(natType0))
	{  
		int mapucheVillageAID = -1;
		mapucheVillageAID = rmCreateGrouping("back mapuche village", natGrpName0+1);
		rmSetGroupingMinDistance(mapucheVillageAID, 0.0);
		rmSetGroupingMaxDistance(mapucheVillageAID, 0.20);
		rmAddGroupingToClass(mapucheVillageAID, rmClassID("natives"));
		rmAddGroupingToClass(mapucheVillageAID, rmClassID("importantItem"));
		rmAddGroupingConstraint(mapucheVillageAID, avoidTradeRoute);
		rmAddGroupingConstraint(mapucheVillageAID, avoidNatives);
		rmPlaceGroupingAtLoc(mapucheVillageAID, 0, 0.83, 0.75);
	}

	if (subCiv1 == rmGetCivID(natType1))
	{  
		int mapuchevillageBID = -1;
		mapuchevillageBID = rmCreateGrouping("left city", natGrpName1+2);
		rmSetGroupingMinDistance(mapuchevillageBID, 0.0);
		rmSetGroupingMaxDistance(mapuchevillageBID, 20.0);
		rmAddGroupingToClass(mapuchevillageBID, rmClassID("natives"));
		rmAddGroupingToClass(mapuchevillageBID, rmClassID("importantItem"));
		rmAddGroupingConstraint(mapuchevillageBID, avoidTradeRoute);
		rmAddGroupingConstraint(mapuchevillageBID, avoidNatives);
		rmPlaceGroupingAtLoc(mapuchevillageBID, 0, 0.73, 0.70);
	}

	if (subCiv2 == rmGetCivID(natType0))
	{  
		int mapuchevillageCID = -1;
		mapuchevillageCID = rmCreateGrouping("right city", natGrpName0+3);
		rmSetGroupingMinDistance(mapuchevillageCID, 0.0);
		rmSetGroupingMaxDistance(mapuchevillageCID, 20.0);
		rmAddGroupingToClass(mapuchevillageCID, rmClassID("natives"));
		rmAddGroupingToClass(mapuchevillageCID, rmClassID("importantItem"));
		rmAddGroupingConstraint(mapuchevillageCID, avoidTradeRoute);
		rmAddGroupingConstraint(mapuchevillageCID, avoidNatives);
		rmPlaceGroupingAtLoc(mapuchevillageCID, 0, 0.85, 0.25);
	}

	if (subCiv3 == rmGetCivID(natType1))
	{  
		int mapuchevillageDID = -1;
		mapuchevillageDID = rmCreateGrouping("front city", natGrpName1+4);
		rmSetGroupingMinDistance(mapuchevillageDID, 0.0);
		rmSetGroupingMaxDistance(mapuchevillageDID, 20.0);
		rmAddGroupingToClass(mapuchevillageDID, rmClassID("natives"));
		rmAddGroupingToClass(mapuchevillageDID, rmClassID("importantItem"));
		rmAddGroupingConstraint(mapuchevillageDID, avoidTradeRoute);
		rmAddGroupingConstraint(mapuchevillageDID, avoidNatives);
		rmPlaceGroupingAtLoc(mapuchevillageDID, 0, 0.73, 0.35);
	}
	...
	rmAddObjectDefConstraint(TCID, avoidRiver);
	...
	if (cNumberTeams == 2)
	{
		rmAddObjectDefConstraint(TCID, avoidMidIsland);
	...
	rmAddObjectDefConstraint(TCID, avoidNat1SocketsTC);
	rmAddObjectDefConstraint(TCID, avoidNat2SocketsTC);
	...
	for(i=0; <numTries)
	{
	   ...
	   rmSetAreaSize(cliffID, rmAreaTilesToFraction(666));
...
//		rmSetAreaCliffPainting(cliffID, false, true, true, 1.5, true);
		...
		rmAddAreaConstraint(cliffID, stayMidIsland);
  ...
  if(rmGetIsKOTH()) {
	...
	//only try to place these 10% of the time
	   if(rmRandFloat(0,1) < 0.10 && rmGetIsTreaty() == false)
	   {
