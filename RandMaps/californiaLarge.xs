...
//		rmSetOceanReveal(true);
	...
	// Resource avoidance
		...
		int avoidHunt1Far = rmCreateTypeDistanceConstraint("avoid hunt1 far", food1, 90.0);
		int avoidHunt1 = rmCreateTypeDistanceConstraint("avoid hunt1", food1, 64.0);
		int avoidHunt1Short = rmCreateTypeDistanceConstraint("avoid hunt1 short", food1, 44.0);
		int avoidHunt1Min = rmCreateTypeDistanceConstraint("avoid hunt1 min", food1, 4.0);
		int avoidHunt2Far = rmCreateTypeDistanceConstraint("avoid hunt2 far", food2, 90.0);
		int avoidHunt2 = rmCreateTypeDistanceConstraint("avoid hunt2", food2, 64.0);
		int avoidHunt2Short = rmCreateTypeDistanceConstraint("avoid hunt2 short", food2, 44.0);
		int avoidHunt2Min = rmCreateTypeDistanceConstraint("avoid hunt2 min", food2, 4.0);
		int avoidHunt3Far = rmCreateTypeDistanceConstraint("avoid hunt3 far", food3, 90.0);
		int avoidHunt3 = rmCreateTypeDistanceConstraint("avoid hunt3", food3, 64.0);
		int avoidHunt3Short = rmCreateTypeDistanceConstraint("avoid hunt3 short", food3, 44.0);
		...
		int avoidGold = rmCreateClassDistanceConstraint ("gold avoid gold med", rmClassID("Gold"), 50.0);
		...
		int avoidNuggetFar=rmCreateTypeDistanceConstraint("nugget avoid nugget far", "AbstractNugget", 88.0);
		int avoidFish=rmCreateTypeDistanceConstraint("avoid fish", fishies, 20+PlayerNum);
	...
	rmSetAreaSize(terrainLayerID, 0.50);
	...
	rmSetAreaSize(terrainLayer2ID, 0.40);
	...
	rmSetAreaSize(terrainLayer3ID, 0.30);
	...
	rmSetAreaSize(terrainLayer4ID, 0.18);
	...
	rmSetAreaSize(terrainLayer5ID, 0.09);
...
//	rmSetAreaBaseHeight(playerAreaID, 5.0);		// for testing
	...
	int socket2ID=rmCreateObjectDef("sockets to dock Trade Posts2");
		...
		vector socketLoc2 = rmGetTradeRouteWayPoint(tradeRoute2ID, 0.12);
		rmPlaceObjectDefAtPoint(socket2ID, 0, socketLoc2);

		socketLoc2 = rmGetTradeRouteWayPoint(tradeRoute2ID, TPLoc1);
		rmPlaceObjectDefAtPoint(socket2ID, 0, socketLoc2);	
		...
		socketLoc2 = rmGetTradeRouteWayPoint(tradeRoute2ID, 0.88);
		rmPlaceObjectDefAtPoint(socket2ID, 0, socketLoc2);	
...
//		rmPlaceGroupingAtLoc(nativeID1, 0, 0.85, 0.65);
		rmPlaceGroupingAtLoc(nativeID2, 0, 0.35, 0.50);
//		rmPlaceGroupingAtLoc(nativeID3, 0, 0.40, 0.60);
		}
	else {
...
//	rmAddObjectDefConstraint(playerTree2ID, stayMidIsland);
    ...
    rmSetObjectDefMaxDistance(playerHerd3ID, 50+PlayerNum/2);
	...
	rmSetObjectDefMaxDistance(playerNuggetID, 28+PlayerNum);
...
//		rmAddAreaToClass(plateau2ID, rmClassID("classPlateau"));
	...
	if (PlayerNum == 2) {
		...
		rmAddObjectDefItem(midMineA, "Minegold", 1, 0.0);
		...
		rmAddObjectDefItem(midMineB, "Minegold", 1, 0.0);
	...
	for (i= 0; < cliffcount) {
		...
		rmAddObjectDefItem(cliffMineID, "Minegold", 1, 0.0);
	...
	if (PlayerNum > 2) {
		...
		rmAddObjectDefItem(midSilverID, "Minegold", 1, 0.0);
		...
		rmPlaceObjectDefAtLoc(midSilverID, 0, 0.50, 0.50, 1.5*PlayerNum);
		}
	...
	// Secondary forest
	int secondforestcount = 10+5*PlayerNum;
	...
	if (PlayerNum == 2) {
		...
		rmPlaceObjectDefAtLoc(desertHerdID, 0, 0.50, 0.50, 1.25*PlayerNum);
	...
	int ffaHerdID = rmCreateObjectDef("herd FFA");
		...
		if (TeamNum > 2)
			rmPlaceObjectDefAtLoc(ffaHerdID, 0, 0.50, 0.50, 2*PlayerNum);
	...
	int NuggetID = rmCreateObjectDef("Nugget"); 
		...
		rmPlaceObjectDefAtLoc(NuggetID, 0, 0.50, 0.50, 2*nuggetcount);
...
//	rmAddObjectDefConstraint(whaleID, avoidWhale);
	...
	rmPlaceObjectDefAtLoc(cornerFishID, 0, 0.25, 0.10, 2);
	rmPlaceObjectDefAtLoc(cornerFishID, 0, 0.25, 0.90, 2);
	...
	rmPlaceObjectDefAtLoc(fishID, 0, 0.50, 0.50, 1.25*fishcount);
