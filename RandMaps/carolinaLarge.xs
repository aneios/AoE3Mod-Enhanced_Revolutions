...
//	rmSetOceanReveal(true);
	...
	int avoidHunt1VeryFar = rmCreateTypeDistanceConstraint("avoid hunt1 very far", food1, 100.0);
	...
	int avoidFishShort=rmCreateTypeDistanceConstraint("avoid fish short", "FishTarpon", 16.0);
	int avoidFish=rmCreateTypeDistanceConstraint("avoid fish", "FishTarpon", 24.0);
	int avoidColonyShip = rmCreateTypeDistanceConstraint("avoid colony ship", "HomeCityWaterSpawnFlag", 6.0);
  int avoidBerries=rmCreateTypeDistanceConstraint("berry vs berry", "berryBush", 90.0);
...
//	rmSetAreaMix(midSmIslandID, "himalayas_a"); 	// for testing
	...
	rmAddObjectDefItem(playerCrateID, "crateOfFood", rmRandInt(3, 3), 4.0);	// 2,3
	rmAddObjectDefItem(playerCrateID, "crateOfWood", rmRandInt(2, 2), 4.0);	// 1,2
	rmAddObjectDefItem(playerCrateID, "crateOfCoin", 2, 4.0);	// 1
	...
	int goldID = rmCreateObjectDef("gold");
		...
		rmPlaceObjectDefAtLoc(goldID, 0, 0.50, 0.50, 1.5*goldcount);
	...
	int beachPalmID = rmCreateObjectDef("beach trees");
		...
		rmPlaceObjectDefAtLoc(beachPalmID, 0, 0.50, 0.50, 4+6*PlayerNum);
	...
	int commonHuntID = rmCreateObjectDef("common herd");
		...
		rmPlaceObjectDefAtLoc(commonHuntID, 0, 0.50, 0.50, 1.25*herdcount);
