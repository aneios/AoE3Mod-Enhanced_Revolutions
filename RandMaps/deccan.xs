...
//minor balance tweaks by Durokan
// edited by vividlyplain, July 2021
...
void main(void)
{
  ...
  float xPlateauLoc = 0.00;
  float yPlateauLoc = 0.00;

  if (rmGetIsTreaty() == false || cNumberTeams > 2)
  {
    xPlateauLoc = 0.5;
    yPlateauLoc = 0.5;
  }
  ...
  int forestMidConstraint=rmCreatePieConstraint("dense middle forests", xPlateauLoc, yPlateauLoc, rmXFractionToMeters(0.0), rmXFractionToMeters(0.16), rmDegreesToRadians(0), rmDegreesToRadians(360));
  ...
  // Player constraints
  int islandConstraint=rmCreateClassDistanceConstraint("avoid islands", classIsland, 10.0);
  ...
  int avoidCoin=rmCreateTypeDistanceConstraint("avoid coin", "mine", 50+5*cNumberNonGaiaPlayers);
  int avoidCoinMin=rmCreateTypeDistanceConstraint("avoid coin min", "mine", 4.0);
  ...
  int avoidElephants = -1;
  if (cNumberNonGaiaPlayers == 2)
    avoidElephants = rmCreateTypeDistanceConstraint("avoid elephants far", "ypWildElephant", 75.0);
  else
    avoidElephants = rmCreateTypeDistanceConstraint("avoid elephants far", "ypWildElephant", 60.0);
  ...
  // Unit avoidance
  int avoidTownCenterShort=rmCreateTypeDistanceConstraint("avoid Town Center short", "townCenter", 8.0);
  int avoidTownCenter=rmCreateTypeDistanceConstraint("avoid Town Center", "townCenter", 24.0);
  int avoidNomadShort=rmCreateTypeDistanceConstraint("avoid tc wagon short", "CoveredWagon", 8.0);
  int avoidNomad=rmCreateTypeDistanceConstraint("avoid tc wagon", "CoveredWagon", 24.0);
  ...
  if (rmGetIsTreaty() == true && cNumberTeams == 2)
  {
    whichVersion = 2;
    randomRiver = 1;
  }
  ...
  if (cNumberTeams == 2)	{
    ...
    if(whichVersion == 1) {
    ...
    else {  // whichVersion == 2 - this is being used for treaty
      rmSetPlacementTeam(0);
      
      if(randomRiver == 1)
      {
        if (cNumberNonGaiaPlayers > 4)
          rmSetPlacementSection(0.06, 0.24);    // this is being used for treaty
        else
          rmSetPlacementSection(0.08, 0.23);    // this is being used for treaty
      }
      else 
        rmSetPlacementSection(0.03, 0.2);

      if(teamZeroCount == 1)
        rmSetPlacementSection(.125, .13);
      if(teamZeroCount == 2)
        rmPlacePlayersCircular(.35, .35, 0);
      else
        rmPlacePlayersCircular(.35+0.005*cNumberNonGaiaPlayers, .35+0.005*cNumberNonGaiaPlayers, 0);
      ...
      {
        if (cNumberNonGaiaPlayers > 4)
          rmSetPlacementSection(.52, .70);    // this is being used for treaty
        else
          rmSetPlacementSection(.53, .68);    // this is being used for treaty
      }
      else 
        rmSetPlacementSection(.53, .70);
      
      if(teamOneCount == 1)
        rmSetPlacementSection(.625, .63);
      if(teamOneCount == 2)
        rmPlacePlayersCircular(.35, .35, 0);
      else
        rmPlacePlayersCircular(.35+0.005*cNumberNonGaiaPlayers, .35+0.005*cNumberNonGaiaPlayers, 0);
  ...
  for(i=1; <cNumberPlayers) {
	...
	rmSetAreaTerrainType(id, "deccan\ground_dirt3_deccan");
  ...
  int northRiver = rmRiverCreate(-1, "Deccan Plateau River", 1, 1, 5, 5);
  int southRiver = rmRiverCreate(-1, "Deccan Plateau River", 1, 1, 5, 5);
  float riverXLoc = 0;
  float riverYLoc = 0;
  ...
  if (randomRiver == 1) {
    if(whichVersion == 1) {
      ...
      riverXLoc = .3;
      riverYLoc = .7;
      rmRiverAddWaypoint(northRiver, 0.0, 0.36);
    }
    if (rmGetIsTreaty() == true && cNumberTeams == 2)
      rmRiverSetShallowRadius(northRiver, 8+cNumberNonGaiaPlayers/2);
    else
    rmRiverSetShallowRadius(northRiver, 5);
    rmRiverAddShallow(northRiver, 0.2);
    if (rmGetIsTreaty() == true && cNumberTeams == 2)
      rmRiverAddShallow(northRiver, 0.4);
    rmRiverAddShallow(northRiver, 0.5);
     if (rmGetIsTreaty() == true && cNumberTeams == 2)
     rmRiverAddShallow(northRiver, 0.6);
    rmRiverAddShallow(northRiver, 0.8);
  ...
  string patchMix = "deccan_grassy_Dirt_a";
  string patchTerrain = "deccan\ground_dirt2_deccan";
  string patchType1 = "deccan\ground_dirt2_deccan";
  string patchType2 = "deccan\ground_dirt3_deccan";
  ...
  for (i=0; < 30) {
	...
	rmAddAreaConstraint(patch, avoidNomadShort);
	rmAddAreaConstraint(patch, avoidTownCenterShort);
    rmBuildArea(patch); 
  }
	...
	rmSetAreaSize(avoidTHisID, 0.08+0.01*cNumberNonGaiaPlayers);
	rmSetAreaLocation(avoidTHisID, xPlateauLoc, yPlateauLoc);
	for(i=1; < cNumberPlayers)
  	rmAddAreaInfluenceSegment(avoidTHisID, xPlateauLoc, yPlateauLoc, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));
  rmAddAreaInfluenceSegment(avoidTHisID, 0.20, 0.80, 0.80, 0.20);
  if (cNumberNonGaiaPlayers >= 4)
  {
    rmAddAreaInfluenceSegment(avoidTHisID, xPlateauLoc, yPlateauLoc, 0.75, 0.65);
    rmAddAreaInfluenceSegment(avoidTHisID, xPlateauLoc, yPlateauLoc, 0.35, 0.25);
    rmAddAreaInfluenceSegment(avoidTHisID, xPlateauLoc, yPlateauLoc, 0.65, 0.65);
    rmAddAreaInfluenceSegment(avoidTHisID, xPlateauLoc, yPlateauLoc, 0.35, 0.35);
    rmAddAreaInfluenceSegment(avoidTHisID, xPlateauLoc, yPlateauLoc, 0.75, 0.55);
    rmAddAreaInfluenceSegment(avoidTHisID, xPlateauLoc, yPlateauLoc, 0.45, 0.25);
  }
...
//	rmSetAreaMix(avoidTHisID, "testmix");   // for testing
	...
	if (cNumberTeams == 2 && rmGetIsTreaty() == true)
		rmBuildArea(avoidTHisID); 

  // Cliff only present in non-FFA games    // actually it's just flat in FFA games
  int startingCliff1=rmCreateArea("Plateau");
  if (rmGetIsTreaty() == false)
    rmSetAreaSize(startingCliff1, .125);
  else
    rmSetAreaSize(startingCliff1, .135);
  rmSetAreaLocation(startingCliff1, xPlateauLoc, yPlateauLoc);
  ...
  if (rmGetIsTreaty() == true && cNumberTeams == 2)
  {
    rmSetAreaCoherence(startingCliff1, .9);
    rmSetAreaCliffEdge(startingCliff1, 1, 1.0, 0, 1.0, 0);
  	if (cNumberNonGaiaPlayers == 2)
      rmAddAreaCliffEdgeAvoidClass(startingCliff1, classAvoidance, 6);
  	else
      rmAddAreaCliffEdgeAvoidClass(startingCliff1, classAvoidance, 4);
  }
  else
  {
  ...
  // When N/W river present, push trade routes towards center
    ...
    // east
    else {    // this is being used for treaty
      if (rmGetIsTreaty() == false || cNumberTeams > 2)
      {
  ...
  {
    if (cNumberNonGaiaPlayers == 2)
      rmPlaceObjectDefAtLoc(socketID, 0, 0.9, 0.6);
    else
  ...
  if(cNumberNonGaiaPlayers > 3 || randomRiver == 1) {
    if(cNumberTeams > 2 && cNumberNonGaiaPlayers == 3) {
      ...
      {
        if (cNumberNonGaiaPlayers == 2)
          rmPlaceObjectDefAtLoc(socketID, 0, 0.70, 0.30);
        else
  ...
  {
    if (cNumberNonGaiaPlayers == 2)
      rmPlaceObjectDefAtLoc(socketID, 0, 0.4, 0.1);
    else
      rmPlaceObjectDefAtLoc(socketID, 0, 0.55, 0.05);
  }
  else
  rmPlaceObjectDefAtPoint(socketID, 0, socketLoc);
...
if (rmGetIsTreaty() == false || (cNumberNonGaiaPlayers == 2 && rmGetIsTreaty() == true) || cNumberTeams > 2)
{
  if(randomRiver == 1) {
    // north
    if(whichVersion == 1) {
...
{
	// Set up Natives
	string natType1 = "Udasi";
	string natType2 = "Bhakti";
	string natType3 = "Sufi";
...
if (whichNative == 1)
{
	natGrpName1 = "native udasi village ";
	natGrpName2 = "native bhakti village ";
}
else if (whichNative == 2)
{
	natGrpName1 = "native udasi village ";
	natGrpName2 = "native sufi mosque deccan ";
}
else
{
	natGrpName1 = "native sufi mosque deccan ";
	natGrpName2 = "native bhakti village ";
}
    ...
    if (cNumberNonGaiaPlayers == 2)
    {
	  	rmPlaceGroupingAtLoc(nativeID0, 0, 0.60, 0.80);
		  rmPlaceGroupingAtLoc(nativeID1, 0, 0.90, 0.44);
	  	rmPlaceGroupingAtLoc(nativeID2, 0, 0.20, 0.40);
		  rmPlaceGroupingAtLoc(nativeID3, 0, 0.56, 0.10);
    }
    else
    {
	  	rmPlaceGroupingAtLoc(nativeID0, 0, 0.54+0.005*cNumberNonGaiaPlayers, 0.75+0.005*cNumberNonGaiaPlayers);
		  rmPlaceGroupingAtLoc(nativeID1, 0, 0.40, 0.95);
	  	rmPlaceGroupingAtLoc(nativeID2, 0, 0.25-0.005*cNumberNonGaiaPlayers, 0.45-0.005*cNumberNonGaiaPlayers);
		  rmPlaceGroupingAtLoc(nativeID3, 0, 0.05, 0.60);
    }
}
...
if (cNumberTeams == 2 && rmGetNumberPlayersOnTeam(0) == rmGetNumberPlayersOnTeam(1))
	...
	rmAddObjectDefConstraint(TCID, avoidNomadShort);
	rmAddObjectDefConstraint(TCID, avoidTownCenterShort);
	...
	rmAddObjectDefConstraint(playerSilverID, avoidNomadShort);
  ...
  if (rmGetIsTreaty() == true)
    rmAddObjectDefItem(playerFoodID, "ypNilgai", 16, 4.0);
  else
  rmAddObjectDefItem(playerFoodID, "ypNilgai", 8, 3.0);
  rmAddObjectDefToClass(playerFoodID, foodClass);
  rmSetObjectDefMinDistance(playerFoodID, 10);
  rmSetObjectDefMaxDistance(playerFoodID, 12);
	rmAddObjectDefConstraint(playerFoodID, avoidAll);
  rmAddObjectDefConstraint(playerFoodID, avoidFood);
  rmAddObjectDefConstraint(playerFoodID, avoidTownCenterShort);
  rmAddObjectDefConstraint(playerFoodID, avoidNomadShort);
  rmAddObjectDefConstraint(playerFoodID, avoidImpassableLand);
  rmSetObjectDefCreateHerd(playerFoodID, true);

	int playerTreeID=rmCreateObjectDef("player trees");
  rmAddObjectDefItem(playerTreeID, "ypTreeDeccan", 10, 6);
  ...
  rmAddObjectDefConstraint(playerTreeID, avoidTownCenterShort);
  rmAddObjectDefConstraint(playerTreeID, avoidNomadShort);
  ...
  rmAddObjectDefConstraint(playerNuggetID, avoidTownCenterShort);
  rmAddObjectDefConstraint(playerNuggetID, avoidNomadShort);
  ...
  rmAddObjectDefConstraint(playerCrateID, avoidTownCenterShort);
  rmAddObjectDefConstraint(playerCrateID, avoidNomadShort);
  rmAddObjectDefConstraint(playerCrateID, avoidImpassableLand);
  
int avoidGold_dk=rmCreateTypeDistanceConstraint("starting berries avoid coin dk", "minegold", 20.0);
  ...
  rmAddObjectDefConstraint(playerBerryID, avoidGold_dk);
  rmAddObjectDefConstraint(playerBerryID, avoidTownCenterShort);
  rmAddObjectDefConstraint(playerBerryID, avoidNomadShort);
  ...
  rmSetStatusText("",0.55);
	...
	int avoidThis2ID=rmCreateArea("trees avoid this");
	rmSetAreaSize(avoidThis2ID, 0.10);
	rmSetAreaLocation(avoidThis2ID, 0.20, 0.80);
  rmAddAreaInfluenceSegment(avoidThis2ID, 0.40, 1.00, 0.00, 0.60);
	rmAddAreaToClass(avoidThis2ID, classIsland);
//	rmSetAreaMix(avoidThis2ID, "testmix");   // for testing
	rmSetAreaCoherence(avoidThis2ID, 1.00);
	rmAddAreaConstraint(avoidThis2ID, avoidWater10);
	if (cNumberTeams == 2 && rmGetIsTreaty() == true)
		rmBuildArea(avoidThis2ID); 

  // rim trees for treaty
  ...
  int rimTreeID = rmCreateObjectDef("rim tree");
  rmAddObjectDefItem(rimTreeID, "ypTreeAshoka", 1, 0.0);
  rmSetObjectDefMinDistance(rimTreeID, rmXFractionToMeters(0.00));
  rmSetObjectDefMaxDistance(rimTreeID, rmXFractionToMeters(0.50));
  rmAddObjectDefConstraint(rimTreeID, stayNearEdge);    
  rmAddObjectDefConstraint(rimTreeID, avoidCoinMin);    
  rmAddObjectDefConstraint(rimTreeID, avoidImpassableLand);
  rmAddObjectDefConstraint(rimTreeID, avoidImportantItem);
  rmAddObjectDefConstraint(rimTreeID, avoidWater10);
  rmAddObjectDefConstraint(rimTreeID, avoidTradeRouteFar);
  rmAddObjectDefConstraint(rimTreeID, avoidTradeRouteSockets);
  rmAddObjectDefConstraint(rimTreeID, islandConstraint);
  rmAddObjectDefConstraint(rimTreeID, playerConstraint);
	if (cNumberTeams == 2 && rmGetIsTreaty() == true)
    rmPlaceObjectDefAtLoc(rimTreeID, false, 0.5, 0.5, 500+100*cNumberNonGaiaPlayers);
  ...
  {
    rmSetObjectDefMinDistance(rightSilverID, rmXFractionToMeters(0.00));
    rmSetObjectDefMaxDistance(rightSilverID, rmXFractionToMeters(0.025));
    rmPlaceObjectDefAtLoc(rightSilverID, 0, 0.5, 0.2, 1);
    rmPlaceObjectDefAtLoc(rightSilverID, 0, 0.8, 0.5, 1);
    rmPlaceObjectDefAtLoc(rightSilverID, 0, 0.6, 0.5, 1);
    rmPlaceObjectDefAtLoc(rightSilverID, 0, 0.4, 0.5, 1);
    rmPlaceObjectDefAtLoc(rightSilverID, 0, 0.55, 0.75, 1);
    rmPlaceObjectDefAtLoc(rightSilverID, 0, 0.25, 0.45, 1);
    rmPlaceObjectDefAtLoc(rightSilverID, 0, 0.8, 0.2, 1);
    rmPlaceObjectDefAtLoc(rightSilverID, 0, 0.2, 0.8, 1);
  }
else
  {
    rmAddObjectDefConstraint(rightSilverID, avoidCoin);    
    rmAddObjectDefConstraint(rightSilverID, avoidGold);
    rmSetObjectDefMinDistance(rightSilverID, rmXFractionToMeters(0.1));
  ...
  for (i=0; <numTries) {   
    ...
    if (rmGetIsTreaty() == true)
    {
    rmSetAreaSize(forest, rmAreaTilesToFraction(100));
  ...
  for (i=0; <numTries) {   
    ...
    rmSetAreaForestUnderbrush(outerForest, 0.4);
    rmSetAreaCoherence(outerForest, 0.5);
    rmAddAreaToClass(outerForest, rmClassID("classForest"));
	rmAddAreaTerrainLayer(outerForest, "deccan\ground_dirt4_deccan", 1);
  ...
  rmAddObjectDefItem(foodID, "ypNilgai", foodCount, 3.0);
  ...
  rmAddObjectDefConstraint(foodID, forestObjConstraint); 
  rmSetObjectDefCreateHerd(foodID, true);
  rmPlaceObjectDefAtLoc(foodID, 0, xPlateauLoc, yPlateauLoc, 3*cNumberNonGaiaPlayers);
  ...
  rmAddObjectDefItem(food2ID, "ypNilgai", foodCount, 3.0);
  ...
  rmAddObjectDefConstraint(food2ID, forestObjConstraint); 
  ...
  rmAddObjectDefItem(food3ID, "ypWildElephant", foodCount, foodCount+1);
  ...
  rmAddObjectDefConstraint(food3ID, forestObjConstraint);
  ...
  if(cNumberNonGaiaPlayers>2){
      ...
      rmPlaceObjectDefAtLoc(herdID, 0, xPlateauLoc, yPlateauLoc, cNumberNonGaiaPlayers*2); 
    }else{
          
        int avoidHerdablesTwoPlayer=rmCreateTypeDistanceConstraint("avoids cattle two player", "ypWaterBuffalo", 28.0); 
        int avoidTC2p = rmCreateTypeDistanceConstraint("avoid TCs 2p", "TownCenter", 32.0);
        int buffaloTwoPlayer=rmCreateObjectDef("buffaloTwoPlayer");
        rmAddObjectDefItem(buffaloTwoPlayer, "ypWaterBuffalo", 1, 1);
        rmSetObjectDefMinDistance(buffaloTwoPlayer, 0.0);
        rmSetObjectDefMaxDistance(buffaloTwoPlayer, rmXFractionToMeters(0.2));
        rmAddObjectDefConstraint(buffaloTwoPlayer, avoidHerdablesTwoPlayer);
        rmAddObjectDefConstraint(buffaloTwoPlayer, avoidAll);
        rmAddObjectDefConstraint(buffaloTwoPlayer, avoidTradeRoute);
        rmAddObjectDefConstraint(buffaloTwoPlayer, avoidImpassableLandShort);
        rmAddObjectDefConstraint(buffaloTwoPlayer, avoidTC2p);
        rmPlaceObjectDefAtLoc(buffaloTwoPlayer, 0, xPlateauLoc, yPlateauLoc, 8); 
    }
  ...
  rmPlaceObjectDefAtLoc(crazyNugget, 0, xPlateauLoc, yPlateauLoc, cNumberNonGaiaPlayers);
  ...
  rmPlaceObjectDefAtLoc(hardNugget, 0, xPlateauLoc, yPlateauLoc, cNumberNonGaiaPlayers);
  ...
  rmPlaceObjectDefAtLoc(mediumNugget, 0, xPlateauLoc, yPlateauLoc, cNumberNonGaiaPlayers*2);
