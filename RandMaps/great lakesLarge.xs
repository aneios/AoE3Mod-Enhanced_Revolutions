...
//		rmSetMapElevationParameters(cElevTurbulence, 0.02, 4, 0.7, 8.0);
   ...
   // Map edge constraints
	...
	int avoidNuggets=rmCreateClassDistanceConstraint("stuff avoids nuggets", rmClassID("nuggets"), 65.0);
	int deerConstraint=rmCreateTypeDistanceConstraint("avoid the deer", "deer", 65.0);
	int shortNuggetConstraint=rmCreateTypeDistanceConstraint("avoid nugget objects", "AbstractNugget", 7.0);
	int shortDeerConstraint=rmCreateTypeDistanceConstraint("short avoid the deer", "deer", 40.0);
	int mooseConstraint=rmCreateTypeDistanceConstraint("avoid the moose", "moose", 65.0);
	...
	int fishLand = rmCreateTerrainDistanceConstraint("fish land", "land", true, 12.0);
...
if (seasonPicker < 0.5)
	...
	if (rmGetIsKOTH() == true) {
		...
		rmSetAreaSize(tradeIslandA, rmAreaTilesToFraction(555));
		rmSetAreaLocation(tradeIslandA, 0.5, 0.5);
  ...
  if(rmGetIsKOTH()) {
    ...
    if(seasonPicker > 0.5) {
      ypKingsHillLandfill(.5, .5, rmAreaTilesToFraction(350), 1.5, "greatlakes_snow", 0);
    ...
    int avoidIslandSilver=rmCreateTypeDistanceConstraint("avoid Island Silver", "mine", 12.0);
...
//	rmAddObjectDefConstraint(startingTCID, avoidTradeRoute);
	...
	// Starting herd - why wasn't this already in the code? :/
	...
	rmSetObjectDefMaxDistance(playerHerdID, 18);
  ...
  int berry = 0;
	...
	rmSetObjectDefMaxDistance(StartElkID, 14);
...
// Define and place forests - north and south
	...
	numTries=5*cNumberNonGaiaPlayers+2;  // DAL - 4 here, 4 below
	failCount=0;
	for (i=0; <numTries)
		{   
	...
	numTries=5*cNumberNonGaiaPlayers+2;  // DAL - 4 here, 4 above.
	failCount=0;
	for (i=0; <numTries)
		{   
...
// Place some extra deer herds.  
	...
	numTries=3*cNumberNonGaiaPlayers+2;
	for (i=0; <numTries)
	{
...
//	rmAddObjectDefConstraint(mooseHerdID, Northward);
	...
	rmAddObjectDefItem(fishID, "FishSalmon", 3, 10.0);
...
//	rmAddObjectDefConstraint(sheepID, avoidCliffs);
	...
	rmPlaceObjectDefAtLoc(sheepID, 0, 0.5, 0.5, 10+cNumberNonGaiaPlayers*3.5);
