// GREAT PLAINS LARGE
// Nov 06 - YP update
// March 2021 edited by vividlyplain for DE - based on changes to regular GP Updated May 2021.
...
//   float NativeVillageLoc = rmRandFloat(0,1); //
   ...
   */
   ...
   rmSetAreaEdgeFilling(hillSoutheastID, 5);
    ...
    int avoidKOTHFar = rmCreateTypeDistanceConstraint("avoid koth far", "ypKingsHill", 24.0);
...
*/
	...
	rmSetObjectDefMinDistance(playerTree2ID, 50);
	...
	// Extra trees around base
	int playerTree3ID = rmCreateObjectDef("player 3rd trees");
	rmAddObjectDefItem(playerTree3ID, "TreeGreatPlains", 8, 8.0);
	rmAddObjectDefItem(playerTree3ID, "TreeTexas", 4, 8.0);
	rmSetObjectDefMinDistance(playerTree3ID, 36);
    rmSetObjectDefMaxDistance(playerTree3ID, 36);
	...
	rmAddObjectDefConstraint(playerTree3ID, avoidSE);
	rmAddObjectDefConstraint(playerTree3ID, avoidNW);
	rmAddObjectDefConstraint(playerTree3ID, avoidNativesShort);
	rmAddObjectDefConstraint(playerTree3ID, avoidTradeRouteSmall);
	rmAddObjectDefConstraint(playerTree3ID, avoidSocketMore);
    ...
    rmSetObjectDefMaxDistance(playerHerd3ID, 60);
	...
	//	vector TCLoc = rmGetUnitPosition(rmGetUnitPlacedOfPlayer(TCID, i));
		...
		rmPlaceObjectDefAtLoc(playerTree3ID, 0, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));
   ...
   // Text
	...
	int pondConstraintMed=rmCreateClassDistanceConstraint("med avoid ponds", rmClassID("pond"), 24.0);
   ...
   rmSetStatusText("",0.50);
	...
	//	rmAddObjectDefConstraint(silverID, avoidSmollIslandMin);
		...
		if (PlayerNum > 2)
			rmPlaceObjectDefAtLoc(silverID, 0, 0.5, 0.5, cNumberNonGaiaPlayers*4);
		else
			rmPlaceObjectDefAtLoc(silverID, 0, 0.5, 0.5, cNumberNonGaiaPlayers*2);
   ...
   rmSetStatusText("",0.60);
	...
	rmAddObjectDefItem(plainsTreeID, "TreeGreatPlains", rmRandInt(13,16), 20.0);
	rmAddObjectDefItem(plainsTreeID, "TreeTexas", rmRandInt(5,6), 12.0);
	...
	rmAddObjectDefConstraint(plainsTreeID, avoidForestVeryFar);
	...
	rmPlaceObjectDefAtLoc(plainsTreeID, 0, 0.50, 0.50, 4+5*cNumberNonGaiaPlayers);

	// Rim Trees
	int rimTreeID = rmCreateObjectDef("rim trees");
	rmAddObjectDefItem(rimTreeID, "TreeGreatPlains", rmRandInt(6,8), 8.0);
	...
	rmAddObjectDefConstraint(rimTreeID, avoidForestFar);
	...
	rmPlaceObjectDefAtLoc(rimTreeID, 0, 0.50, 0.50, 4+5*cNumberNonGaiaPlayers);
   ...
   rmSetStatusText("",0.70); 
	...
	rmAddObjectDefItem(NWTreeID, "TreeGreatPlains", rmRandInt(8,10), 8.0);
	...
	rmAddObjectDefConstraint(NWTreeID, avoidForestFar);
	...
	rmAddObjectDefItem(SETreeID, "TreeGreatPlains", rmRandInt(8,10), 8.0);
	...
	rmAddObjectDefConstraint(SETreeID, avoidForestFar);
   ...
   rmSetStatusText("",0.80);
	...
	rmAddObjectDefConstraint(midbisonID, avoidBison);
	...
	if (PlayerNum == 2) {
		...
		rmAddObjectDefConstraint(staticBison1ID, avoidBison);
	...
	if (cNumberNonGaiaPlayers > 2)
		rmPlaceObjectDefAtLoc(pronghornID, 0, 0.5, 0.5, 2*cNumberNonGaiaPlayers);
	else 
   ...
   rmSetObjectDefCreateHerd(pronghorn2ID, true);
	if (cNumberNonGaiaPlayers > 2)
		rmPlaceObjectDefAtLoc(pronghorn2ID, 0, 0.5, 0.5, 2*cNumberNonGaiaPlayers);
	else 
   ...
   // Define and place Nuggets
	...
	int Nugget3ID = rmCreateObjectDef("nugget lvl3 "); 
		...
		rmPlaceObjectDefAtLoc(Nugget3ID, 0, 0.50, 0.50, 2*treasure3count);
	...
	int Nugget2ID = rmCreateObjectDef("nugget lvl2 "); 
		...
		rmPlaceObjectDefAtLoc(Nugget2ID, 0, 0.50, 0.50, 2*treasure2count);
...
*/
	...
	if ( areThereBuffalo == 1 )
	{
		...
		rmPlaceGroupingAtLoc(bisonCarcass, 0, 0.5, 0.5, 1+howManyBuffalo);
	}
	...
	rmPlaceObjectDefAtLoc(vultureID, 0, 0.5, 0.5, 3);
