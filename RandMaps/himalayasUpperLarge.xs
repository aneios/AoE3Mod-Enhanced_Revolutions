// LARGE Himalayas - started with a modified version of Rockies.
// Aug 06
// PJJ
...
void main(void)
{
  ...
  string lightingType = "HimalayasUpper_skirmish";
  ...
  int forestVsForestConstraint=rmCreateClassDistanceConstraint("forest vs. other forests", rmClassID("classForest"), 35.0);
  if ( cNumberNonGaiaPlayers < 3 ) {
    forestVsForestConstraint=rmCreateClassDistanceConstraint("forest vs. other forests smaller", rmClassID("classForest"), 25.0);
  }
  ...
  int avoidHuntable1=rmCreateTypeDistanceConstraint("huntable1 avoids food", huntable1, 55.0);
  int avoidHuntable2=rmCreateTypeDistanceConstraint("huntable2 avoids food", huntable2, 60.0);
  ...
  int coinAvoidCoin= rmCreateTypeDistanceConstraint("coin avoids coin", "gold", 75.0);
  ...
  int avoidNuggetsLong=rmCreateTypeDistanceConstraint("stuff avoids nuggets far", "AbstractNugget", 75.0);
  ...
  // ridge constraints
  int ridgeAvoidsRidge = rmCreateClassDistanceConstraint("ridges avoid other ridges", classStartingRidge, 40+3*cNumberNonGaiaPlayers);
  int resourceAvoidsRidge = rmCreateClassDistanceConstraint("resources avoid ridges", classStartingRidge, 8.0);
...
//  rmAddGroupingConstraint(lakotaVillageAID, playerConstraintImportantItem);
  ...
  rmPlaceGroupingAtLoc(lakotaVillageAID, 0, 0.5, 0.35);
...
//  rmAddGroupingConstraint(lakotaVillageBID, playerConstraintImportantItem);
  ...
  rmPlaceGroupingAtLoc(lakotaVillageBID, 0, 0.5, 0.65);
  ...
  if(cNumberNonGaiaPlayers > 4) {
    ...
    rmAddGroupingConstraint(lakotaVillageAID, avoidTradeRoute);
    rmAddGroupingConstraint(lakotaVillageAID, avoidSocket);
    rmPlaceGroupingAtLoc(lakotaVillageAID, 0, 0.5, 0.65);
    ...
    rmAddGroupingConstraint(lakotaVillageBID, avoidTradeRoute);
    rmAddGroupingConstraint(lakotaVillageBID, avoidSocket);
    rmPlaceGroupingAtLoc(lakotaVillageBID, 0, 0.5, 0.35);
 ...
 // build ridges with ridge avoidance to start a basic topography   
  ...
  randomRidge = 4;
  ridgeAvoid = 5;  
  extraRidges = 8 + cNumberNonGaiaPlayers;
  numTries = -1;
  failCount = -1;
  ...
  for(i=0; < extraRidges) {
    ...
    if(rmRandInt(1,5) <= randomRidge) {
      ...
      rmSetAreaSize(startingRidge, rmAreaTilesToFraction(75), rmAreaTilesToFraction(125));
      rmSetAreaWarnFailure(startingRidge, false);
      rmSetAreaCoherence(startingRidge, midCliffCoherence);
...
// Vary some terrain
  ...
  for (i=0; <numTries) {   
    ...
    rmAddAreaConstraint(northForest, avoidImpassableLand);
    if(cNumberNonGaiaPlayers==2){
  ...
  for (i=0; <numTries) {   
    ...
    rmAddAreaConstraint(northForest2, avoidImpassableLand);
    ...
    int avoidGoldTypeFar = rmCreateTypeDistanceConstraint("avoid gold type 2 far ", "gold", 48.0);
...
int mapElk = rmCreateObjectDef("mapElk");
    ...
    rmSetObjectDefMaxDistance(mapElk, 26);
    ...
    rmAddObjectDefConstraint(mapElk, resourceAvoidsRidge);    
    ...
    rmAddObjectDefConstraint(mapHunts, resourceAvoidsRidge);
    rmAddObjectDefConstraint(mapHunts, avoidWater6);
        
    //right side hunts
    rmPlaceObjectDefAtLoc(mapHunts, 0, 0.72, 0.43, 1);
    rmPlaceObjectDefAtLoc(mapHunts, 0, 0.66, 0.2, 1);   
    rmPlaceObjectDefAtLoc(mapElk, 0, 0.89, 0.42, 1);   

    //left side hunts
    rmPlaceObjectDefAtLoc(mapHunts, 0, 0.28, 0.57, 1);
    rmPlaceObjectDefAtLoc(mapHunts, 0, 0.34, 0.8, 1);   
    rmPlaceObjectDefAtLoc(mapElk, 0, 0.11, 0.58, 1);   
          
    //tp line hunts
    rmPlaceObjectDefAtLoc(mapHunts, 0, 0.5, 0.15, 1);   
    rmPlaceObjectDefAtLoc(mapHunts, 0, 0.5, 0.85, 1);
    rmPlaceObjectDefAtLoc(mapElk, 0, 0.5, 0.38, 1);   
    rmPlaceObjectDefAtLoc(mapElk, 0, 0.5, 0.62, 1);   
  ...
  rmAddObjectDefConstraint(nuggetID3, stayMidSmIsland);
  
  rmSetNuggetDifficulty(4, 4);
  if (cNumberNonGaiaPlayers > 2 && rmGetIsTreaty() == false)
  	rmPlaceObjectDefAtLoc(nuggetID3, 0, 0.5, 0.5, cNumberNonGaiaPlayers); 
  ...
  rmSetNuggetDifficulty(3, 3);
 	rmPlaceObjectDefAtLoc(nuggetID2, 0, 0.5, 0.5, cNumberNonGaiaPlayers*2); 
