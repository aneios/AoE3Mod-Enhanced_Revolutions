...
// initialize map type variables
  ...
  string baseMix = "coastal_honshu_b";
  string baseTerrain = "water";
  string playerTerrain = "coastal_japan\ground_dirt5_co_japan";
  string playerTerrain2 = "coastal_japan\ground_dirt2_co_japan";  
  string seaType = "Coastal Honshu";
  string cliffType = "Coastal Honshu";
 
  string forestType = "Coastal Japan Forest";
  string startTreeType = "ypTreeJapaneseMaple";
  
  string baseIslandTerrain = "coastal_honshu_c";
  string islandTerrain = "coastal_honshu_b";
  ...
  string patchTerrain = "coastal_japan\ground_forest_co_japan";
  string patchType1 = "coastal_japan\ground_dirt4_co_japan";
  string patchType2 = "coastal_japan\ground_dirt2_co_japan";
  
  string lightingType = "Honshu_skirmish";
...
void main(void)
{
  ...
  int avoidTownCenter=rmCreateTypeDistanceConstraint("avoid Town Center", "townCenter", 10.0);
	...
	// Fish Constraints
	int fishVsFishID=rmCreateTypeDistanceConstraint("fish v fish", fish1, 30.0);		
  ...
  int avoidGold=rmCreateTypeDistanceConstraint("avoid gold", "minegold", 75.0);	
  ...
  int avoidRandomTurkeys=rmCreateTypeDistanceConstraint("avoid random turkeys", huntable1, 80.0);	
	avoidNugget=rmCreateTypeDistanceConstraint("nugget avoid nugget", "abstractNugget", 55.0); 
  ...
  int avoidNuggetFar=rmCreateTypeDistanceConstraint("nugget avoid nugget far", "abstractNugget", 100.0); 
  ...
  int avoidBanner = rmCreateTypeDistanceConstraint("versus banners", "ypPropsJapaneseBanner", 75.0);
	...
	int avoidWater20 = rmCreateTerrainDistanceConstraint("avoid water long", "Land", false, 35.0);
  ...
  if (cNumberTeams == 2 ) {
	...
	float playerFraction=rmAreaTilesToFraction(300);//20
    ...
    rmAddAreaTerrainLayer(id, playerTerrain2, 0, 2);
  ...
  rmEchoInfo("70 percent map loaded");
	...
	rmSetObjectDefMaxDistance(TCID, 2.0); //8
  ...
  rmAddObjectDefConstraint(playerBerriesID, avoidImpassableLand);
	...
	//Prepare to place player starting pop block		// new shrine nugget
	int playerPopBlock=rmCreateObjectDef("player pop block");
//	rmAddObjectDefItem(playerPopBlock, "ypPopBlock", 1, 0.0);	
	rmAddObjectDefItem(playerPopBlock, "Nugget", 1, 0.0);
	rmSetNuggetDifficulty(222,222);
  rmSetObjectDefMinDistance(playerPopBlock, 16);	// 7	
  rmSetObjectDefMaxDistance(playerPopBlock, 18);	// 10
  ...
  rmClearClosestPointConstraints(); 
	...
	for(i=1; <cNumberPlayers)
   {
    ...
    rmPlaceObjectDefAtLoc(playerCastle, i, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));      
		...
		rmPlaceObjectDefAtLoc(playerBerriesID, 0, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));   
    rmPlaceObjectDefAtLoc(playerGoldID, 0, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));   
		rmPlaceObjectDefAtLoc(playerTurkeyID, 0, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));  										
		rmPlaceObjectDefAtLoc(playerCrateID, i, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));
    //~ rmPlaceObjectDefAtLoc(startBannerID, i, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));
    //~ rmPlaceObjectDefAtLoc(startBannerID, i, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));
    rmPlaceObjectDefAtLoc(playerPopBlock, i, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));
	rmSetAreaTerrainType(i, playerTerrain);
    ...
    int fishingBoatID=rmCreateObjectDef("FishingBoat1"+i);
    
	if (rmGetPlayerCiv(i) ==  rmGetCivID("Japanese") || rmGetPlayerCiv(i) ==  rmGetCivID("Indians") || rmGetPlayerCiv(i) ==  rmGetCivID("Chinese"))
		rmAddObjectDefItem(fishingBoatID, "ypFishingBoatAsian", 2, 10.0);    
	else if (rmGetPlayerCiv(i) ==  rmGetCivID("DEHausa") || rmGetPlayerCiv(i) ==  rmGetCivID("DEEthiopians"))
		rmAddObjectDefItem(fishingBoatID, "deFishingBoatAfrican", 2, 10.0);    
	else
		rmAddObjectDefItem(fishingBoatID, "FishingBoat", 2, 10.0);
    
    rmPlaceObjectDefAtLoc(fishingBoatID, i, rmXMetersToFraction(xsVectorGetX(closestPoint)), rmZMetersToFraction(xsVectorGetZ(closestPoint)));
  ...
  int nugget2b= rmCreateObjectDef("nugget water hard"); 
  ...
  rmAddObjectDefConstraint(nugget2b, avoidNuggetFar);
  ...
  // Regicide Triggers
    ...
    rmSetTriggerConditionParam("SrcObject", rmGetScenarioNameOfUnitPlacedOfPlayer(playerDaimyo, i), false);
    
    rmAddTriggerEffect("Set Telemetry Regicide");
