...
//Durokan's June 28 2020 1v1 rebalance for DE
// Updated for African DLC by vividlyplain, June 2021
...
void main(void)
{
  ...
  string seaType = "indochina Water";
...
// Constraints
  ...
  int avoidHuntable1=rmCreateTypeDistanceConstraint("avoid huntable1", huntable1, 75.0);
	int avoidHuntable2=rmCreateTypeDistanceConstraint("avoid huntable2", huntable2, 75.0);
  int avoidNuggetsShort=rmCreateTypeDistanceConstraint("vs nugget short", "AbstractNugget", 10.0);
  int avoidNugget=rmCreateTypeDistanceConstraint("nugget vs. nugget", "AbstractNugget", 65.0);
	int avoidNuggetsFar=rmCreateTypeDistanceConstraint("nugget vs. nugget far", "AbstractNugget", 85.0);
  int avoidNuggetWater=rmCreateTypeDistanceConstraint("nugget vs. nugget water", "AbstractNugget", 85.0);
  int avoidHerdable=rmCreateTypeDistanceConstraint("herdables avoid herdables", herdableType, 75.0);
  int avoidBerries=rmCreateTypeDistanceConstraint("avoid berries", "berrybush", 75.0);
  ...
  int nuggetVsFlag = rmCreateTypeDistanceConstraint("nugget v flag", "HomeCityWaterSpawnFlag", 8.0);
    ...
    int avoidSocket2=rmCreateClassDistanceConstraint("socket avoidance gold", rmClassID("socketClass"), 8.0);
    ...
    int avoidHunt2=rmCreateTypeDistanceConstraint("herds avoid herds2", "huntable", 64.0);
    int avoidHunt3=rmCreateTypeDistanceConstraint("herds avoid herds3", "huntable", 28.0);
...
int avoidStartingGold_dk =rmCreateTypeDistanceConstraint("starting berries avoid starting coin dk", "Mine", 16.0);
  ...
  rmAddAreaInfluenceSegment(mainIslandID, 1.0, 0.85, 0.0, 0.85);
   ...
   // Main river
  int mainRiver = rmRiverCreate(-1, "Indochina Water", 8, 8, 8, 11);
  ...
  for (i = 1; < cNumberPlayers)
  {
	...
	rmAddAreaTerrainLayer(id, baseTerrain);
  ...
  rmSetObjectDefMinDistance(startingTCID, 0); //5
	rmSetObjectDefMaxDistance(startingTCID, 2); //10
...
if (rmGetPlayerCiv(i) ==  rmGetCivID("DEEthiopians") || rmGetPlayerCiv(i) ==  rmGetCivID("DEHausa"))
  ...
  for (i = 0; < 30)
  {
    ...
    rmAddAreaTerrainLayer(patch, patchTerrain, 0, 1);
  ...
  // Forests
	int numTries=4+10*cNumberNonGaiaPlayers; 
	int failCount=0;
  for (i = 0; < numTries)
  {
 ...
 // Silver
  ...
  else
  {
    ...
    //beach mines
    rmPlaceObjectDefAtLoc(topMine, 0, 0.9, 0.6, 1);
    rmPlaceObjectDefAtLoc(topMine, 0, 0.7, 0.4, 1);
    rmPlaceObjectDefAtLoc(topMine, 0, 0.1, 0.6, 1);
    rmPlaceObjectDefAtLoc(topMine, 0, 0.3, 0.4, 1);
    //central water mines
    rmPlaceObjectDefAtLoc(topMine, 0, 0.3, 0.85, 1);
    rmPlaceObjectDefAtLoc(topMine, 0, 0.38, 0.6, 1);
    rmPlaceObjectDefAtLoc(topMine, 0, 0.7, 0.85, 1);
    rmPlaceObjectDefAtLoc(topMine, 0, 0.62, 0.6, 1);
  }
  ...
  if (cNumberNonGaiaPlayers > 2)
  {
    ...
    rmPlaceObjectDefAtLoc(food2ID, 0, 0.5, 0.5, 4*cNumberNonGaiaPlayers);
  }
  else
  {
    //1v1 hunts
    ...
    rmSetObjectDefMaxDistance(mapHunts, 16);
    ...
    rmAddObjectDefConstraint(mapHunts, avoidWater5);  
    ...
    //left side
        //elifents
    rmPlaceObjectDefAtLoc(mapElephants, 0, 0.2, 0.5, 1);
    rmPlaceObjectDefAtLoc(mapElephants, 0, 0.35, 0.3, 1);
        //serow
    rmPlaceObjectDefAtLoc(mapHunts, 0, 0.37, 0.64, 1);
    rmPlaceObjectDefAtLoc(mapHunts, 0, 0.39, 0.88, 1);
    //right side
        //elifents
    rmPlaceObjectDefAtLoc(mapElephants, 0, 0.8, 0.5, 1);
    rmPlaceObjectDefAtLoc(mapElephants, 0, 0.65, 0.3, 1);
        //serow
    rmPlaceObjectDefAtLoc(mapHunts, 0, 0.63, 0.64, 1);
    rmPlaceObjectDefAtLoc(mapHunts, 0, 0.61, 0.88, 1);
  }
	// Text
  ...
  if (cNumberNonGaiaPlayers < 5)
  {
    ...
    rmPlaceObjectDefAtLoc(fish3ID, 0, 0.5, 0.1, 4+5*cNumberNonGaiaPlayers);
  }  
