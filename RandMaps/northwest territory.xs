...
//   int longPlayerConstraint=rmCreateClassDistanceConstraint("land stays away from players", classPlayer, 24.0);
   ...
   //  MAIN RIVER
	...
	if ( cNumberTeams == 2 )
	{
		...
		rmRiverSetShallowRadius(columbiaRiver, 7+cNumberNonGaiaPlayers);
		...
		rmRiverSetShallowRadius(riverBranchNorth, 7+cNumberNonGaiaPlayers);
		...
		rmRiverSetShallowRadius(riverBranchSouth, 7+cNumberNonGaiaPlayers);
   ...
   if ( cNumberTeams == 2 )
   {
		...
		{
			rmPlacePlayersLine(0.30, 0.40, 0.2, 0.2, 0.0, 0.2);
		}
		...
		{
			rmPlacePlayersLine(0.60, 0.70, 0.8, 0.8, 0.0, 0.2);
		}
...
//1v1 hunts
       ...
       //east bot->top
        ...
        //top->bot
        rmPlaceObjectDefAtLoc(mapElk, 0, 0.15, 0.30, 1);
        rmPlaceObjectDefAtLoc(mapElk, 0, 0.70, 0.85, 1);
            //
	...
	rmPlaceObjectDefAtLoc(fishID, 0, 0.1, 0.9, 5*cNumberNonGaiaPlayers);
