...
//   		whichNative = 6;	// for testing
   ...
   // Nature avoidance
	...
	int avoidForestFar=rmCreateClassDistanceConstraint("forest avoids forest far", rmClassID("classForest"), 52.0);
	int avoidturkey=rmCreateTypeDistanceConstraint("avoids turkey", "turkey", 55.0);
	int avoiddeer=rmCreateTypeDistanceConstraint("deer avoids deer", "deer", 55.0);
	int avoiddeerFar=rmCreateTypeDistanceConstraint("deer avoids deer Far", "deer", 75.0);
   ...
   int avoidSocket=rmCreateClassDistanceConstraint("stuff avoids sockets", rmClassID("socketClass"), 8.0);
	int avoidNugget=rmCreateTypeDistanceConstraint("nugget avoid nugget", "AbstractNugget", 144.0);
	int avoidNuggetShort=rmCreateTypeDistanceConstraint("nugget avoid nugget short", "AbstractNugget", 64.0);
   ...
   // Place Natives
	  ...
	  else
      {
		  if (cNumberNonGaiaPlayers == 2) {
         	...
         	rmPlaceGroupingAtLoc(nativeVillageA, 0, 0.47, 0.27);
         	rmPlaceGroupingAtLoc(nativeVillageA, 0, 0.27, 0.47);
		  	}
      }
      ...
      else
      {
		  if (cNumberNonGaiaPlayers == 2) {
         	...
         	rmPlaceGroupingAtLoc(nativeVillageB, 0, 0.73, 0.53);
         	rmPlaceGroupingAtLoc(nativeVillageB, 0, 0.53, 0.73);
		  	}
      }
...
//		{
			rmSetAreaSize(smallMesaID, rmAreaTilesToFraction(200+100*cNumberNonGaiaPlayers), rmAreaTilesToFraction(200+100*cNumberNonGaiaPlayers));  //First # is minimum square meters of material it will use to build.  Second # is maximum.  Currently I have them both set to the same because I want a certain size mountain every time.
//		}
...
//		if ( cNumberNonGaiaPlayers < 6 )
//		{
			rmSetAreaSize(smallMesaID2, rmAreaTilesToFraction(200+100*cNumberNonGaiaPlayers), rmAreaTilesToFraction(200+100*cNumberNonGaiaPlayers));  //First # is minimum square meters of material it will use to build.  Second # is maximum.  Currently I have them both set to the same because I want a certain size mountain every time.
//		}
//		else if ( cNumberNonGaiaPlayers < 8 )
//		{
...
//		if ( cNumberNonGaiaPlayers < 6 )
//		{
			rmSetAreaSize(smallMesaID3, rmAreaTilesToFraction(200+100*cNumberNonGaiaPlayers), rmAreaTilesToFraction(200+100*cNumberNonGaiaPlayers));  //First # is minimum square meters of material it will use to build.  Second # is maximum.  Currently I have them both set to the same because I want a certain size mountain every time.
//		}
//		else if ( cNumberNonGaiaPlayers < 8 )
//		{
...
// new tp to adjust for KOTH and FFA spawn fixes - modified from Mexico
  ...
  //constraints for durokan's 1v1 balance
	...
	int treesNearEdge = rmCreatePieConstraint("trees stay near edge",0.5,0.5,rmXFractionToMeters(0.47+0.00033*cNumberNonGaiaPlayers), rmXFractionToMeters(0.49), rmDegreesToRadians(0),rmDegreesToRadians(360));
...
//	rmAddObjectDefConstraint(rimTrees, avoidAll);
	...
	if (rmGetIsTreaty() == true)
		rmPlaceObjectDefAtLoc(rimTrees, 0, 0.5, 0.5, 500+150*cNumberNonGaiaPlayers);
...
//1v1 mines
	...
	for (i=0; <numTries)
		{   
			...
			rmAddAreaConstraint(forestID, avoidSocket2Far);
			if(cNumberNonGaiaPlayers==2){
...
//1v1 hunts
	...
	if (rmGetIsTreaty() == true)
	    rmAddObjectDefItem(mapElk, "deer", rmRandInt(8,8), 3.0);
    else
...
int huntVariation = rmRandInt(1,2);
	...
	if(rmRandFloat(0,1) < 0.50 && cNumberNonGaiaPlayers > 2) //only places more hard nuggets 50% of the time
		{
		...
		rmSetObjectDefMaxDistance(nuggethard2ID, rmXFractionToMeters(0.35));
	...
	else if (rmRandFloat(0,1) < 0.25 && cNumberNonGaiaPlayers > 2)  //only try to place nuts 25% of the time
		{
	...
	int nuggethardID= rmCreateObjectDef("nugget hard"); 
		...
		rmSetObjectDefMaxDistance(nuggethardID, rmXFractionToMeters(0.35));
	...
	rmAddObjectDefConstraint(nuggetmediumEastID, avoidKOTH);
	rmPlaceObjectDefAtLoc(nuggetmediumEastID, 0, 0.5, 0.5, cNumberNonGaiaPlayers);
	...
	rmAddObjectDefConstraint(nuggetmediumWestID, avoidKOTH);
	rmPlaceObjectDefAtLoc(nuggetmediumWestID, 0, 0.5, 0.5, cNumberNonGaiaPlayers);
	...
	rmSetObjectDefMaxDistance(nuggeteasyID, rmXFractionToMeters(0.5));
  	rmAddObjectDefConstraint(nuggeteasyID, avoidNuggetShort);
	...
	rmAddObjectDefConstraint(nuggeteasyID, avoidKOTH);
	rmPlaceObjectDefAtLoc(nuggeteasyID, 0, 0.5, 0.5, cNumberNonGaiaPlayers);
