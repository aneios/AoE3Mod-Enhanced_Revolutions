...
void main(void)
{
   ...
   rmTerrainInitialize("water");
	...
	rmSetLightingSet("Pymouth_Skirmish");
	rmSetMapType("grass");
   ...
   int avoidCoin = rmCreateTypeDistanceConstraint("coin avoids coin", "gold", 64);
   ...
   // Avoid impassable land
	...
	int wallConstraint = rmCreateClassDistanceConstraint("wall vs. wall", rmClassID("classWall"), 80.0);
   ...
   rmSetAreaSmoothDistance(bigContinentID, 25);
	rmSetAreaMix(bigContinentID, "plymouth_grass");
   ...
   rmSetAreaSmoothDistance(smallContinent1ID, 25);
	rmSetAreaMix(smallContinent1ID, "plymouth_grass");
   ...
   rmSetAreaSmoothDistance(smallContinent2ID, 25);
	rmSetAreaMix(smallContinent2ID, "plymouth_grass");
...
/*	   for (i = 0; < (cNumberPlayers - 1))
   ...
   rmSetAreaMix(bonusIslandID1, "plymouth_grass");
   ...
   if (cNumberTeams == 2)
      ...
      rmPlaceGroupingAtLoc(huronVillageAID, 0, 0.5, 0.65);
   ...
   if (cNumberTeams == 2)
      ...
      rmPlaceGroupingAtLoc(huronVillageCID, 0, 0.5, 0.55);
   ...
   rmSetObjectDefMaxDistance(playerNuggetID, 25.0);
	...
	for(i = 1; <cNumberPlayers)
	{
      ...
      // Placing starting berries
		...
		// Nuggets
		rmSetNuggetDifficulty(111, 111);	// new house nugget
   ...
   rmSetStatusText("",0.60);
	...
	for(i = 0; <numTries)
	{
      ...
      // Avoid other cliffs, please!
		...
		rmAddAreaConstraint(cliffID, avoidNuggetSmall);
   ...
   rmPlaceObjectDefAtLoc(fishID, 0, 0.5, 0.0, 4+cNumberNonGaiaPlayers*5);
   ...
   // Place resources that we want forests to avoid
	...
	// FORESTS
   int forestTreeID = 0;
   numTries = 6*cNumberNonGaiaPlayers+2;
   failCount = 0;
