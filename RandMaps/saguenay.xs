//Saguenay for DE, was hudson bay.
// was HUDSON BAY (1V1, TEAM, FFA)
// designed by Garja
...
//	rmSetMapElevationHeightBlend(1);
    ...
    //new for DE
    randomfrozen = rmRandFloat(0.1,1.5); // 0.1-0.5 frozen
...
//	randomfrozen = 0.2; //<----- TEST
	...
	rmSetLightingSet("Saguenay_Winter_Skirmish"); // rockies
...
//	int avoidTownCenterMed=rmCreateTypeDistanceConstraint("avoid Town Center med", "townCenter", 40.0);
	...
	int avoidWhale=rmCreateTypeDistanceConstraint("avoid whale", "fish", 40-PlayerNum);
				...
				if (teamZeroCount == 1 || teamOneCount == 1) // one team is one player
				{
					if (teamZeroCount < teamOneCount) // 1v2, 1v3, 1v4, etc.
					{
						...
						if (teamOneCount == 2)
							...
							rmSetPlacementSection(0.350, 0.540); //
						rmSetTeamSpacingModifier(0.25);
						rmPlacePlayersCircular(0.38, 0.38, 0);
					}
					else // 2v1, 3v1, 4v1, etc.
					{
						...
						if (teamZeroCount == 2)
							...
							rmSetPlacementSection(0.710, 0.900); //
						rmSetTeamSpacingModifier(0.25);
						rmPlacePlayersCircular(0.38, 0.38, 0);
	...
	if (randomfrozen >= 0.5)
	{
		...
		rmSetAreaSize(HudsonBayWaterID, 0.20);
	...
	else
	{
		...
		rmSetAreaSize(HudsonBayWaterIceID, 0.15);
	...
	//  rmSetAreaBaseHeight(HudsonBayWaterIceID, 3.0);
		rmSetAreaCoherence(HudsonBayWaterIceID, 0.60);
		...
		int stayNearBayWaterIce = rmCreateAreaMaxDistanceConstraint("stay near bay water ice", HudsonBayWaterIceID, 18);
...
//		rmSetAreaTerrainType(IceareaID, "great_lakes\ground_ice1_gl");
		rmSetAreaCoherence(IceareaID, 0.69);
	...
	if (randomfrozen >= 0.5)
	{
		...
		rmSetAreaSize(IslandgrassID, rmAreaTilesToFraction(60+20*PlayerNum));
		rmSetAreaLocation(IslandgrassID, rmRandFloat(0.63,0.67), rmRandFloat(0.80, 0.86));
		rmSetAreaMix(IslandgrassID, "saguenay tundra");
		rmSetAreaBaseHeight(IslandgrassID, 1.5);
		rmSetAreaCoherence(IslandgrassID, 0.555);
		rmAddAreaToClass(IslandgrassID, classIsland);
		rmAddAreaConstraint(IslandgrassID, avoidLand);
		...
		rmSetAreaSize(Islandgrass2ID, rmAreaTilesToFraction(60+20*PlayerNum));
		rmSetAreaLocation(Islandgrass2ID, rmRandFloat(0.80,0.86), rmRandFloat(0.63, 0.67));
		rmSetAreaMix(Islandgrass2ID, "saguenay tundra");
		rmSetAreaBaseHeight(Islandgrass2ID, 1.5);
		rmSetAreaCoherence(Islandgrass2ID, 0.555);
		rmAddAreaToClass(Islandgrass2ID, classIsland);
		rmAddAreaConstraint(Islandgrass2ID, avoidLand);
		...
		rmSetAreaSize(Islandgrass3ID, rmAreaTilesToFraction(60+20*PlayerNum));
	...
	//	rmSetAreaElevationVariation(Islandgrass3ID, 0.5);
		rmSetAreaCoherence(Islandgrass3ID, 0.555);
		...
		rmAddAreaConstraint(Islandgrass3ID, avoidLand);
		if (PlayerNum >= 6)
...
//	TPvariation = 0.6; // <--- TEST
	if (TeamNum >= 3 || teamZeroCount != teamOneCount)
		TPvariation = 2.0;
	...
	if (TPvariation <= 0.5)
	{
		...
		if (randomfrozen >= 0.5)
		{
			...
			rmAddTradeRouteWaypoint(tradeRoute2ID, rmRandFloat(0.15,0.15), rmRandFloat(0.15,0.15));
			rmAddTradeRouteWaypoint(tradeRoute2ID, rmRandFloat(0.18,0.21), rmRandFloat(0.18,0.21));
			rmAddTradeRouteWaypoint(tradeRoute2ID, rmRandFloat(0.29,0.32), rmRandFloat(0.29,0.32));
			rmAddTradeRouteWaypoint(tradeRoute2ID, rmRandFloat(0.38,0.41), rmRandFloat(0.38,0.41));
		...
		else
		{
			rmSetObjectDefTradeRouteID(socket2ID, tradeRoute2ID);
			rmAddTradeRouteWaypoint(tradeRoute2ID, rmRandFloat(0.15,0.15), rmRandFloat(0.15,0.15));
			rmAddTradeRouteWaypoint(tradeRoute2ID, rmRandFloat(0.23,0.26), rmRandFloat(0.23,0.26));
			rmAddTradeRouteWaypoint(tradeRoute2ID, rmRandFloat(0.32,0.34), rmRandFloat(0.32,0.34));
			rmAddTradeRouteWaypoint(tradeRoute2ID, rmRandFloat(0.43,0.46), rmRandFloat(0.43,0.46));
	...
	else if (TPvariation <= 1.0)
	{
		...
		rmAddTradeRouteWaypoint(tradeRouteID, 0.50, 0.32);
		rmAddTradeRouteWaypoint(tradeRouteID, rmRandFloat(0.38,0.42), rmRandFloat(0.38,0.42));
		rmAddTradeRouteWaypoint(tradeRouteID, 0.32, 0.50);
		...
		rmAddTradeRouteWaypoint(tradeRouteID, 0.01, rmRandFloat(0.51,0.53));
			...
			rmAddTradeRouteWaypoint(tradeRouteID, 0.50, 0.01);
			rmAddTradeRouteWaypoint(tradeRouteID, 0.52, 0.10);
			rmAddTradeRouteWaypoint(tradeRouteID, 0.54, 0.36);
			rmAddTradeRouteWaypoint(tradeRouteID, 0.445, 0.445);
			rmAddTradeRouteWaypoint(tradeRouteID, 0.36, 0.54);
			rmAddTradeRouteWaypoint(tradeRouteID, 0.10, 0.52);
			rmAddTradeRouteWaypoint(tradeRouteID, 0.01, 0.50);
			}
	...
	if (rmGetIsKOTH()) {
        ...
        int avoidCenter_dk = rmCreateClassDistanceConstraint("avoids the center for KOTH _dk", classCenter_dk, 5);
...
//	rmAddGroupingConstraint(nativeID0, avoidNatives);
	rmPlaceGroupingAtLoc(nativeID0, 0, 0.35, 0.85);
...
//	rmAddGroupingConstraint(nativeID2, avoidNatives);
	rmPlaceGroupingAtLoc(nativeID2, 0, 0.85, 0.35);
...
//	rmAddObjectDefConstraint(playermooseID, avoidStartingResources);
	...
	if (randomfrozen >= 0.5)
		rmAddObjectDefItem(player2ndherdID, "elk", rmRandInt(9,9), 5.0);
	else
		rmAddObjectDefItem(player2ndherdID, "caribou", rmRandInt(10,10), 5.0);
...
//	rmAddObjectDefConstraint(player2ndherdID, avoidTradeRouteShort);
	...
	if (randomfrozen >= 0.5)
		rmAddObjectDefItem(player3rdherdID, "moose", rmRandInt(5,5), 5.0);
	else
		rmAddObjectDefItem(player3rdherdID, "muskOx", rmRandInt(6,6), 5.0);
...
//	rmAddObjectDefConstraint(player3rdherdID, avoidTradeRouteShort);
	...
	if (PlayerNum > 2)
		rmAddObjectDefConstraint(player3rdherdID, avoidMidIsland);
	if (teamZeroCount != teamOneCount)
		rmAddObjectDefConstraint(player3rdherdID, stayNearEdge);
...
//		rmPlaceObjectDefAtLoc(playergold3ID, i, rmXMetersToFraction(xsVectorGetX(TCLoc)), rmZMetersToFraction(xsVectorGetZ(TCLoc)));
	...
	// ************* Mines **************
		...
		rmSetObjectDefMaxDistance(extraGoldID, rmXFractionToMeters(0.05));
		...
		rmAddObjectDefConstraint(extraGoldID, avoidStartingResources);
		rmAddObjectDefConstraint(extraGoldID, avoidEdge);
		if (randomfrozen < 0.5)
			rmAddObjectDefConstraint(extraGoldID, avoidIceAreaFar);
		else
		{
			rmAddObjectDefConstraint(extraGoldID, avoidIsland1);
			rmAddObjectDefConstraint(extraGoldID, avoidIsland2);
			rmAddObjectDefConstraint(extraGoldID, avoidIsland3);
		}
		if (PlayerNum == 2) {
			rmPlaceObjectDefAtLoc(extraGoldID, 0, 0.45, 0.95);
			rmPlaceObjectDefAtLoc(extraGoldID, 0, 0.95, 0.45);
			}

		int extraGoldCount = PlayerNum-2;
		int goldCount = 2+2*PlayerNum+extraGoldCount;  // 3,3
//		int northgoldCount = rmRandInt(3,3);  // 3,3
	...
	for(i=0; < goldCount)
	{
		...
		rmAddObjectDefConstraint(commongoldID, avoidStartingResources);
		rmAddObjectDefConstraint(commongoldID, avoidTownCenterFar);
//		rmAddObjectDefConstraint(commongoldID, avoidEdge);
		rmAddObjectDefConstraint(commongoldID, avoidCenter_dk);
		if (randomfrozen < 0.5)
...
//		rmSetAreaObeyWorldCircleConstraint(forestID, false);
		...
		rmAddAreaConstraint(forestID, avoidCenter_dk);
//		rmAddAreaConstraint(forestID, avoidEdge);
	...
	for (i=0; < randomtreecount)
	{
		...
		rmAddObjectDefConstraint(randomtreenorthID, avoidCenter_dk);
		if (randomfrozen >= 0.5)
		{
...
//		rmAddObjectDefConstraint(northerdID, avoidTradeRouteSocket);
		...
		rmAddObjectDefConstraint(northerdID, avoidStartingResources);
...
//		rmAddObjectDefConstraint(northerdID, avoidEdge);
		rmAddObjectDefConstraint(northerdID, avoidCenter_dk);
		if (randomfrozen >= 0.5)
		{
	...
	for (i=0; < middleherdcount)
	{
		...
		rmAddObjectDefConstraint(middleherdID, avoidStartingResources);
		...
		rmAddObjectDefConstraint(middleherdID, avoidCenter_dk);
		if (randomfrozen >= 0.5)
...
//		rmAddObjectDefConstraint(southherdID, avoidEdge);
		rmAddObjectDefConstraint(southherdID, avoidCenter_dk);
		if (randomfrozen >= 0.5)
		{
...
//		rmAddObjectDefConstraint(Nugget4ID, avoidEdge);
		...
		if (PlayerNum >= 6 && rmGetIsTreaty() == false)
...
//		rmAddObjectDefConstraint(Nugget2ID, avoidEdge);
		rmAddObjectDefConstraint(Nugget2ID, avoidCenter_dk);
		if (randomfrozen >= 0.5)
		{
...
//			rmAddObjectDefConstraint(sheepID, avoidEdge);
			rmAddObjectDefConstraint(sheepID, avoidCenter_dk);
...
//	fishcount = 3+3*PlayerNum;
	int whalecount = 2+1*PlayerNum;
		...
		if (PlayerNum == 2)
		{
			rmSetObjectDefMinDistance(whaleID, 0.00);
			rmSetObjectDefMaxDistance(whaleID, rmXFractionToMeters(0.066));
		}
		else
		{
			rmSetObjectDefMinDistance(whaleID, 0.025);
		...
		if (PlayerNum > 2)
			rmPlaceObjectDefAtLoc(whaleID, 0, 0.7, 0.7, whalecount);
		else
		{
			rmPlaceObjectDefAtLoc(whaleID, 0, 0.55, 0.75);
			rmPlaceObjectDefAtLoc(whaleID, 0, 0.75, 0.55);
			rmPlaceObjectDefAtLoc(whaleID, 0, 0.7, 0.9);
			rmPlaceObjectDefAtLoc(whaleID, 0, 0.9, 0.7);
		}
	...
	for (i=0; < fishcount)
	{
		...
		if (randomfrozen < 0.5)
			rmAddObjectDefConstraint(fishID, avoidIceAreaFar);
		rmPlaceObjectDefAtLoc(fishID, 0, 0.7, 0.7);
	}
