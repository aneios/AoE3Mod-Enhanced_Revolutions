...
//		whichFeature = 1;	// for testing
	...
	rmSetAreaSize(bisonCanyonID, rmAreaTilesToFraction(800+50*PlayerNum), rmAreaTilesToFraction(800+50*PlayerNum)); 
	...
	rmSetAreaCliffEdge(bisonCanyonID, 1, 1.00, 0.0, 1.0, 0);
	...
	rmAddObjectDefItem(canyonBisonID, "bison", 7, 4.0);
	...
	rmSetAreaSize(bigCanyonID, rmAreaTilesToFraction(555+10*PlayerNum)); 
...
//	rmAddAreaInfluenceSegment(bigCanyonID, 0.65, 0.50, 0.60, 0.60);
	...
	rmSetAreaSize(mountain4ID, rmAreaTilesToFraction(40));
	...
	rmSetAreaSize(bigMesaID, rmAreaTilesToFraction(350)); 
...
//		rmSetAreaTerrainType(mesaID, "sonora\ground1_son");  
	...
	for (i=0; <5*PlayerNum)
	{
		...
		rmSetAreaSize(smallmesaID, rmAreaTilesToFraction(11), rmAreaTilesToFraction(22));		
		...
		rmSetAreaCoherence(smallmesaID, 0.666);
...
//		rmSetAreaTerrainType(smallmesaID, "sonora\ground1_son");  
		...
		rmAddAreaConstraint(smallmesaID, avoidCliffMed);
	...
	rmAddObjectDefItem(treesID, "TreeSonora", 5, 4.0);
	...
	rmAddObjectDefConstraint(treesID, avoidForestShort);
...
//	rmAddObjectDefConstraint(treesID, avoidBigMesaFar);
	rmPlaceObjectDefAtLoc(treesID, 0, 0.50, 0.50, 10+10*cNumberNonGaiaPlayers);
...
//	rmAddObjectDefConstraint(randTreesID, avoidBigMesaFar);
	rmPlaceObjectDefAtLoc(randTreesID, 0, 0.50, 0.50, 3*cNumberNonGaiaPlayers);
	...
	int mapBisonID = -1;
		...
		rmAddObjectDefConstraint(mapBisonID, avoidEdgeMore);
		if (PlayerNum > 2) {
	...
	for (i=0; < treasure34count)
	{
		...
		if (PlayerNum < 5 || rmGetIsTreaty() == true)
