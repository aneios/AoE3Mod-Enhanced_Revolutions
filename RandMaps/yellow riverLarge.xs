...
// Vary some terrain
  for (i=0; < 30) {
   ...
   for (i=0; <10) {
    ...
    rmSetAreaMix(dirtPatch, patchMix);
    rmSetAreaTerrainType(dirtPatch, patchType1);
    rmAddAreaTerrainLayer(dirtPatch, patchType2, 0, 1);
    rmAddAreaToClass(dirtPatch, rmClassID("patch"));
  ...
  int startingTCID= rmCreateObjectDef("startingTC");
	...
	rmSetObjectDefMaxDistance(startingTCID, 2);//TCfloat
...
// Lumber
  ...
  for (i=0; <numTries)	{   
    ...
    rmSetAreaForestUnderbrush(forestID2, 0.4);
  ...
  for (i=0; <numTries)	{   
    ...
    rmSetAreaForestUnderbrush(forestID3, 0.4);
  ...
  if (rmRandFloat(0,1) > .5 && rmGetIsTreaty() == false) {
