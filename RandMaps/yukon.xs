...
// April 2021 update by Vividlyplain
...
//		whichNative = 1;	// for testing
   ...
   int circleConstraint=rmCreatePieConstraint("circle Constraint", 0.5, 0.5, 0, rmZFractionToMeters(0.47), rmDegreesToRadians(0), rmDegreesToRadians(360));
	...
	if (cNumberNonGaiaPlayers < 7)
	{
        ...
        coinAvoidsCoin2=rmCreateTypeDistanceConstraint("coin avoids coin2", "gold", 60.0);
	}
	else
	{
		coinAvoidsCoin=rmCreateTypeDistanceConstraint("coin avoids coin", "gold", 40.0);
        coinAvoidsCoin2=rmCreateTypeDistanceConstraint("coin avoids coin2", "gold", 55.0);
...
//	rmAddObjectDefConstraint(socketID, avoidImpassableLand);
	...
	if(whichMap > 1)
	{
		...
		if(whichMap > 2) // NS river
		{
			...
			rmRiverAddShallow(riverID, rmRandFloat(0.1, 0.15));
			rmRiverAddShallow(riverID, rmRandFloat(0.45, 0.55));
			rmRiverAddShallow(riverID, rmRandFloat(0.85, 0.9));
	...
	*/
	...
	for (i=0; <numTries)
		{   
			...
			rmSetAreaForestDensity(forestID, .69);
	...
	for (i=0; <numTries)
		{   
			...
			rmSetAreaForestDensity(snowforestID, .69);
