// LARGE UPPER ANDES - river removed, player positions improved, resource balance improved, trade route position altered slightly modified by Rikikipu - September 2016
...
void main(void)
{
	...
	// Nature avoidance
	int avoidForestShort = rmCreateClassDistanceConstraint("forest avoids forest short", rmClassID("classForest"), 15.0);
	int avoidForest = rmCreateClassDistanceConstraint("forest avoids forest", rmClassID("classForest"), 36.0);
	...
	int avoidGuanacoFar = rmCreateTypeDistanceConstraint("guanaco avoids guanaco far", "Guanaco", 60.0);
	...
	if (cNumberNonGaiaPlayers < 4)
		...
		avoidCoinFar = rmCreateTypeDistanceConstraint("avoid coin far", "Mine", 44.0);
	int avoidClearing = rmCreateClassDistanceConstraint("avoid clearings", rmClassID("classClearing"), 8.0);

	int avoidGold = rmCreateClassDistanceConstraint("avoid gold", rmClassID("classGold"), 40.0);
	...
	int avoidGoldFar = rmCreateClassDistanceConstraint("avoid gold far", rmClassID("classGold"), 60.0);
...
//	rmSetAreaMix(midSmIslandID, "great plains drygrass"); 	// for testing
	...
	rmSetAreaSize(cliffIDA1, rmAreaTilesToFraction(275*cNumberNonGaiaPlayers), rmAreaTilesToFraction(275*cNumberNonGaiaPlayers));	// 0.01
	...
	rmSetAreaSize(cliffIDA2, rmAreaTilesToFraction(125*cNumberNonGaiaPlayers), rmAreaTilesToFraction(125*cNumberNonGaiaPlayers));		// 0.005
	...
	rmSetAreaSize(cliffIDA3, rmAreaTilesToFraction(333*cNumberNonGaiaPlayers), rmAreaTilesToFraction(333*cNumberNonGaiaPlayers));		// 0.015
	if (cNumberNonGaiaPlayers == 2) {
	...
	rmSetAreaSize(cliffIDB1, rmAreaTilesToFraction(275*cNumberNonGaiaPlayers), rmAreaTilesToFraction(275*cNumberNonGaiaPlayers));	// 0.01
	...
	rmSetAreaSize(cliffIDB2, rmAreaTilesToFraction(125*cNumberNonGaiaPlayers), rmAreaTilesToFraction(125*cNumberNonGaiaPlayers));		// 0.005
	...
	rmSetAreaSize(cliffIDB3, rmAreaTilesToFraction(333*cNumberNonGaiaPlayers), rmAreaTilesToFraction(333*cNumberNonGaiaPlayers));		// 0.015
	if (cNumberNonGaiaPlayers == 2) {
	...
	rmAddObjectDefItem(playerTree2ID, "TreeAndes", 20, 10.0);
...
/*	// LOL what are these and who ever thought this was a good idea?!
...
*/
	...
	if (cNumberNonGaiaPlayers == 2)
		rmSetObjectDefMaxDistance(guanacoID, rmXFractionToMeters(0.15));
	else if (cNumberTeams > 2)
	...
	if (cNumberNonGaiaPlayers == 2)
		rmSetObjectDefMaxDistance(guanacoSEID, rmXFractionToMeters(0.15));
	else if (cNumberTeams > 2)
	...
	if (cNumberNonGaiaPlayers > 2)
		rmPlaceObjectDefAtLoc(nuggethardID, 0, 0.00, 0.50, cNumberNonGaiaPlayers);
