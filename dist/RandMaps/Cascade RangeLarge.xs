...
// April 2021 edited by vividlyplain for DE, June 2021
...
//	rmSetBaseTerrainMix("rockies_grass"); // nwt_grass1	// greatlakes_snow
	...
	int avoidElkFar = rmCreateTypeDistanceConstraint("avoid elk far", "elk", 72.0+PlayerNum);
	...
	int avoidNuggetShort = rmCreateTypeDistanceConstraint("nugget avoid nugget short", "AbstractNugget", 60.0);
...
//		rmSetAreaTerrainType(valleyforestID, "pampas\groundforest_pam");
	...
	rmAddObjectDefItem(fishID, "Fishsalmon", rmRandInt(2,2), 12.0);
	...
	if (TeamNum <= 2)
	{
		rmPlaceObjectDefAtLoc(fishID, 0, 1.00, 0.70, 3+PlayerNum);
		rmPlaceObjectDefAtLoc(fishID, 0, 0.00, 0.30, 3+PlayerNum);
	}
	else
	{
		rmPlaceObjectDefAtLoc(fishID, 0, 1.00, 0.50, 3+PlayerNum);
		rmPlaceObjectDefAtLoc(fishID, 0, 1.00, 0.50, 3+PlayerNum);
	}
