// Guianas (formerly vivid's Pantanal Marsh and the OG vivid's Empire Wars Marsh)
...
void main(void)
{
	...
	// Resource avoidance
	int avoidForestVeryFar=rmCreateClassDistanceConstraint("avoid forest very far", rmClassID("Forest"), 32.0);
	int avoidForestFar=rmCreateClassDistanceConstraint("avoid forest far", rmClassID("Forest"), 24.0);
	...
	int avoidHunt1Far = rmCreateTypeDistanceConstraint("avoid hunt1 far", food1, 40.0);
	...
	int avoidGoldVeryFar = rmCreateClassDistanceConstraint ("gold avoid gold very far", rmClassID("Gold"), 70.0);
	...
	int avoidCliffShort = rmCreateClassDistanceConstraint("avoid cliff short", rmClassID("Cliffs"), 6.0);
	int avoidCliff = rmCreateClassDistanceConstraint("avoid cliff", rmClassID("Cliffs"), 8.0);
	int avoidCliffMed = rmCreateClassDistanceConstraint("avoid cliff medium", rmClassID("Cliffs"), 16);
	int avoidCliffFar = rmCreateClassDistanceConstraint("avoid cliff far", rmClassID("Cliffs"), 32);
	int avoidCliffFarther = rmCreateClassDistanceConstraint("avoid cliff farther", rmClassID("Cliffs"), 36+3*PlayerNum);
	int avoidCliffVeryFar = rmCreateClassDistanceConstraint("avoid cliff very far", rmClassID("Cliffs"), 64.0);
		...
		if (cNumberTeams <= 2) // 1v1 and TEAM
		{
			...
			else if (teamZeroCount == teamOneCount) // equal N of players per TEAM
			{
				if (teamZeroCount == 2) // 2v2
				{
					...
					rmSetTeamSpacingModifier(0.25);
					if (rmGetIsTreaty() == true)
						rmPlacePlayersCircular(0.34, 0.34, 0.02);
					else
...
//	rmSetAreaMix(midBigIslandID, forTesting);
	...
	int avoidBigMidIslandMin = rmCreateAreaDistanceConstraint("avoid big mid island min", midBigIslandID, 4.0);
...
//	rmSetAreaMix(midLineTRID, forTesting);
	...
	int avoidTRMidLineShort = rmCreateAreaDistanceConstraint("avoid treaty mid line short", midLineTRID, 4.0);
	...
	int stayNearTRMidLine = rmCreateAreaMaxDistanceConstraint("stay near treaty mid line ", midLineTRID, 30.0);
...
//	rmSetAreaBaseHeight(natIsland4ID, 2.0); 
	...
	if (TeamNum <= 2 && teamZeroCount == teamOneCount) {
		...
		if (rmGetIsTreaty() == true) {
			if (PlayerNum <= 4)
		        rmPlaceObjectDefAtLoc(socketID, 0, 0.10, 0.30);
	        else
				rmPlaceObjectDefAtLoc(socketID, 0, 0.125, 0.225);
			}
        else
		...
		if (PlayerNum > 4) {
			...
			if (rmGetIsTreaty() == true)
		        rmPlaceObjectDefAtLoc(socketID, 0, 0.40, 0.15);
			else
				rmPlaceObjectDefAtPoint(socketID, 0, socketLoc1);		
			
			socketLoc1 = rmGetTradeRouteWayPoint(tradeRouteID, 0.65);
			if (rmGetIsTreaty() == true)
		        rmPlaceObjectDefAtLoc(socketID, 0, 0.60, 0.15);
			else
				rmPlaceObjectDefAtPoint(socketID, 0, socketLoc1);		
			}
		...
		socketLoc1 = rmGetTradeRouteWayPoint(tradeRouteID, 0.85);
 		if (rmGetIsTreaty() == true) {
			 if (PlayerNum <= 4)
		        ...
		        rmPlaceObjectDefAtLoc(socketID, 0, 0.875, 0.225);
		 	}
        else
			rmPlaceObjectDefAtPoint(socketID, 0, socketLoc1);
...
//	int cliffcount = 4; 
	...
	for (i= 0; < cliffcount)
	{
		...
		if (TeamNum == 2 && teamZeroCount == teamOneCount && rmGetIsTreaty() == true)
			rmSetAreaSize(cliff1ID, 0.0035-0.0001*PlayerNum);
		else if (TeamNum == 2 && teamZeroCount == teamOneCount && rmGetIsTreaty() == false)
	...
	//	rmSetAreaHeightBlend(cliff1ID, 1);
		...
		if (TeamNum == 2 && teamZeroCount == teamOneCount)
		{
			...
			if (rmGetIsTreaty() == false && PlayerNum == 2)
				rmAddAreaConstraint(cliff1ID, avoidCliffFar);
			else if (rmGetIsTreaty() == false)
				rmAddAreaConstraint(cliff1ID, avoidCliffFarther);
			else 
...
//		rmAddAreaConstraint(cliff1ID, avoidMidLine);
		if (rmGetIsTreaty() == true)
		{
			rmAddAreaConstraint(cliff1ID, avoidIsland);
			if (PlayerNum <= 4)
				rmAddAreaConstraint(cliff1ID, avoidTRMidLine);
			else
			{
				rmAddAreaConstraint(cliff1ID, avoidTRMidLineShort);
				rmAddAreaConstraint(cliff1ID, stayNearTRMidLine);
			}
		}
	...
	for (i= 0; < cliffcount)
	{
		...
		if (TeamNum == 2 && teamZeroCount == teamOneCount && rmGetIsTreaty() == true)
			rmSetAreaSize(cliff2ID, 0.0035-0.0001*PlayerNum);
		else if (TeamNum == 2 && teamZeroCount == teamOneCount && rmGetIsTreaty() == false)
	...
	//	rmSetAreaHeightBlend(cliff2ID, 1);
		...
		if (TeamNum == 2 && teamZeroCount == teamOneCount)
		{
			...
			if (rmGetIsTreaty() == false && PlayerNum == 2)
				...
				rmAddAreaConstraint(cliff2ID, avoidCliffFarther);
			else 
...
//		rmAddAreaConstraint(cliff2ID, avoidMidLine);
		if (rmGetIsTreaty() == true)
		{
			rmAddAreaConstraint(cliff2ID, avoidIsland);
			if (PlayerNum <= 4)
				rmAddAreaConstraint(cliff2ID, avoidTRMidLine);
			else
			{
				rmAddAreaConstraint(cliff2ID, avoidTRMidLineShort);
				rmAddAreaConstraint(cliff2ID, stayNearTRMidLine);
			}
		}
...
//	rmSetAreaMix(GoldZoneSEID, forTesting);	
	...
	if (rmGetIsTreaty() == true)
	{
		rmAddObjectDefItem(playerTreeID, treeType1, 2, 3.0);
		rmAddObjectDefItem(playerTreeID, treeType2, 4, 3.0);
	}
	else
	{
	...
	if (rmGetIsTreaty() == true)
	{
		rmAddObjectDefItem(playerTree2ID, treeType1, 8, 4.0);
		rmAddObjectDefItem(playerTree2ID, treeType2, 8, 4.0);
	}
	else
	{
	...
	rmAddObjectDefConstraint(playerTree2ID, avoidCliffMed);
	if (TeamNum == 2 && teamZeroCount == teamOneCount) {
	...
	int cliff3ID = rmCreateArea("cliff3");
		...
		if (rmGetIsTreaty() == true) {
			...
			rmSetAreaCoherence(cliff3ID, 0.85);
			}
		else {
	...
	int cliff4ID = rmCreateArea("cliff4");
		...
		if (rmGetIsTreaty() == true) {
			...
			rmSetAreaCoherence(cliff4ID, 0.85);
			}
		else {
	...
	int treesNearEdge = rmCreatePieConstraint("trees stay near edge",0.5,0.5,rmXFractionToMeters(0.475+0.0005*cNumberNonGaiaPlayers), rmXFractionToMeters(0.49), rmDegreesToRadians(060),rmDegreesToRadians(300));
	...
	rmAddObjectDefConstraint(rimTrees, avoidGoldTypeShort);
	rmAddObjectDefConstraint(rimTrees, avoidNativesShort);
	rmAddObjectDefConstraint(rimTrees, treesNearEdge);
	rmAddObjectDefConstraint(rimTrees, avoidTownCenterShort);
	rmAddObjectDefConstraint(rimTrees, avoidStartingResourcesShort);
	rmAddObjectDefConstraint(rimTrees, avoidHunt1Min);
	rmAddObjectDefConstraint(rimTrees, avoidHunt2Min);
	rmAddObjectDefConstraint(rimTrees, avoidTradeRouteMin);
	rmAddObjectDefConstraint(rimTrees, avoidTradeRouteSocket);
	rmAddObjectDefConstraint(rimTrees, avoidGoldMin);
	rmAddObjectDefConstraint(rimTrees, avoidNuggetMin);
	rmAddObjectDefConstraint(rimTrees, avoidIslandMin);
	rmAddObjectDefConstraint(rimTrees, avoidImpassableLandShort);
	rmAddObjectDefConstraint(rimTrees, avoidNatZone);
	rmAddObjectDefConstraint(rimTrees, avoidCliff);
	rmAddObjectDefConstraint(rimTrees, avoidMidLine);
	rmAddObjectDefConstraint(rimTrees, treeWater);
	rmAddObjectDefConstraint(rimTrees, avoidFauxRiver1Min);
	rmAddObjectDefConstraint(rimTrees, avoidFauxRiver2Min);
	if (rmGetIsTreaty() == true && TeamNum == 2 && teamZeroCount == teamOneCount)
		rmPlaceObjectDefAtLoc(rimTrees, 0, 0.5, 0.5, 150+50*cNumberNonGaiaPlayers);

	// Text
	rmSetStatusText("",0.70);

	// ____________________ Forest ____________________
	int mainforestcount = 20+5*PlayerNum;
	if (rmGetIsTreaty() == true)
		mainforestcount = 10+2.5*PlayerNum;
	...
	//	rmSetAreaTerrainType(forestpatchID, forestPaint);
		...
		if (rmGetIsTreaty() == true)
			rmAddAreaConstraint(forestpatchID, avoidTownCenterFar);
		else
		...
		if (rmGetIsTreaty() == true)
		{
			rmAddAreaConstraint(forestpatchID, avoidForestVeryFar);
		...
		rmAddObjectDefItem(foresttreeID, treeType1, 7, 8.0);
		...
		rmAddObjectDefConstraint(foresttreeID, avoidHunt1Min);
		rmAddObjectDefConstraint(foresttreeID, avoidHunt2Min);
		...
		rmAddObjectDefItem(foresttree2ID, treeType1, 7, 8.0);
		rmAddObjectDefItem(foresttree2ID, treeType2, 3, 6.0);
		...
		rmAddObjectDefConstraint(foresttree2ID, avoidGoldTypeShort);
		rmAddObjectDefConstraint(foresttree2ID, avoidImpassableLandShort);
		rmAddObjectDefConstraint(foresttree2ID, avoidHunt1Min);
		rmAddObjectDefConstraint(foresttree2ID, avoidHunt2Min);
	...
	if (rmGetIsTreaty() == false)
		...
		rmAddObjectDefConstraint(randdomTreeID, avoidForest);
	rmAddObjectDefConstraint(randdomTreeID, avoidTownCenter);
	rmAddObjectDefConstraint(randdomTreeID, avoidGoldTypeShort);
	...
	if (rmGetIsTreaty() == true)
	{
		rmAddObjectDefItem(SETreeID, treeType1, 6, 4.0);
		rmAddObjectDefItem(SETreeID, treeType2, 2, 4.0);
	}
	else
	{
	...
	if (rmGetIsTreaty() == true)
	{
		rmAddObjectDefItem(NWTreeID, treeType1, 6, 4.0);
		rmAddObjectDefItem(NWTreeID, treeType2, 2, 4.0);
	}
	else
	{
