...
//	rmSetMapElevationHeightBlend(1);
	...
	rmSetMapType("kamchatka"); //("northwestTerritory");	// himalayas
	rmSetMapType("grass");
	rmSetMapType("land");
//	rmSetLightingSet("nwterritory");
	if (season >= 0.5)
		rmSetLightingSet("Kamchatka_Skirmish"); //was himalayas, changed for DE
	else
		rmSetLightingSet("Kamchatka_Winter_Skirmish"); //great lakes winter
...
//	int avoidForestFar=rmCreateClassDistanceConstraint("forest vs forest", rmClassID("Forest"), 35.0); //15.0
	...
	int avoidCaribou = rmCreateTypeDistanceConstraint("avoid caribou", "caribou", 44.0);
	...
	int avoidMuskdeerFar = rmCreateTypeDistanceConstraint("avoid muskdeer far", "ypmuskdeer", 64.0);
	...
	int avoidElkShort = rmCreateTypeDistanceConstraint("avoid elk short", "elk", 36.0);
	int avoidIbex = rmCreateTypeDistanceConstraint("avoid ibex", "ypibex", 55.0);
...
//	int avoidGoldMed = rmCreateTypeDistanceConstraint("coin avoids coin", "gold", 30.0);
	...
	int avoidGoldFar = rmCreateClassDistanceConstraint ("avoid gold far", rmClassID("Gold"), 80.0);
	int avoidGoldMed = rmCreateClassDistanceConstraint ("avoid gold med", rmClassID("Gold"), 60.0);
	int avoidNuggetMin = rmCreateTypeDistanceConstraint(" avoid nugget min", "AbstractNugget", 4.0);
	int avoidNuggetShort = rmCreateTypeDistanceConstraint(" avoid nugget short", "AbstractNugget", 50.0);
	int avoidNugget=rmCreateTypeDistanceConstraint(" avoid nugget", "AbstractNugget", 68.0);
	...
	int avoidGoat = rmCreateTypeDistanceConstraint("goat avoid goat", "ypGoat", 85.0);
			...
			if (teamZeroCount == 1 && teamOneCount == 1) // 1v1
			{
				...
				if ( OneVOnePlacement < 0.5)
				{
					if (Layoutvariation == 0)
						...
						rmPlacePlayer(1, 0.62, 0.16);
					if (Layoutvariation == 0)
						rmPlacePlayer(2, 0.64, 0.84);
					else
						rmPlacePlayer(2, 0.62, 0.84);
				}
				else
				{
					if (Layoutvariation == 0)
			...
			else // unequal N of players per TEAM
			{
				if (teamZeroCount == 1 || teamOneCount == 1) // one team is one player
				{
					if (teamZeroCount < teamOneCount) // 1v2, 1v3, 1v4, etc.
					{
						...
						{
							if (Layoutvariation == 0)
								rmPlacePlayersLine(0.75, 0.80, 0.30, 0.85, 0.00, 0.25);		// rmPlacePlayersLine(0.72, 0.78, 0.36, 0.84, 0.00, 0.25);
							else
								rmPlacePlayersLine(0.75, 0.80, 0.30, 0.85, 0.00, 0.25);		// rmPlacePlayersLine(0.68, 0.82, 0.32, 0.82, 0.00, 0.25);
						}
					}
					else // 2v1, 3v1, 4v1, etc.
					{
						...
						{
							if (Layoutvariation == 0)
								rmPlacePlayersLine(0.75, 0.20, 0.30, 0.15, 0.00, 0.25);		// rmPlacePlayersLine(0.72, 0.22, 0.36, 0.16, 0.00, 0.25);
							else
								rmPlacePlayersLine(0.75, 0.20, 0.30, 0.15, 0.00, 0.25);		// rmPlacePlayersLine(0.68, 0.18, 0.32, 0.18, 0.00, 0.25);
						}
					}
				}
	...
	{
		//river1
		int riverID1 = rmRiverCreate(-1, "Kamchatka Water", 4, 6, 5+PlayerNum/2, 5+PlayerNum/2); //  (-1, "Northwest Territory Water", 4, 6, 5, 5)
		...
		//river2
		int riverID2 = rmRiverCreate(-1, "Kamchatka Water", 4, 6, 5+PlayerNum/2, 5+PlayerNum/2); //  (-1, "Northwest Territory Water", 4, 6, 5, 5);
		...
		//shallows
		rmRiverSetShallowRadius(riverID1, 8+PlayerNum);
		rmRiverSetShallowRadius(riverID2, 8+PlayerNum);
		if (TeamNum <= 2)
		{
			if (Layoutvariation == 0) 
			{
...
//    }
	...
	rmAddObjectDefItem(playerbighornID, "bighornsheep", 6, 4.0);
	rmSetObjectDefMinDistance(playerbighornID, 16.0);
...
//		rmAddObjectDefConstraint(extragoldID, avoidCenter);
		if (PlayerNum >= 4)
			rmAddObjectDefConstraint(extragoldID, avoidGoldMed);
		else
		...
		rmPlaceObjectDefAtLoc(extragoldID, 0, 0.50, 0.50, extragoldCount);	
...
//		rmAddAreaConstraint(mainforestID, avoidStartingResources);
		...
		if (rmGetIsKOTH() == true)
			rmAddAreaConstraint(mainforestID, avoidKOTH);
		if (season < 0.5)
		{
...
//			rmAddObjectDefToClass(maintreeID, classForest);
	...
	// Caribou herds
	int cariboucount = 2+3*PlayerNum;
...
//	rmAddObjectDefConstraint(elkID, avoidGoldMin);
	...
	if (rmGetIsKOTH() == true)
		rmAddObjectDefConstraint(elkID, avoidKOTH);
	if (season < 0.5)
	{
	...
	if (season < 0.5)
	{
		for (i=0; < 8*PlayerNum)
		{
			...
			if (rmGetIsKOTH() == true)
				rmAddObjectDefConstraint(RivertreeID, avoidKOTH);
			if (i < 8*PlayerNum/2)
	...
	for (i=0; <goatcount)
	{
		...
		rmAddObjectDefConstraint(GoatID, avoidKOTH);
