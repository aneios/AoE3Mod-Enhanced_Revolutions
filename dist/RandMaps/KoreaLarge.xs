// LARGE Korea - adapted from Garja's Adirondacks by Durokan & Interjection for DE
// designed by Garja
...
//	rmEchoInfo("subCiv0 is Jesuit "+subCiv0);
	...
	// Resource avoidance
	int avoidForestFar=rmCreateClassDistanceConstraint("avoid forest far", classForest, 44.0); //15.0
	...
	int avoidDeerFar = rmCreateTypeDistanceConstraint("avoid deer far", "Deer", 55.0);
	int avoidDeer = rmCreateTypeDistanceConstraint("avoid deer", "Deer", 40.0);
	...
	int avoidElkFar = rmCreateTypeDistanceConstraint("avoid Elk far", "Deer", 85.0);
	int avoidElk = rmCreateTypeDistanceConstraint("avoid  Elk", "Deer", 70.0);
	int avoidElkShort = rmCreateTypeDistanceConstraint("avoid  Elk short", "Deer", 50.0);
	...
	int avoidGoldFar = rmCreateClassDistanceConstraint ("gold avoid gold far", classGold, 85.0); //70
	int avoidGoldVeryFar = rmCreateClassDistanceConstraint ("gold avoid gold very ", classGold, 100.0);
	...
	int avoidNuggetFar = rmCreateTypeDistanceConstraint("nugget avoid nugget Far", "AbstractNugget", 68.0);
...
//  rmAddObjectDefItem(playerTreeID, "TreeNorthwestTerritory", rmRandInt(1,2), 4.0);
	rmAddObjectDefItem(playerTreeID, "ypTreeJapaneseMaple", 12, 6.0); //6,6 5.0
...
//		if (nugget0count == 2)
	...
	// ********** Mines ***********
		...
		int silvermineCount = 2+3*PlayerNum;  // 3,3 
...
*/		rmPlaceObjectDefAtLoc(silvermineID, 0, 0.50, 0.50);
	...
	// Forest
	int forestcount = 4+6*PlayerNum; // 14*PlayerNum/2
	int stayInForest = -1;
...
//		rmAddObjectDefConstraint(RandomtreeID, avoidEdge);
	...
	rmSetObjectDefMaxDistance(deerHerdID, rmXFractionToMeters(0.05));
	...
	int elkcount = extraHerds+4*PlayerNum;
	
	for(i=0; < elkcount)
	{
		int elkID = rmCreateObjectDef("south Elk"+i);
		rmAddObjectDefItem(elkID, "Deer", rmRandInt(10,10), 7.0);
	...
	//	rmAddObjectDefConstraint(elkID, stayMidIsland);	
		if (TeamNum <= 2 && PlayerNum == 2)
		{
			if (i < 1)
	...
	int fishcount = -1;
	fishcount = 6+5*PlayerNum;
	
	//Fish
	int fishID = rmCreateObjectDef("fish");
		rmAddObjectDefItem(fishID, "ypFishCatfish", rmRandInt(2,2), 8.0);
