// Mexico -- relocated from Garja's ARIZONA
// designed by Garja
...
//	rmSetBaseTerrainMix("painteddesert_groundmix_4"); //
	...
	int stayBack = rmCreatePieConstraint("Stay Back",0.5,0.5, rmXFractionToMeters(0.42),rmXFractionToMeters(0.45), rmDegreesToRadians(0),rmDegreesToRadians(360));
	int stayForward = rmCreatePieConstraint("Stay Forward",0.5,0.5, rmXFractionToMeters(0.25),rmXFractionToMeters(0.28), rmDegreesToRadians(0),rmDegreesToRadians(360));
...
//	int shortAvoidCliff=rmCreateClassDistanceConstraint("short stuff vs. cliff", classCliff, 8.0);
	...
	int avoidPronghornMed = rmCreateTypeDistanceConstraint("avoid food med", "pronghorn", 18.0);
...
//	int avoidGoldMed = rmCreateTypeDistanceConstraint("coin avoids coin", "gold", 30.0);
	...
	int avoidNativesFar = rmCreateClassDistanceConstraint("avoid natives far", classNative, 12.0);
...
//	rmSetAreaTerrainType(mesasurfaceID, "texas\ground5_tex");
	...
	if (TeamNum <3 && rmGetIsKOTH() == false)
	{	
		//1
	...
	else
	{
		...
		int mesa3ID = rmCreateArea("mesa3");
			rmSetAreaSize(mesa3ID, rmAreaTilesToFraction(255*PlayerNum), rmAreaTilesToFraction(255*PlayerNum));  
		...
		if (rmGetIsKOTH() == true)
			rmSetAreaCliffEdge(mesa3ID, 4+PlayerNum/2, 0.08, 0.0, 1.0, 0);
		else
	...
	if(natVariant ==1){
        ...
        nativeID0 = rmCreateGrouping("Maya village A", "native Maya village "+1); //SW
    }
	...
	if(natVariant ==1){
        ...
        nativeID2 = rmCreateGrouping("Maya village B", "native Maya village "+4); // NE
    }
	...
	{
		if (PlayerNum == 2)
			rmPlaceGroupingAtLoc(nativeID2, 0, 0.86, 0.44);
		else if (teamZeroCount < 3 && teamOneCount < 3)
    ...
    if(natVariant ==1){
        ...
        nativeID1 = rmCreateGrouping("zapotec village A", "native zapotec village "+1); // NW
    }
	...
	{
		if (PlayerNum == 2)
			rmPlaceGroupingAtLoc(nativeID1, 0, 0.44, 0.84); 
		else if (teamZeroCount < 3 && teamOneCount < 3)
    ...
    if(natVariant ==1){
        ...
        nativeID3 = rmCreateGrouping("zapotec village B", "native zapotec village "+5); // SE
    }
...
//		rmAddAreaConstraint(patchID, avoidEdge);
	...
	rmSetAreaTerrainType(internalpatch, "texas\river1_tex"); //borneo\shoreline1_borneo
...
//	rmAddObjectDefConstraint(playergoldID, avoidGoldType);
	...
	if (PlayerNum == 2)
		rmAddObjectDefConstraint(playergoldID, stayForward);
	
	// 2nd mine
	int playergold2ID = rmCreateObjectDef("player second mine");
	rmAddObjectDefItem(playergold2ID, "MineCopper", 1, 0);
	rmSetObjectDefMinDistance(playergold2ID, 40.0); //68
	rmSetObjectDefMaxDistance(playergold2ID, 40.0); //70
//	rmAddObjectDefToClass(playergold2ID, classStartingResource);
	...
	if (PlayerNum == 2)
		rmAddObjectDefConstraint(playergold2ID, stayBack);
	if (TeamNum <3)
	{
...
//	rmAddObjectDefConstraint(playergold2ID, avoidCenter);
	...
	rmSetObjectDefMaxDistance(playerbisonID, 13.0);
    ...
    rmSetObjectDefMaxDistance(playerPronghornID, 30);
	...
	rmAddObjectDefConstraint(playerPronghornID, avoidStartingResourcesShort);
//	rmAddObjectDefConstraint(playerPronghornID, avoidCenter);
	rmAddObjectDefConstraint(playerPronghornID, avoidEdge);
	if (PlayerNum == 2)
	{
		rmAddObjectDefConstraint(playerPronghornID, avoidMesaSurfaceShort);
		rmAddObjectDefConstraint(playerPronghornID, stayForward);
	}
	...
	rmAddObjectDefConstraint(playerPronghorn2ID, avoidStartingResourcesShort);
	if (PlayerNum == 2)
		rmAddObjectDefConstraint(playerPronghorn2ID, stayBack);
	if (TeamNum <3)
	{
...
//		rmPlaceObjectDefAtLoc(playergoldID, i, rmXMetersToFraction(xsVectorGetX(TCLoc)), rmZMetersToFraction(xsVectorGetZ(TCLoc)));
	...
	for(i=1; < copperCount+1)
	{
		...
		rmAddObjectDefConstraint(coppermineID, avoidKOTH);
//		if (i != copperCount)
//		{
			if (PlayerNum == 2)
			{
...
*/
		...
		rmAddObjectDefConstraint(silverIDSA, avoidNativesFar);
		rmAddObjectDefConstraint(silverIDSA, avoidTradeRouteSocket);
		rmAddObjectDefConstraint(silverIDSA, avoidGoldFar);
		rmAddObjectDefConstraint(silverIDSA, avoidTownCenter);
		rmAddObjectDefConstraint(silverIDSA, avoidMesaSurface);
		rmAddObjectDefConstraint(silverIDSA, avoidPatchMin);
		...
		while (minePlacement < copperCount/2) 
		{
	...
	for (i=0; < bigforestcount)
	{
		...
		rmSetAreaObeyWorldCircleConstraint(bigforestID, true);
...
//		rmAddAreaToClass(bigforestID, classForest);
		...
		rmAddAreaConstraint(bigforestID, avoidKOTH);
		if (i < bigforestcount/2)
	...
	for (i=0; < treesmallcount)
	{
		...
		rmAddObjectDefConstraint(randomsmalltreeID, avoidKOTH);
...
//		rmAddObjectDefConstraint(bisonID, avoidTradeRoute);
	...
	*/
		...
		rmAddObjectDefItem(mapHuntAID, "bison", 9, 5.0);
...
//		rmAddObjectDefConstraint(mapHuntAID, avoidTradeRoute);
		rmAddObjectDefConstraint(mapHuntAID, avoidImpassableLand);
		rmAddObjectDefConstraint(mapHuntAID, avoidNativesFar);
		rmAddObjectDefConstraint(mapHuntAID, avoidTradeRoute);
		rmAddObjectDefConstraint(mapHuntAID, avoidGoldShort);		
		rmAddObjectDefConstraint(mapHuntAID, avoidTownCenter);
		rmAddObjectDefConstraint(mapHuntAID, avoidKOTH);
		rmAddObjectDefConstraint(mapHuntAID, avoidMesaSurfaceShort);
		if (TeamNum <3)
		{
			rmAddObjectDefConstraint(mapHuntAID, avoidMesa);
			rmAddObjectDefConstraint(mapHuntAID, avoidMesa2);
		}
		else
			rmAddObjectDefConstraint(mapHuntAID, avoidMesa3);
		rmAddObjectDefConstraint(mapHuntAID, avoidPronghorn); 
		rmAddObjectDefConstraint(mapHuntAID, avoidBisonFar); 
		rmAddObjectDefConstraint(mapHuntAID, avoidPatchMin);
		rmAddObjectDefConstraint(mapHuntAID, avoidEdge);

		int mapHuntBID = -1;
		mapHuntBID = rmCreateObjectDef("hunt B");
		rmAddObjectDefItem(mapHuntBID, "bison", 9, 5.0);
		...
		while (huntPlacement < PlayerNum/2) 
		{
			huntPositionX=rmRandFloat(0.06,0.94);
			huntPositionZ=rmRandFloat(0.06,0.94);
...
*/
		int mapHuntA2ID = -1;
		mapHuntA2ID = rmCreateObjectDef("hunt A2");
		rmAddObjectDefItem(mapHuntA2ID, "bison", 9, 5.0);
		rmSetObjectDefMinDistance(mapHuntA2ID, rmXFractionToMeters(0.00));
		rmSetObjectDefMaxDistance(mapHuntA2ID, rmXFractionToMeters(0.00));
		rmSetObjectDefCreateHerd(mapHuntA2ID, true);
		rmAddObjectDefConstraint(mapHuntA2ID, avoidTradeRouteShort);
		rmAddObjectDefConstraint(mapHuntA2ID, avoidImpassableLand);
		rmAddObjectDefConstraint(mapHuntA2ID, avoidNativesShort);
		rmAddObjectDefConstraint(mapHuntA2ID, avoidTradeRouteSocket);
		rmAddObjectDefConstraint(mapHuntA2ID, avoidGoldTypeShort);
		rmAddObjectDefConstraint(mapHuntA2ID, avoidTownCenter);
		rmAddObjectDefConstraint(mapHuntA2ID, avoidEdge);
		rmAddObjectDefConstraint(mapHuntA2ID, avoidKOTH);
		rmAddObjectDefConstraint(mapHuntA2ID, avoidMesaSurfaceShort);
		if (TeamNum <3)
		{
			rmAddObjectDefConstraint(mapHuntA2ID, avoidMesaFar);
			rmAddObjectDefConstraint(mapHuntA2ID, avoidMesa2Far);
		}
		else
			rmAddObjectDefConstraint(mapHuntA2ID, avoidMesa3Far);
		rmAddObjectDefConstraint(mapHuntA2ID, avoidPronghorn); 
		rmAddObjectDefConstraint(mapHuntA2ID, avoidBison); 
		rmAddObjectDefConstraint(mapHuntA2ID, avoidForestMin); 
		rmAddObjectDefConstraint(mapHuntA2ID, avoidPatchMin);

		int mapHuntB2ID = -1;
		mapHuntB2ID = rmCreateObjectDef("hunt B2");
		rmAddObjectDefItem(mapHuntB2ID, "bison", 9, 5.0);
		rmSetObjectDefMinDistance(mapHuntB2ID, rmXFractionToMeters(0.00));
		rmSetObjectDefMaxDistance(mapHuntB2ID, rmXFractionToMeters(0.00));
		rmSetObjectDefCreateHerd(mapHuntB2ID, true);
		...
		while (huntPlacement < pronghornCount/2) 
		{
			huntPositionX=rmRandFloat(0.06,0.94);
			huntPositionZ=rmRandFloat(0.06,0.94);
			resultF=rmPlaceObjectDefAtLoc(mapHuntAID, 0, huntPositionX, huntPositionZ);
