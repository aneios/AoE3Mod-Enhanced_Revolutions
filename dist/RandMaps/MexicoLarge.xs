// LARGE Mexico -- relocated from Garja's ARIZONA
// designed by Garja
...
//	rmSetBaseTerrainMix("painteddesert_groundmix_4"); //
	...
	// Resource avoidance
	int avoidForestBig=rmCreateClassDistanceConstraint("avoid forest big", classForestBig, 55.0+2*PlayerNum); //15.0
...
//	int shortAvoidCliff=rmCreateClassDistanceConstraint("short stuff vs. cliff", classCliff, 8.0);
	int avoidPronghornFar = rmCreateTypeDistanceConstraint("avoid food far", "pronghorn", 64.0);
	int avoidPronghorn = rmCreateTypeDistanceConstraint("avoid food", "pronghorn", 50.0);
	...
	int avoidBisonFar = rmCreateTypeDistanceConstraint("avoid bison far", "bison", (70.0-2*PlayerNum));
	int avoidBison = rmCreateTypeDistanceConstraint("avoid bison", "bison", 55.0);
...
//	int avoidGoldMed = rmCreateTypeDistanceConstraint("coin avoids coin", "gold", 30.0);
	...
	int avoidGoldFar = rmCreateClassDistanceConstraint ("avoid gold far", classGold, (82.0-2*PlayerNum));
	int avoidGoldShort = rmCreateClassDistanceConstraint ("avoid gold short", classGold, 8.0);
	int avoidNuggetFar = rmCreateTypeDistanceConstraint("nugget avoid nugget Far", "AbstractNugget", (75.0-1*PlayerNum));
...
//	rmSetAreaTerrainType(mesasurfaceID, "texas\ground5_tex");
	...
	if(natVariant ==1){
        ...
        nativeID2 = rmCreateGrouping("Maya village A", "native Maya village "+4); // NE
    }
...
//		}
	...
	//Forest
	int bigforestcount = 4+5*PlayerNum;
...
//		rmAddObjectDefConstraint(bisonID, avoidTradeRoute);
	...
	int	nugget2count = 4+2*PlayerNum; 
	int	nugget3count = 2+PlayerNum; 
	int	nugget4count = -1;
	if (TeamNum <= 2)
