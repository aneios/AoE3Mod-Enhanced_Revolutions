// LARGE Minas Gerais (formerly vivid's Alberta merged with vivid's Twin Sisters)
...
// large map by vividlyplain, September 2021
...
void main(void)
{
	...
	int avoidForestMed=rmCreateClassDistanceConstraint("avoid forest med", rmClassID("Forest"), 40.0);
	int avoidForest=rmCreateClassDistanceConstraint("avoid forest", rmClassID("Forest"), 30.0);
	int avoidForestFar=rmCreateClassDistanceConstraint("avoid forest far", rmClassID("Forest"), 50.0);
	int avoidForestVeryFar=rmCreateClassDistanceConstraint("avoid forest very far", rmClassID("Forest"), 64.0);
	...
	int avoidHunt3Far = rmCreateTypeDistanceConstraint("avoid hunt3 far", food3, 70.0);
	int avoidHunt3VeryFar = rmCreateTypeDistanceConstraint("avoid hunt3 very far", food3, 90.0);
	int avoidHunt3 = rmCreateTypeDistanceConstraint("avoid hunt3", food3, 50.0);
	int avoidHunt3Med = rmCreateTypeDistanceConstraint("avoid hunt3 med", food3, 40.0);
	...
	int avoidHunt2Far = rmCreateTypeDistanceConstraint("avoid hunt2 far", food2, 70.0);
	...
	int avoidHunt2Med = rmCreateTypeDistanceConstraint("avoid hunt2 med", food2, 40.0);
	...
	int avoidHunt1Far = rmCreateTypeDistanceConstraint("avoid hunt1 far", food1, 70.0);
	...
	int avoidHunt1Med = rmCreateTypeDistanceConstraint("avoid hunt1 med", food1, 40.0);
	...
	int avoidGoldType = rmCreateTypeDistanceConstraint("coin avoids coin ", "gold", 44.0);
	...
	int avoidNuggetFar=rmCreateTypeDistanceConstraint("avoid nugget far", "AbstractNugget", 75.0);
	int avoidNuggetVeryFar=rmCreateTypeDistanceConstraint("avoid nugget very far", "AbstractNugget", 100.0);
...
//	rmSetAreaMix(midSmIslandID, "great plains drygrass"); 	// for testing
	...
	int southherdID = rmCreateObjectDef("south herd");
		...
		if (PlayerNum == 2) {
			...
			rmPlaceObjectDefAtLoc(southherdID, 0, 0.50, 0.50, 2.5*PlayerNum);	
			}
	...
	int northherdID = rmCreateObjectDef("north herd");
		...
		if (PlayerNum == 2) {
			...
			rmPlaceObjectDefAtLoc(northherdID, 0, 0.50, 0.50, 2.5*PlayerNum);
			}
	...
	for (i=0; < mainforestcount)
    {
        ...
        rmSetAreaSize(forestpatchID, rmAreaTilesToFraction(150));
		...
		int foresttree2ID = rmCreateObjectDef("second forest trees"+i);
			rmAddObjectDefItem(foresttree2ID, treeType1, 4, 8.0);
			rmAddObjectDefItem(foresttree2ID, treeType4, 2, 6.0);
	...
	for (i=0; < secondforestcount)
	{
		...
		rmSetAreaSize(secondforestID, rmAreaTilesToFraction(66));
