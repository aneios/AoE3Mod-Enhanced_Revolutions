...
void main(void)
{
	...
	int avoidForestFar = rmCreateClassDistanceConstraint("forest avoids forest far", rmClassID("classForest"), 64.0);
	int avoidCapybara = rmCreateTypeDistanceConstraint("avoids Capybara", "capybara", 64.0);
	int avoidTapir = rmCreateTypeDistanceConstraint("Tapir avoids Tapir", "Tapir", 64.0);
	int avoidTapirMin = rmCreateTypeDistanceConstraint("Tapir avoids Tapir min", "Tapir", 4.0);
	int avoidTapirFar = rmCreateTypeDistanceConstraint("Tapir avoids Tapir Far", "Tapir", 75.0);
	int avoidCoin = rmCreateTypeDistanceConstraint("avoid coin", "gold", 64.0);
	int avoidCoinFar = rmCreateTypeDistanceConstraint("avoid coin far", "gold", 80.0);
	...
	int avoidGoldFar = rmCreateClassDistanceConstraint("avoid gold far", rmClassID("classGold"), 64.0);
	int avoidGoldVeryFar = rmCreateClassDistanceConstraint("avoid gold very far", rmClassID("classGold"), 80.0);
...
//	rmSetAreaMix(midSmallIslandID, "testmix");   // for testing
	...
	{
		rmRiverSetShallowRadius(riverID, 24 + cNumberNonGaiaPlayers);
	}
	if (cNumberTeams == 2)
...
//	rmAddObjectDefConstraint(randTreeID, stayMidIsland);
	...
	if (PlayerNum > 2 && rmGetIsTreaty() == false)
		rmPlaceObjectDefAtLoc(nuggetnutsWID, 0, 0.5, 0.5, PlayerNum);
