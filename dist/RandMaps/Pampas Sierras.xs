...
//	rmAddGroupingConstraint(nativeID3, avoidNatives);
	...
	rmSetAreaSize(midIslandID, 0.39+0.01*PlayerNum);
...
//	rmAddAreaConstraint(midSmIslandID, avoidWaterShort);
	...
	rmSetObjectDefMinDistance(playergold2ID, 36.0); //58
	rmSetObjectDefMaxDistance(playergold2ID, 36.0); //62
...
//	rmAddObjectDefConstraint(playergold2ID, avoidCenter);
	...
	rmAddObjectDefItem(playerTreeID, mainTreeType, 2, 2.0);
...
//		rmAddAreaConstraint(forestpatchID, avoidPatch4);
		...
		int foresttreeID = rmCreateObjectDef("forest trees"+i);
			rmAddObjectDefItem(foresttreeID, mainTreeType, 1, 3.0);
			rmAddObjectDefItem(foresttreeID, mainTreeType, 2, 3.0);
		...
		//	rmAddObjectDefConstraint(foresttreeID, avoidForestShort);
			...
			rmPlaceObjectDefAtLoc(foresttreeID, 0, 0.50, 0.50, 20);
    }
	...
	//	rmAddAreaToClass(secondforestID, classForest);
		...
		int secondforesttreeID = rmCreateObjectDef("secondary forest trees"+i);
			rmAddObjectDefItem(secondforesttreeID, mainTreeType, 2, 2.0);
		...
		//	rmAddObjectDefConstraint(secondforesttreeID, avoidForestShort);
			...
			rmPlaceObjectDefAtLoc(secondforesttreeID, 0, 0.50, 0.50, 10);
	}
	...
	for (i=0; < grasszonetreecount)
	{
		...
		rmAddAreaConstraint(grasszonetreeID, avoidStartingResources);
	...
	for (i=0; < 3*PlayerNum)
		{
			int plateautreeID = rmCreateObjectDef("plateau trees"+i);
			...
			rmAddObjectDefConstraint(plateautreeID, avoidStartingResources);
...
//		rmAddObjectDefConstraint(centralguanacoID, avoidPlateau);
		...
		rmAddObjectDefConstraint(centralguanacoID, avoidStartingResources);
	...
	for(i=0; < grasszoneherdcount)
	{
		...
		rmAddObjectDefConstraint(eastrheaID, avoidStartingResources);
...
//	rmAddObjectDefConstraint(Nugget2ID, avoidGrasszone);
	...
	for (i=0; < 4*PlayerNum)
	{
		...
		rmAddObjectDefConstraint(GrasszoneunderbrushID, avoidStartingResources);
	...
	for (i=0; < 10*PlayerNum)
	{
		...
		rmAddObjectDefConstraint(PlateauunderbrushID, avoidStartingResources);
