// Punjab -- Relocated from Garja's ARKANSAS (1v1, TEAM, FFA)
// designed by Garja
...
//	int avoidWesthalf = rmCreateAreaDistanceConstraint("avoid west", WestID, 185.0); 
	...
	int avoidGoldVeryFar = rmCreateClassDistanceConstraint ("gold avoid gold very ", classGold, 60.0);
...
//  rmAddObjectDefItem(playerTree2ID, "ypTreeDeccan", rmRandInt(1,2), 4.0);
	...
	rmAddObjectDefItem(playerturkeyID, "ypNilgai", rmRandInt(6,6), 3.0);
	rmSetObjectDefMinDistance(playerturkeyID, 13.0);
	rmSetObjectDefMaxDistance(playerturkeyID, 13.0);
    ...
    rmSetObjectDefMaxDistance(playerypNilgaiID, 26);
...
//		rmAddObjectDefConstraint(RandomtreeID, avoidEdge);
    ...
    int avoidHerdables=rmCreateTypeDistanceConstraint("avoids cattle 2", "ypWaterBuffalo", 28.0+5*PlayerNum); 
