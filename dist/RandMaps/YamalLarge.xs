...
void main(void) 
{
	...
	int avoidHunt=rmCreateTypeDistanceConstraint("hunts avoid hunts", "huntable", 75.0);
    ...
    int avoidHerd=rmCreateTypeDistanceConstraint("herds avoid herds", "herdable", 125.0);

	int avoidCoin=rmCreateTypeDistanceConstraint("avoid coin", "mineTin", 8.0);
    int avoidCoinMed=rmCreateTypeDistanceConstraint("avoid coin medium", "mineTin", 75.0);
...
//    rmSetAreaBaseHeight(continent2, -2.0);
	...
	nativeID1 = rmCreateGrouping("native B", natGrpName1+rmRandInt(1,5));
	nativeID2 = rmCreateGrouping("native C", natGrpName1+rmRandInt(1,5));
...
//		rmPlaceGroupingAtLoc(nativeID0, 0, xNatLocA, yNatLocA);
    ...
    rmAddObjectDefItem(treeID, treeType1, 5, 6.0);
    rmAddObjectDefItem(treeID, treeType2, 5, 6.0);
    rmAddObjectDefItem(treeID, treeType3, 5, 6.0);
    rmSetObjectDefMinDistance(treeID, 16.0);
    rmSetObjectDefMaxDistance(treeID, 16.0);
	...
	*/
    ...
    // Forests
    int mainforestcount = 25 + 8 * PlayerNum;
	...
	int fishVsFishID=rmCreateTypeDistanceConstraint("fish v fish", "FishSalmon", 24.0);
	int fishLand = rmCreateTerrainDistanceConstraint("fish land", "land", true, 4.0);
	int whaleVsWhaleID=rmCreateTypeDistanceConstraint("whale v whale", "Beluga", 90);
	...
	rmPlaceObjectDefAtLoc(fishID, 0, 0.5, 0.5, 20+12*cNumberNonGaiaPlayers);
