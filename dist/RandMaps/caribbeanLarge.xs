// LARGE Caribbean
// Nov 06 - Yp update
// updated June 2021 by vividlyplain
...
void main(void)
{
  ...
  // Text
   ...
   rmSetSeaType("caribbean coast");
 	rmSetBaseTerrainMix("caribbean grass");		// caribbeanSkirmish
   ...
   int playerEdgeConstraintShort=rmCreatePieConstraint("player edge of map short", 0.5, 0.5, rmXFractionToMeters(0.0), rmXFractionToMeters(0.48), rmDegreesToRadians(0), rmDegreesToRadians(360));
  ...
  int avoidTCMedium=rmCreateTypeDistanceConstraint("stay away from TC by a bit", "TownCenter", 12.0);
	...
	int avoidNativesMed=rmCreateClassDistanceConstraint("stuff avoids natives med", rmClassID("natives"), 30.0);
   ...
   int islandConstraintFar=rmCreateClassDistanceConstraint("islands avoid each other far", classIsland, 60.0);
  // int bonusIslandConstraint=rmCreateClassDistanceConstraint("bonus islands avoid islands", classIsland, 30.0);
   ...
   int whaleVsWhaleID=rmCreateTypeDistanceConstraint("whale v whale", "MinkeWhale", 70.0);
...
//	else
      ...
      rmSetAreaSmoothDistance(bigIslandID, 20);
		rmSetAreaMix(bigIslandID, "caribbean grass");	// caribbeanSkirmish
  ...
  float yLoc = 0.0;
	...
	rmPlacePlayersCircular(0.33, 0.33, 0);	// rmDegreesToRadians(5.0)
	float isleSize = 0.00;
	if (cNumberTeams == 2)
		isleSize = (0.29 / cNumberTeams);
	else
		isleSize = (0.24 / cNumberTeams);
  ...
  if (cNumberTeams == 2)
	...
	playerNativeID = rmCreateGrouping("player native", "native carib village 0"+whichSettlement);
	...
	rmAddGroupingConstraint(playerNativeID, avoidCW);
  ...
  rmAddObjectDefConstraint(playerSilverID, avoidNativesShort);

	int playerDeerID=rmCreateObjectDef("player deer");
  rmAddObjectDefItem(playerDeerID, "deer", 15, 10.0);
  ...
  rmAddObjectDefConstraint(playerDeerID, avoidNativesShort);
  ...
  for (i=0; <numTries) {   
    ...
    rmSetAreaForestType(forest, "Caribbean Palm Forest Skirmish");
 ...
 // Define and place Nuggets
  ...
  if(rmGetIsKOTH()) {
	...
	for (i=0; <cNumberTeams)
	{
		rmPlaceObjectDefInArea(nugget1, 0, rmAreaID("team "+i), cNumberNonGaiaPlayers*3);
	}
