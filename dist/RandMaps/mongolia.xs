...
//	rmSetBaseTerrainMix(paintMix1); 
	...
	int avoidHunt2Far = rmCreateTypeDistanceConstraint("avoid hunt2 far", huntable2, 60.0);
	int avoidHunt2 = rmCreateTypeDistanceConstraint("avoid hunt2", huntable2, 40.0);
	...
	int avoidHunt1Far = rmCreateTypeDistanceConstraint("avoid hunt1 far", huntable1, 60.0);
	int avoidHunt1 = rmCreateTypeDistanceConstraint("avoid hunt1 ", huntable1, 40.0);
	...
	int avoidNuggetVeryFar = rmCreateTypeDistanceConstraint("nugget avoid nugget Very Far", "AbstractNugget", 75.0);
	...
	int avoidCattle=rmCreateTypeDistanceConstraint("avoid cattle", herdable1, 44.0);
	int avoidCattleMed=rmCreateTypeDistanceConstraint("avoid cattle med", herdable1, 32.0-PlayerNum);
	int avoidCattleShort=rmCreateTypeDistanceConstraint("avoid cattle short", herdable1, 22.0-PlayerNum);
...
//	rmSetAreaMix(midLineID, "coastal_japan_b"); 	// for testing
	...
	rmSetGroupingMaxDistance(playerNativeID, 32);
  ...
  rmSetObjectDefMaxDistance(playerTreeID, 20);
	...
	rmSetObjectDefMinDistance(playerYak2ID, 60.0);
	rmSetObjectDefMaxDistance(playerYak2ID, 90.0);
	...
	if (PlayerNum == 2)
		rmAddObjectDefConstraint(playerYak2ID, stayMidLine);
	if (TeamNum == 2 && teamZeroCount == teamOneCount)
	...
	int gold2ID = rmCreateObjectDef("gold north");
		...
		if (PlayerNum > 2) {
			...
			rmPlaceObjectDefAtLoc(gold2ID, 0, 0.50, 0.50, PlayerNum*3);
			}
		else {
	...
	// Main forest patches
	int mainforestcount = 30+15*PlayerNum;
	...
	int Nugget3TID = rmCreateObjectDef("nugget L3 top"); 
		...
		if (PlayerNum == 2)	
			...
			rmAddObjectDefConstraint(Nugget3TID, avoidNuggetShort);
		else
			rmAddObjectDefConstraint(Nugget3TID, avoidNugget);
