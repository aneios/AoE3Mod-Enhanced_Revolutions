// LARGE ROCKIES - started with a modified version of Great Plains.
// July 2004
// Nov 06 - YP update
...
//		switchAroo = 2; // for testing
	...
	string paintMix1 = "rockies_grass";
	...
	string food1 = "bighornsheep";
	string food2 = "elk";
	string treeType1 = "TreeYukon";
	...
	string natType1 = "Cheyenne";
	string natType2 = "";
	string natGrpName1 = "native cheyenne village ";
	string natGrpName2 = "";
	...
	rmTerrainInitialize(cliffPaint1, 8.00); 
	...
	int avoidHunt2 = rmCreateTypeDistanceConstraint("avoid hunt2", food2, 75.0);
...
//	rmBuildArea(outerRimID);
	...
	rmSetAreaSize(playerLevelID,0.64);
...
//	rmSetAreaMix(midIslandID, forTesting);
	...
	rmSetObjectDefMaxDistance(playerHerdID, 13);
	...
	rmAddObjectDefItem(playerHerd3ID, food1, 8, 6.0);
...
//		rmAddObjectDefConstraint(playerNuggetID, avoidMidIsland);
	...
	rmSetObjectDefMaxDistance(staticgoldID, rmXFractionToMeters(0.05));
	...
	if (PlayerNum > 2) {
		rmPlaceObjectDefAtLoc(mapMinesID, 0, 0.50, 0.50, (3*PlayerNum));
		if (TeamNum > 2 || teamZeroCount != teamOneCount)
	...
	for (i=0; < valleyforestcount)
    {
		...
		rmSetAreaObeyWorldCircleConstraint(valleyForestPatchID, false);
...
//		rmAddAreaConstraint(valleyForestPatchID, stayValley); 
		...
		int valleyForestTreeID = rmCreateObjectDef("valley forest trees"+i);
			rmAddObjectDefItem(valleyForestTreeID, treeType6, 2+PlayerNum/2, 3+PlayerNum);
			rmAddObjectDefItem(valleyForestTreeID, treeType7, 2+PlayerNum/2, 3+PlayerNum);
	...
	for (i=0; < mountainforestcount)
    {
		...
		rmSetAreaObeyWorldCircleConstraint(mountainForestPatchID, false);
	...
	if (PlayerNum > 2) {
		rmPlaceObjectDefAtLoc(mapHuntsID, 0, 0.50, 0.50, (5*PlayerNum));
		if (TeamNum > 2 || teamZeroCount != teamOneCount)
	...
	int playerLevelPropsID = rmCreateObjectDef("player level decor");
		rmAddObjectDefItem(playerLevelPropsID, propType1, rmRandInt(1,4), 8.0);
		rmAddObjectDefItem(playerLevelPropsID, propType2, rmRandInt(2,3), 8.0);
		rmAddObjectDefItem(playerLevelPropsID, propType5, rmRandInt(1,3), 8.0);
		...
		rmPlaceObjectDefAtLoc(playerLevelPropsID, 0, 0.50, 0.50, 10+5*PlayerNum);
