// LARGE Land Unknown adapted to Rocket
// ver 11 Jan 2005
...
//		everyoneGetsAWagon = 984;	// for testing
...
// ============= Constraints =============
   ...
   int avoidFood = rmCreateTypeDistanceConstraint("food avoids food", "food", 40.0);
	int avoidFood1 = rmCreateTypeDistanceConstraint("food avoids food1", critterOneName, 64.0);
	int avoidFood2 = rmCreateTypeDistanceConstraint("food avoids food2", critterTwoName, 64.0);
	int avoidFood1Far = rmCreateTypeDistanceConstraint("food avoids food1 far", critterOneName, 64+2.5*cNumberNonGaiaPlayers);
	int avoidFood2Far = rmCreateTypeDistanceConstraint("food avoids food2 far", critterTwoName, 64+2.5*cNumberNonGaiaPlayers);
	int avoidSilver = rmCreateTypeDistanceConstraint("fast coin avoids coin", "gold", 75+5*cNumberNonGaiaPlayers);
	int avoidHuntable = rmCreateTypeDistanceConstraint("hunt avoids hunt", "huntable", 64);
	...
	int avoidGold = rmCreateClassDistanceConstraint("avoid gold class", classGold, 64);
	int avoidGoldFar = rmCreateClassDistanceConstraint("stay away from minerals far", classGold, 40+5*cNumberNonGaiaPlayers);
	int avoidGoldVeryFar = rmCreateClassDistanceConstraint("stay away from minerals very far", classGold, 80+2*cNumberNonGaiaPlayers);
...
//	rmAddClosestPointConstraint(shortAvoidImpassableLand);
   vector tradeRoutePoint = cOriginVector;
	...
	{
		// If continent, scoot linear trade route in a bit
		if(oceanRing == 1)
		{
   ...
   rmSetObjectDefTradeRouteID(socketID, tradeRouteID);
   vector socketLoc = rmGetTradeRouteWayPoint(tradeRouteID, tpLocStart);
   rmPlaceObjectDefAtPoint(socketID, 0, socketLoc);

	// If continent, then 2 slots. Otherwise 4.
	if(oceanRing < 1)
	{
   ...
   socketLoc = rmGetTradeRouteWayPoint(tradeRouteID, tpLocEnd);
   rmPlaceObjectDefAtPoint(socketID, 0, socketLoc);
...
// Sometimes teams closer together
	...
	float playerFraction = rmAreaTilesToFraction(555);
...
//   rmAddObjectDefConstraint(ripTCID, shortAvoidImpassableLand);
  ...
  rmAddObjectDefItem(farDeerID, critterTwoName, rmRandInt(14,16), 8.0);
   ...
   if (oceanRing == 1)
   {
	...
	rmAddObjectDefConstraint(startSilver3ID, avoidPlayers);
...
// Silver
	...
	if (rmGetNomadStart() == true)
		silverCount = cNumberNonGaiaPlayers*7;
	if (oceanRing == 1)
		silverCount = cNumberNonGaiaPlayers*6;
	if (rmGetIsTreaty() == true)
		silverCount = cNumberNonGaiaPlayers*4.5;
...
//Food
   ...
   if (rmGetNomadStart() == false)
   {
   		...
   		rmPlaceObjectDefAtLoc(bisonID, 0, 0.5, 0.5, cNumberNonGaiaPlayers*3.5);
   }
   else
   {
   		rmAddObjectDefConstraint(bisonID, avoidPlayers);
   		rmPlaceObjectDefAtLoc(bisonID, 0, 0.5, 0.5, 2+cNumberNonGaiaPlayers*3);
   }
...
int nuggetUnknownID= rmCreateObjectDef("unknown special nuggets"); 
	...
	if (cNumberNonGaiaPlayers == 2 || rmGetIsFFA() == true)
  	...
  	rmPlaceObjectDefAtLoc(nuggetUnknownID, 0, 0.5, 0.5, 6+cNumberNonGaiaPlayers*3);
...
int houseNuggetID= rmCreateObjectDef("house nugget");
	...
	if ((saguenayMap == 1 && someRNG <= 0.5) || (euMap == 1 && someRNG <= 0.5) || (asianMap == 1 && someRNG <= 0.5))
		rmPlaceObjectDefAtLoc(houseNuggetID, 0, 0.5, 0.5, 1+cNumberNonGaiaPlayers);
...
int nuggetID= rmCreateObjectDef("map nugget"); 
	...
	if (placeWaterFlag == 1)
	{
		...
		rmPlaceObjectDefAtLoc(fishID, 0, 0.5, 0.5, 6*cNumberNonGaiaPlayers + 10);
		if (oceanOffCenter == 1 && oceanMiddle == 1)
			rmPlaceObjectDefAtLoc(fishID, 0, 0.5, 0.5, 5*cNumberNonGaiaPlayers + 10);
		if (oceanRing == 1 && oceanMiddle == 1)
			rmPlaceObjectDefAtLoc(fishID, 0, 0.5, 0.5, 5*cNumberNonGaiaPlayers + 10);
		if (oceanRing == 1 && riverExists == 1)
			rmPlaceObjectDefAtLoc(fishID, 0, 0.5, 0.5, 2*cNumberNonGaiaPlayers + 10);
		if (oceanOffCenter == 1 && riverExists == 1)
			rmPlaceObjectDefAtLoc(fishID, 0, 0.5, 0.5, 2*cNumberNonGaiaPlayers + 10);
		if (oceanRing == 1)
			rmPlaceObjectDefAtLoc(fishID, 0, 0.5, 0.5, 6*cNumberNonGaiaPlayers + 10);
